...
| [`dev-setup.sh`](dev-setup.sh) | Installs system build dependencies (apt/dnf/pacman on Linux; Xcode Command Line Tools and Homebrew `openssl`/`pkg-config` on macOS), Rust via rustup, `cargo-watch`, and pnpm. May run package-manager commands with sudo. | Package-manager installs need root on most hosts. Idempotent (existing installs are detected and skipped). |
