...
//!
//! # Why a gate, when the closure is already clean
//!
//! It is clean because `deploy-remote` is off by default and carries the only
//! `mvm-http` edge. Nothing was holding that. The dependency-cutting work is
//! done and the gates are the gap: one default-on feature, or one new
//! unconditional dependency, quietly puts a C TLS stack back into the closure
//! that every language SDK loads in-guest, and it surfaces as a
//! cross-compilation failure a long way from the edit that caused it.
//!
//! The check reads crate names, not the feature name, so renaming or
//! restructuring the feature does not blind it.
