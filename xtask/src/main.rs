...
mod check_core_module_ownership;
...
fn main() -> Result<()> {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("check-core-module-ownership") => {
            let workspace = workspace_root();
            check_core_module_ownership::run(&workspace)
        }
        Some("check-public-function-names") => {
            let workspace = workspace_root();
            check_public_function_names::run(&workspace)
        }
        ...
        Some("image-mirror") => image_mirror::run(&args[2..]),
        ...
        Some("check-single-display-path") => {
            let workspace = workspace_root();
        ...
        None => {
            ...
            eprintln!(
                "  check-core-module-ownership             Keep retired modules out of mvm-core and single-consumer modules with their owners"
            );
            eprintln!(
            ...
            eprintln!(
                "  telemetry-baseline                     measure emit-path baselines (hardware-dependent; not a gate)"
            );
            eprintln!(
            ...
            eprintln!(
                "  image-mirror <manifest> <source> <mirror>  Verify legacy mirror bytes against the canonical release"
            );
            eprintln!(
