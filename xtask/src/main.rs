...
fn main() -> Result<()> {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("check-plan-numbers") => {
            let workspace = workspace_root();
            check_plan_numbers::run(&workspace)
        }
        ...
        None => {
            ...
            eprintln!(
                "  check-plan-numbers                      CI gate — fail if a new specs/plans/ number collides"
            );
            println!(
