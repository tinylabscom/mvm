...
If an integration needs language-neutral automation today, load
`libmvm_hostlib`, the C ABI the Python and TypeScript SDKs bind to (JSON in,
JSON out; see `crates/mvm-hostlib/README.md`), or call `mvmctl` with JSON
output and explicit state directories. The SDKs themselves only ever use the
library.
