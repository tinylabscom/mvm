...
`mvm` supports local microVM workflows on native Linux with KVM and on macOS 26+
Apple Silicon through native HVF. Native Windows is still future host work. Linux hosts without `/dev/kvm`
...
| macOS 26+ Apple Silicon   | aarch64         | HVF                        | Supported               | Auto-detect default on this tier; Hypervisor.framework with no Homebrew or libkrun prerequisites. OCI `--allow-host` runs use the HVF no-guest-NIC host-vsock-proxy path when `mvm-hvf-supervisor` is available; otherwise the CLI fails closed before pull/boot work. |
| macOS 13–25 Apple Silicon | aarch64         | None                       | Unsupported             | Update to macOS 26+ or use a Linux KVM host; standard builds and releases do not ship the optional libkrun integration. |
...
| Browser (Chromium/Chrome) | Any             | WebLinux                   | Dev/test                | Browser-tier backend (`--hypervisor web-linux`); boots a real Nix-built Linux kernel under QEMU-Wasm inside the browser's WebAssembly engine. No hypervisor boundary, so it is a claim-free tier for demos and browser-local development.                                                                          |
...
| WSL2 with nested KVM      | x86_64, aarch64 | None auto-selected         | Experimental            | Standard builds and releases do not ship libkrun. Only the explicit QEMU dev/test tier is available; keep both the repo and `MVM_HOME` on the WSL ext4 filesystem rather than `/mnt/<drive>/...`.                         |
...
| WSL2 with nested KVM | No standard workload runtime; the explicit QEMU tier remains dev/test-only.                           | Explicit development setup only                          |
...
- Browsers boot through the WebLinux backend, running a real Nix-built Linux kernel under QEMU-Wasm inside the browser's own WebAssembly engine.
...
- WSL2 has no standard workload backend; QEMU remains an explicit dev/test tier.
...
| ------------------------------ | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Live-memory snapshot/restore   | Resume captured guest RAM and device state.                                              | Advertised by the Firecracker runner: `machine fork` of a running machine captures and resumes it, and each forked child restores the saved state with its device paths remapped into its own state dir. |
| Save/restore machine state     | Restore a serialized VMM machine state without claiming live-memory fidelity.            | Advertised by the HVF runner (`machine fork` of a running machine resumes from its saved state).                                              |
...
| Native HVF macOS path           | Supported local microVM path with no third-party VMM dependency. OCI `--allow-host` on `--image` is the HVF host-vsock-proxy path, not guest-NIC networking. |
