...
| Save/restore machine state     | Restore a serialized VMM machine state without claiming live-memory fidelity.            | Advertised by the HVF runner (`machine fork` of a running machine resumes from its saved state). Live-memory capture on Firecracker remains behind its per-child network model. |
