...
| `mvmctl run --prod --image <ref@sha256:...>`                        | Require the digest-pinned reference to satisfy the OCI policy, boot its sealed rootfs, and run the image's declared Entrypoint/Cmd via the ProdSafe `RunEntrypoint` verb under the signed plan. The plan must grant `run-entrypoint`; the guest refuses the dispatch without it. A trailing command, `--launch-plan`, and `--profile dev` remain refused before anything is pulled because they require DevOnly behavior                                                                                                                                |
| `mvmctl image pull --prod <ref@sha256:...>`                         | Verify the digest-pinned image against the OCI policy and build/cache its sealed rootfs without running it                                                                                                                                                                                                                                                                                                                          |
