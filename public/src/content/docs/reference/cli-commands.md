...
| `mvmctl secret set <name> ... --approve ask`              | Require an approval before a run first uses the secret toward a destination (`never` by default). `secret ls` shows `approve=ask` |
...
| `mvmctl run --approval <tty\|deny\|webhook=URL> -- <cmd>`               | Choose who answers an `ask` route rule or a secret bound with `--approve ask`. Repeatable; `--approval-mode all\|any` (default `all`) combines several. Replaces `[approval]` in `mvm.toml`; with neither, the terminal when an operator is at one, deny otherwise. Also on `machine run`. See [Runtime approvals](/guides/runtime-approvals/)                                         |
