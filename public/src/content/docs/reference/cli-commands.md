...
| `MVM_BUILDER_LOCK_WAIT_SECS`              | Seconds to wait for a builder lock (the shared Nix store image, the Stage 0 locks, the guest runtime build locks) when another `mvmctl` holds it; set to `0` to fail fast instead of queueing                                                                                                                                                                                                                                                                | `3600` (1 hour)                  |
