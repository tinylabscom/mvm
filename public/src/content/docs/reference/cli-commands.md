...
| `mvmctl prepare <IMAGE_OR_FLAKE> --pack-source <SOURCE> ...` | Install a local or HTTPS attested pack archive through the same quarantine/verification/cache promotion path as `cache install-pack`, then resolve whether it satisfies the requested input. Plain HTTP requires `--allow-http`; `--pack-mirror-base <BASE>` resolves relative pack and revocation sources under a local, HTTPS, or opt-in HTTP mirror base |
...
| `MVM_BOOTSTRAP_PACK_MIRROR_BASE` | Optional local path, HTTPS URL, or opt-in HTTP URL used to resolve relative bootstrap pack and revocation sources. | None |
...
the same schema from an HTTPS or local source. Enterprise mirror deployments can
set `--pack-mirror-base` or `MVM_BOOTSTRAP_PACK_MIRROR_BASE` and then pass
relative pack/revocation source names; `..` components are refused before any
download or file read.
