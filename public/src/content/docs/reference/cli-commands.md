...
| `MVM_BUILDER_BACKEND`                     | Builder VMM selection: `hvf`, `qemu`, or the optional development-only `libkrun` integration. Supported platform defaults are macOS 26+ Apple Silicon → `hvf` and native Linux → `qemu`; unsupported hosts fail availability checks rather than gaining a hidden standard libkrun dependency.                                                                                  | Platform default                 |
