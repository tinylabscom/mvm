...
:::note
On Linux with `/dev/kvm`, the dev-VM layer is skipped entirely -- Firecracker runs directly. On macOS Apple Silicon, microVMs run on HVF (macOS 26+) or libkrun (macOS 13–25). There is no Docker or container runtime path.
:::
