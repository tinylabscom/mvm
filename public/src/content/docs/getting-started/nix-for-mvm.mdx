...
The OS is always `linux` — even when building on macOS — because the VM image runs Linux inside the microVM, whichever standard backend boots it (Firecracker on Linux/KVM or native HVF on supported Apple Silicon macOS hosts). You're cross-compiling for the guest, not building for your host.
...
default. A writable disk image is accepted under every profile that allows
volumes, the default `standard` included, because the guest writes into its own
image file rather than a host directory. `:rw` on a host directory needs
