...
- Other backends must document their own restore contract before the SDK treats them as shipped cold-mode targets.

The SDK contract should hide backend mechanics but not hide security semantics. A cold sandbox still has sensitive state on disk. Restore should surface whether the snapshot matched the recorded audit evidence or seal, and retention policy must be explicit.
