...
- `mvmctl checkpoint create --class vm-full` / `mvmctl checkpoint restore` captures memory state and records content hashes in the audit chain (currently unsupported on macOS — Firecracker-only, on Linux — pending HVF save/restore).
