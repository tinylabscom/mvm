...
- Live mode runs the script with `MVM_SDK_MODE=live`; the SDK calls the host library `libmvm_hostlib` in-process to boot a real microVM and apply command/file operations. It never runs `mvmctl`.
