...
declares it fails to boot. Declare the reference in Workload IR, which an SDK
declaration compiles to (`mvm.toml` has no secret declaration), then run the
entrypoint with that IR as described in [Agent sandbox](/guides/agent-sandbox/).
