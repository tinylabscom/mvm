...
Store the secret together with its egress binding, the destinations it may be
sent to and how it authenticates:

```sh
mvmctl secret set openai-api-key --provider openai
mvmctl secret set my-api --host api.example.com --type bearer
```

Add `--approve ask` to hold each run's use of the secret until you approve it;
see [Runtime approvals](/guides/runtime-approvals/).

A secret stored with `mvmctl secret put` has no binding, and a workload that
declares it fails to boot. Bind the secret to a run with `--secret NAME`,
declare it for every run of a project in `mvm.toml`'s `[secrets]` table, or
declare the reference in Workload IR, which an SDK declaration compiles to, and
run the entrypoint with that IR — all three are described in
...
## Planned runtime SDK shape (not shipped)

:::caution[Not implemented]
The code below is a design target. The `Sandbox` handle has no `commands.run`
today. For a secret that reaches a running workload as a placeholder, use the
CLI path in [Agent sandbox](/guides/agent-sandbox/).
:::

The runtime SDK should preserve the same shape:
