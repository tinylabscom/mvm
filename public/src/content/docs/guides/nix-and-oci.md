...
- Expect a `--prod` run to get its own sealed rootfs. The sealed and dev builds of an image are cached separately, a `--prod` run never reuses the dev one, and a `--prod` run whose image is not sealed is refused. The trust policy is checked before anything is built or signed.
