...
Per [ADR-031](https://github.com/tinylabscom/mvm/blob/main/specs/adrs/031-libkrun-pivot.md), the dev/prod distinction is encoded in the entrypoint shape (`shell` → accessible, `command`/`services` → sealed). The same `mvm.lib.<system>.mkGuest` API serves both.
