...
The host never runs `nix` itself. The standard builder VM runs on native HVF on supported Apple Silicon macOS hosts and QEMU on native Linux. The optional libkrun integration is not installed, linked, or selected by the standard macOS path; the build contract is otherwise unchanged.
...
- The design for `mkFunctionWorkload`
