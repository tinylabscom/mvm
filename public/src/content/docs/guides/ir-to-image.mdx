...
`mvmctl emit entry.ts` runs the declaration under the same `MVM_EMITTING=1` and `MVM_IR_OUT=/path/to/workload-ir.json` contract. The TypeScript helpers emit canonical IR through the same schema-owned shape as Python.
