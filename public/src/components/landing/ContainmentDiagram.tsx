// The matryoshka containment model (see
...
//
// Nested rounded rects rather than bullseye circles, with a *mild* offset
// (more room top-left than bottom-right) so the stack reads as layered
// cards rather than a dartboard. The offset is kept small on purpose: an
// earlier hero-column version of this diagram used a much larger offset
// and left a dead void in the bottom-right of the outer rings once ported
// to a full-width section. Here the workload block spans most of the
// innermost ring's width instead of hugging one corner, so there is no
// empty quadrant to stare at.
...
const DESCRIPTION =
  ...
  "contains your workload — four nested boundaries. A single vsock channel " +
  "pierces all four rings from the outside straight through to the " +
  "workload; it is the only way in or out, and egress is deny-all until " +
  "policy admits it. The guest has no network interface of its own.";
...
/** Nested, mildly-inset rects: a touch more room is taken from the
 *  top-left on each step in than the bottom-right, so the stack drifts
 *  gently toward the bottom-right rather than sitting as a perfect
 *  bullseye — without the offset being large enough to leave a visible
 *  empty quadrant. */
...
function NestDiagram({
  ...
  // ring's own tab (so it never collides with the label) and spans nearly
  // the full width of the kernel ring rather than hugging one corner —
  // that's what keeps the ring from leaving an empty side once the
  // workload block is placed.
  const kernelLabelBlockH = tabInset + labelSize * 0.7 + subSize + 20;
  const workloadRect: RingRect = {
    x: kernelRect.x + workloadInsetX,
    y: kernelRect.y + kernelLabelBlockH,
    w: kernelRect.w - workloadInsetX * 2,
    h: workloadH,
