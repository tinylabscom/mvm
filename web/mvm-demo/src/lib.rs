...
use mvm_contract::policy::projection::{
    Proto, canonicalize_network_policy,
};
...
pub fn decide_egress(policy_json: &str, url: &str) -> String {
    let outcome = (|| -> Result<bool, String> {
        ...
        let ip = lookup_host(&host)
            .ok_or_else(|| format!("demo fixture has no IP for host: {host}"))?;
...
pub fn verify(chain_bytes: &str, pubkey_hex: &str) -> String {
    let outcome =
        verifying_key_from_hex(pubkey_hex).and_then(|key| verify_audit_chain_bytes(chain_bytes, &key));
    match outcome {
...
fn lookup_host(host: &str) -> Option<core::net::IpAddr> {
    ...
    match host {
        "api.openai.com" => Some(core::net::IpAddr::V4(core::net::Ipv4Addr::new(13, 65, 147, 86))),
        "api.github.com" => Some(core::net::IpAddr::V4(core::net::Ipv4Addr::new(140, 82, 121, 6))),
        _ => None,
    }
}
