...
Lima was the historical macOS host abstraction. It was removed on 2026-05-14 (Plan 72 W0–W6 + Plan 75 W0). libkrun is the default macOS 13-25 backend; vz (Apple Virtualization.framework via the per-VM Rust supervisor) is the macOS 26+ Apple Silicon default; Firecracker is the Linux KVM path. There is no `--lima` flag and no Lima fallback.
...
- **Vz** — Apple Virtualization.framework. Default on macOS 26+ Apple Silicon (the one AVF backend — the in-process Apple Container backend converged onto it, Plan 177/ADR-076). macOS-only.
...
- `mvm-backend` -- `VmBackend` trait + every backend impl + selection/dispatch; `base` (host substrate, was `mvm-base`), `libkrun`/`vz`/`firecracker`/`qemu` dispatch (8→4 backend consolidation, ADR-076; `codesign` carries the VZ/HVF entitlement signing).
...
mvm-backend: `base/` (shell, ui, linux_env, cow, runtime_meta, config — was mvm-base), `libkrun.rs`/`vz.rs`/`firecracker.rs`/`qemu.rs` (dispatch), `codesign.rs`, `artifacts/`
...
- **Dev mode**: `mvmctl dev` (or `mvmctl dev up`) auto-bootstraps then drops into a dev shell. On macOS 26+ Apple Silicon: boots the dev VM on the vz supervisor backend with Nix + build tools via PTY-over-vsock console. On other macOS: libkrun builder VM. On Linux with KVM: Firecracker directly. `mvmctl dev down` stops it. `mvmctl dev shell` opens a shell. `mvmctl dev status` shows environment info. It does NOT start or SSH into a Firecracker microVM.
