...
- `mvm-core` -- types, IDs, config, protocol, signing, routing, crypto. Absorbs `plan` (typed signed `ExecutionPlan`), `policy` (incl. `policy::security` session-policy), and `crypto` (the former `mvm-security`: attestation/keystore/secret_store/snapshot crypto + opt-in cosign behind `manifest-verify`). It is *mostly* runtime-free, but **not fully** — it carries `tokio` (`io-util` base; `rt`/`net`/… feature-gated) for `core::framing`'s async length-prefixed transport, and has since the first workspace import. Making it genuinely runtime-free (move `core::framing` to `mvm-hostd`, where its only callers live, + a CI gate) is a tracked plan-121 deferred follow-up.
