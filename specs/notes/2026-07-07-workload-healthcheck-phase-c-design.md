...
Persisted to the already-provisioned, currently-unused name-registry fields (`crates/mvm/src/vm/name_registry.rs`): `VmRegistration.readiness: Option<InstanceReadiness>` + `last_readiness_change_at`, via the existing atomic `set_readiness`. Mapping onto `InstanceReadiness` (`crates/mvm-core/src/domain/instance.rs`): `Starting → ServicesStarting`, `Healthy → ServicesReady`, `Unhealthy → Degraded`. `machine ls` gains a health column and `machine inspect` a health field (net-new rendering; storage + enum already exist). When the daemon is disabled/absent, health reads `unknown` and the machine still runs — clean degradation.
