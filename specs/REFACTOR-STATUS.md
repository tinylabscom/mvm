...
- [ ] **PLAN 125** — CLI surface + SDK DX veneer · 🟢 **Phases B + C complete** + D1 TS `exec` parity, in **both** SDKs (imperative `Sandbox` whole; `CodeSandbox` + `BrowserSandbox` presets); Phase E underway — E2 `--secret NAME:host` + E1 Python⇔TS decorator coherence + E3 doctor backend-capability matrix landed (the "one IR" mirror guarantee; `mvm.toml`/flake aren't Workload surfaces — reframed); E4 profiles, E5 host-services SDK remain
...
- [ ] **PLAN 193** — rvproxy network substrate (replace gvproxy/passt) · 🟡 WS-1 PROVEN (libkrun-unixgram live `dev up` through rvproxy, ~540k connections relayed, builder rootfs built — rvproxy #38/#42/#53) + WS-1.5 parity-gate scaffold `scripts/rvproxy-gateway-parity.sh` (gvproxy↔rvproxy conformance, refuses non-conforming binary) + drafted CI lane `.github/workflows/rvproxy-parity.yml` (manual macos-latest, pinned-rev candidate via `RVPROXY_CHECKOUT_TOKEN`, fail-closed); WS-2 native-flow port (the biggest win — replaces the in-line claim-10 datapath wrapper, Plan 141) + WS-3/4 + CI-lane activation (provision secret + PR trigger) remain, cross-repo
