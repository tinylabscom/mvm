...
- [~] Performance and smoke coverage: add builder-VM/KVM `machine run --image alpine -- true`, `machine run --net --image alpine -- nslookup example.com`, phase timing, cached hot-start benchmark, and macOS backend measurements before making latency claims. **macOS Vz phase-timing captured** (`specs/perf/plan-200/vz-machine-run.md`, debug binary so upper-bound): `machine run --image alpine -- true` works end-to-end on Vz; warm steady state ~2.46–2.49 s total (N=3), dominated by `backend_start` ~2.25–2.30 s (Vz create + alpine-rootfs boot), `teardown` ~1–2 ms (B2 eager-kill), `vsock_wait` low; cold first run ~13.9 s (one-time supervisor rebuild + first OCI pull + ext4 materialize). **Finding: the `--net` smoke FAILS on Vz** — `machine run --net --image alpine -- nslookup example.com` exits before boot with "policy bundle for local:exec-… not found", an open egress-policy-bundle gap on the transient `machine run --net` path (related to in-flight `fix/plan-200-up-egress-enforcement`), not a measurement artifact. Remaining: release-build numbers, the `--net` smoke once the egress-bundle gap is fixed, the Linux KVM / builder-VM (Firecracker) lane, and a committed cached-hot-start benchmark harness + `<200 ms` dispatch-bar assertion (`backend_start` is the lever before any latency claim).
