...
**Additional 2026-06-21 — Plan 204 WS-D typed `BuildGuestImage` dev_build route landed (opt-in, default-off).** The first guest-image-build `BuilderdClient` consumer is wired: `dev_build_via_builder_vm_uncached` now tries `try_typed_persistent_build` before the legacy persistent dispatch when `MVM_BUILDERD_TYPED` is set and a daemon is reachable, reading the daemon's host-readable `/out` export (#1213) into the dev build cache and falling back to legacy on `Fellback`/any error (#1215). The override-input artifact-equality gap is closed by **scoping, not a wire change**: `typed_route_blocked_by_local_override` keeps source-checkout flakes (where the legacy path injects `--override-input mvm path:/work/nix`) on the legacy dispatch, so the typed route only fires for genuinely override-free builds (2 unit tests). This corrects the prior rollup/PR claim that the legacy persistent path "ignores the source-checkout staging" — it does not. **Remaining:** a builder-image rebake (the running daemon predates the `output_dir` field), a live override-free artifact-equality proof (shell vs typed, identical `vmlinux`+`rootfs.ext4`) per backend, then flip `resolve_route`'s build default, then the raw-shell debug gate (gated on the flip).
...
PLAN 204 — Builder VM resident control plane   🟢 IN PROGRESS (ADR-089)
  ...
  [~] Move flake check, guest image build, and host-tool build to typed requests.
      ...
      adapter landed (#1211 `run_build`/`try_typed_build`); both `BuildGuestImage`
      wire gaps closed — host-readable `output_dir` export to the `/out` share
      (#1213) and `dev_build_via_builder_vm_uncached` wiring (#1215, opt-in). The
      override-input gap is closed by scoping rather than a wire change: the typed
      route bails to legacy for source-checkout flakes where the legacy path would
      `--override-input mvm path:/work/nix` (`typed_route_blocked_by_local_override`),
      so it fires only for override-free builds. Remaining: a builder-image rebake +
      a live override-free artifact-equality proof (shell vs typed) per backend,
      then flip `resolve_route`'s build default.)
