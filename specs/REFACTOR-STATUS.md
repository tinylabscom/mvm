...
**Additional 2026-06-21 — Plan 209 / ADR-093 slim microVM kernel: Gate 0 PASS + groundwork landing.** One slim kernel we own, two backends. Tasks 1/2 (promote `nix/images/builder-vm/kernel/` → shared `nix/images/kernel/`, name the dm-verity workload delta), 7 (`mvm-core::kernel_artifact::KernelArtifactId`), 10a (`verify_fetched_kernel`), and the `path:`-flake import fix are in PR #1238 (Gate-0-validated, auto-merging). **Gate 0 PASS, live on macOS-26/libkrun:** built the slim workload `vmlinux` inside the libkrun builder VM (20.2 MiB), built the full builder image, booted a workload under `--hypervisor libkrun`, guest agent answered over vsock (`vm wait`→0; boot-report control-plane ready). Gate 0 caught a regression the host-side checks missed: the promotion imported via `import ../kernel/X.nix`, which works as a `git+file` flake (so `nix flake check` + `.drv`-hash diffs passed) but escapes the store copy under the `path:` flake the libkrun builder VM fetches — fixed by routing through `workspaceRoot` like `nix/lib`. Continuation branch `feat/plan-209-kernel-flake` (Tasks 8 standalone kernel flake, 6 `xtask check-kernel-config-budget`, 9 CI publish-manifest + stale-trigger fix, 11-core `resolve_kernel`) pending rebase onto main after #1238 merges. **Remaining:** Task 5 audit-subtract shrink (iterative: rebuild + boot-smoke per removal), Task 11 boot-path threading + `vm rekernel` verb + live re-boot, CLAUDE.md kernel-claim note. Deferred follow-ups (own plans): kernel-CVE detection watcher; mvmd fleet rollout.
...
PLAN 204 — Builder VM resident control plane   🟢 IN PROGRESS (ADR-089)
  ...
  [~] Move flake check, guest image build, and host-tool build to typed requests.
      ...
      Deferred to a quiet box: the literal over-the-wire artifact-equality proof,
      then the build-specific `resolve_route` default-flip, then the allowlist drop.)
