...
- [ ] **PLAN 213** — Attested fast-first-boot packs · 🟢 in progress ([ADR-097](adrs/097-attested-downloadable-runtime-and-builder-packs.md)) — Workstream A pack schema/verifier core is implemented; Workstream C local content-addressed pack cache, status, prune, archive promotion, and `cache install-pack` local/HTTPS download path are implemented. Remaining: release publishing, prepare UX resolution, runtime/builder fast paths, OCI/flake preparation, launch attestation/explainability, revocation/mirror policy, metrics, and broader docs/migration workstreams.
