...
- [ ] **PLAN 213** — Attested fast-first-boot packs · 🟢 in progress ([ADR-097](adrs/097-attested-downloadable-runtime-and-builder-packs.md)) — Workstream A pack schema/verifier core is implemented; Workstream C local content-addressed pack cache, status, prune, archive promotion, `cache install-pack` local/HTTPS download path, and prepare download resolution are implemented; Workstream D has the first cache-backed `mvm prepare`/`--dry-run` resolver slice; Workstream G has explicit prepare-time OCI tag-to-digest resolution; Workstream H has launch-attestation record/log core plus launch-boundary emission. Remaining: release publishing, default install-time preparation, runtime/builder fast paths, flake lock resolution, builder-VM artifact preparation, setup-cache keying, `mvm explain`, revocation/mirror policy, metrics, and broader docs/migration workstreams.
...
PLAN 213 — Attested fast-first-boot packs   🟢 IN PROGRESS (ADR-097)
  ...
  [ ] Workstream H — Launch attestation and explainability.
