...
**Additional 2026-06-21 — Plan 204 WS-D: first production BuilderdClient consumer (host-side dispatch adapter).** Beyond the WS-D lint (`check-builder-shell-job-sites`, #1199), the routing itself starts: `mvm_build::builderd_dispatch` is the host-side adapter — `resolve_ready_builder_socket` (scan the builder-VM state root, return the first daemon that answers a handshake), the pure `classify_flake_check_outcome` verdict mapper, and `flake_check_via_builderd` (a thin `BuilderdClient::run_operation` wrapper). `mvmctl build validate` now routes a flake check through the resident daemon as a typed `FlakeCheck` behind `MVM_BUILDERD_DISPATCH=1`, falling back to the legacy shell path. Shipped with a daemon-side correctness fix: `builderd::flake_check_argv` no longer `path:`-wraps a scheme-qualified ref (`github:`/`path:`/`git+…`) — the prior unconditional wrap made `path:github:…` (a non-existent path). Routing is scoped to *remote* refs: the builder VM does not mount the host work tree, so a local-path flake is unreachable inside it (routing one would falsely report it invalid); local-flake/job staging is the next WS-D piece, shared with the build-op routing. Live-proven over the wire on macOS-26 Vz that a routed check reaches the daemon, which runs `nix flake check` and returns a typed terminal the adapter maps to the verb output; 9 new unit tests (adapter classify/socket-resolution + `flake_check_argv` scheme handling), fmt/clippy/WS-D-lint clean. Remaining WS-D: emit the compat-adapter diagnostic, route the build ops (output-model reconciliation `/out`↔store-path), local-flake staging, raw-shell debug gate, and the WS-C no-host-Nix test.
