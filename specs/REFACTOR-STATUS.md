...
**Latest update: 2026-06-19** Plan 200 C2 Rust SDK/CLI parity proof started: Rust SDK `MachineRun` argv now round-trips through the real CLI `machine run` parser into `mvmctl run` dry-run/preflight and receipt summaries, proving default-deny and `--allow-host` network posture stay CLI-owned; Rust SDK `MachineCreate --manifest` argv reaches the CLI manifest parser and proves strict unknown-key rejection remains CLI-owned. Remaining C2 proof: full admission-input and audit-summary parity, artifact-verification non-bypass, Python/TS parser/preflight parity, and live admission proof. Plan 118 PR-10c's first pure-substrate slice landed: density and concurrent-launch report shapes plus VM-free footprint/distribution helpers and tests are in place; live density CLI, platform footprint accessors, concurrency orchestration, baselines, and Vz/FC lanes remain open. Plan 159 is closed: VZ/macOS scope is shipped, verb/install/product polish is rehomed to Plan 181 / Plan 200, and signed delta-image distribution is descoped until an owning artifact/distribution plan is opened.
...
**Additional 2026-06-19 — Plan 200 C2 Rust SDK/CLI parity proof:** Rust SDK machine builder argv now round-trips through the CLI machine parser and `mvmctl run` dry-run/preflight/receipt helpers without invoking Nix or booting a VM. Coverage proves SDK default-deny and `--allow-host` network posture matches the CLI receipt path, and SDK `MachineCreate --manifest` reaches the CLI strict-manifest unknown-key gate. Remaining: full admission-input equivalence, artifact-verification non-bypass, audit-summary parity, Python/TS parser/preflight proof, and live admission proof.
...
- [ ] **PLAN 118** — Supervisor standby pool · 🟡 **vz + libkrun DONE; PR-10c substrate started** (saved-standby warm pool live-validated + self-replenish #840 + overshoot flock closed; density/concurrent-launch report schemas and pure summary helpers landed). Open boxes: the **non-vz** FC standby pool, live density CLI, platform footprint accessors, concurrency orchestration, baselines, and Vz/FC lanes.
...
PLAN 202 — Host services daemon (per-tenant, not per-VM spawn)   🟡 IN PROGRESS (ADR-084, #977)
  ...
  [ ] Phase 5 — mvmd daemon-lifecycle + Firecracker substrate slices landed
      ...
      and guest→host `BROKER_PORT` on `runtime/v.sock_5300`), and the first delegated
      `host.cost.v1::tenant` slice now rides typed ALPN messages through `mvmd-agent`
      with cross-tenant refusal. The broader cross-VM endpoint/authz surface and the
      final density/key-boundary proof remain open.
...
- Plan 200 is actively landing in slices. The persistent spec + running-VM wrapper slice landed in #1048; persistent image-backed `machine start`, manifest-to-machine runtime mapping, policy surfacing, dev-tier SSH-agent forwarding, Python/TypeScript/Rust SDK machine wrappers, and Rust SDK -> CLI parser/preflight parity proof for default-deny/allow-host/unknown-key are now landed or in the active PR lane. The next open slices are the remaining SDK/CLI admission/artifact/audit parity proof, Python/TS parser/preflight proof, scenario/limitations docs, portable artifacts, perf/smoke coverage, and dependency/binary budgets.
- Plan 202 Phase 2 signer-helper daemonization, all of Phase 3 (including 3d's host-gated Vz live/default re-verify), Phase 4 registration/restart semantics, Phase 6 local fork retirement, and mvmd Phase 5's daemon-lifecycle + Firecracker substrate slices are complete in the active worktrees. The first delegated `host.cost.v1::tenant` cross-VM proxy/authz slice is landed too. Remaining product work is the broader cross-VM endpoint/authz surface plus the density/key-boundary proof; ADR-084 acceptance is still admin/review closeout.
- mvmd PR `#160` is the CI-plumbing dependency for the next cross-repo slice: it pins sibling checkout to `tinylabscom/mvm`, installs `zigbuild` before sibling builds, and skips embedded-binary PR gates in the PR lane. Let that merge before resuming the delegated host-services follow-on in mvmd.
- The next executable Plan 202 slice is now explicit: in the sibling `mvmd` worktree, finish the reusable delegated host-services substrate first (dedicated host-services route layer + centralized tenant/workload authz for delegated calls), then land the next low-risk delegated read verb (`host.catalog.v1`), then expand the hostile-tenant proof set across the delegated surface, and only after that collect fleet-shaped density evidence plus the per-tenant key-boundary proof.
...
- `mvmd#160` / `fix/ci-mvm-sibling-ref`: CI sibling-checkout + `zigbuild` plumbing for the cross-repo Plan 202 mvmd slice.
...
1. Continue Plan 200 with the remaining C2 SDK parity proof: SDK/CLI admission-input, receipt/audit-summary, artifact-verification, default-deny network, unknown-key, and source-conflict non-bypass tests. Python/TypeScript/Rust lifecycle wrappers are landed.
2. Merge `mvmd#160` once CI is green so the cross-repo Plan 202 baseline is stable.
3. Resume Plan 202 Phase 5 in the sibling `mvmd` worktree in this order: (a) finish the reusable delegated host-services substrate (`W1`/`W2`: dedicated host-services route layer + centralized tenant/workload authz), (b) land `host.catalog.v1` as the next delegated read verb, (c) extend the hostile-tenant / malformed-response proof set across the delegated surface, then (d) capture fleet-shaped `O(active tenants)` density evidence plus the per-tenant key-boundary proof.
4. ADR-084 acceptance remains the final admin/review closeout once the product evidence above is landed.
5. Clean stale worktrees: several are old/behind or already landed (`mvm-202-3c-doctor`, `mvm-pr1009`, `mvm-p200-ociboot`, old status/170/vz100 branches). Do not sequence new work from them.
6. Confirm Plan 125 remains closed/rehome-only; remaining per-tenant daemon work belongs to Plan 202.
7. Do Plan 199 in parallel if builder VM time is available: native `libkrunfw`/`libkrun` Nix recipes and flake/build verification.
8. Do Plan 195 before Plan 189 acceptance: fingerprint narrowing reduces builder churn and supports cached fast-boot validation.
9. Continue Plan 193 only after rvproxy cross-repo slices are ready: delete splice, remove Plan-141 hooks, default-on bridge, transparent terminator.
10. Then Plan 189 VZ DX parity: save/restore verbs, JSON coverage, base pinning.
11. Resolve Plan 126's remaining blocked deps (`oci-client`, `aws-lc-rs`, `reqwest` major unification) rather than treating rehomed `sigstore` or superseded `pgp` work as normal TODOs.
12. Plan 118/175 are live-KVM gated; do density bench substrate first, then FC standby/warm-start once the KVM environment is ready.
13. Plan 159 and 201 are lowest priority: Plan 159 mostly needs residuals rehomed/descope, and Plan 201 is a convenience layer after warm-pool fundamentals.
...
- [ ] `mvm.toml` machine mapping: parser/model slice, runtime pass, policy surfacing, and dev-tier SSH-agent transport are landed — `machine create --manifest <path>` now persists `net`, `[network].allow_hosts`, `cpus`, `mem`, `mem_initial`, `[dev].init`, `[dev].volumes`, and `[auth].ssh_agent` into `MachineSpec`; `machine start --name` now applies `mem_initial`, admitted volume shares, dev-init execution, and `SSH_AUTH_SOCK` Unix-socket forwarding to `/run/mvm/ssh-agent.sock`; and `machine start --dry-run` / `--receipt` now surface the effective policy. ADR-088 fixes the design boundary: mutable dev-machine state is never an implicit prod input, SSH-agent forwarding is dev-tier only, and Nix templates cannot add SSH packages/config/material. Remaining is signed-plan auth/admission metadata refinement, sealed/prod policy follow-up, live SSH-agent round-trip validation, and non-standard-port SSH protocol-denial proof.
- [ ] SDK parity: Python, TypeScript, and Rust machine wrappers now reuse the CLI path by shelling only to `mvmctl machine ...`, expose structured `MachineError`s, and have fake-CLI lifecycle/error tests. Remaining: SDK/CLI admission inputs, artifact verification, default-deny network, unknown keys, source conflicts, and receipt/audit summary non-bypass/parity tests.
