...
- [ ] **PLAN 189** — VZ DX parity (post-convergence) · 🟡 in progress — WS-3 `dev status/down/up --json` plus checkpoint/snapshot JSON landed; WS-1 `vm save`/`vm restore` aliases landed (#1118 — over the `vm-full` checkpoint, Vz `save-restore`-tier-gated, live macOS save→stop→restore acceptance open); remaining: cached fast-boot acceptance, linux-native richer JSON detail, base pinning (spun out of 177; sibling of 159)
