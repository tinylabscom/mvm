...
PLAN 204 — Builder VM resident control plane   🟢 IN PROGRESS (ADR-089)
  ...
  [~] Move flake check, guest image build, and host-tool build to typed requests.
      ...
      Build-default flip + raw-shell gate implemented (branch
      `feat/plan-204-wsd-flip-build-default`, merge held): `try_typed_build` reads
      ...
      (3 new unit tests). Allowlist stays at 4 — the `persistent_builder.rs` legacy
      ...
      `MVM_BUILDERD_RAW_SHELL=1`). Merge gate still open: the literal over-the-wire
      artifact-equality proof — a live run hit the documented cascade
      (stale-builder `/work`-not-a-flake, missing `mvm-vz-supervisor`, stale socket,
      cold-cache Stage 0 `BadActivate`), none of it WS-D code; the flip stays on the
      branch until a clean per-backend equality proof is green. A clean retry
  ...
  [~] Retire normal-path raw shell jobs behind a gated debug-only adapter.
      (Implemented as the `MVM_BUILDERD_RAW_SHELL` build-route opt-out on branch
      `feat/plan-204-wsd-flip-build-default`; merge held with the flip above.)
