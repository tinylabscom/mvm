...
PLAN 204 — Builder VM resident control plane   🟢 IN PROGRESS (ADR-089)
  ...
  [~] Move flake check, guest image build, and host-tool build to typed requests.
      ...
      adapter in-flight (#1211 `run_build`/`try_typed_build`); `dev_build` wiring
      blocked on two `BuildGuestImage` wire gaps — no override-input field (dev-agent
      override builds not artifact-equal) and no host-readable output dir (returns the
      in-VM `/nix/store` path; `dev_build_via_builder_vm` needs the `/out` share).
      Both need a wire change before routing; build-image/host-tool daemon ops remain
      boot-gated.)
