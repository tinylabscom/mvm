...
- [ ] **PLAN 213** — Attested fast-first-boot packs · 🟢 in progress ([ADR-097](adrs/097-attested-downloadable-runtime-and-builder-packs.md)) — Workstream A pack schema/verifier core is implemented; Workstream B signed runtime/builder release pack publishing, fail-closed release asset verification, and pack rebuild-and-compare reproducibility are implemented; Workstream C local content-addressed pack cache, status, prune, archive promotion, `cache install-pack` local/HTTPS download path, and prepare download resolution are implemented; Workstream D has the cache-backed `mvm prepare`/`--dry-run` resolver slice, machine-run dry-run preparation reason diagnostics, and explicit bootstrap runtime/builder pack preloading from operator-provided sources/policy; Workstream G has explicit prepare-time OCI tag-to-digest, flake-lock identity, and setup-cache identity/resolution slices; Workstream H has launch-attestation record/log core, launch-boundary emission, and `mvm explain`; Workstream I has explicit policy modes, channel signing-key pins, key-rotation windows, mirror identity checks, offline revocation freshness refusal, and local/HTTPS revocation metadata fetching. Remaining: runtime/builder fast paths, builder-VM artifact preparation, actual setup-cache artifact production/consumption, enterprise mirror source resolution, metrics, and broader docs/migration workstreams.
...
PLAN 213 — Attested fast-first-boot packs   🟢 IN PROGRESS (ADR-097)
  ...
  [~] Workstream D — Install and prepare UX.
      ...
      metadata. Remaining: automatic channel/mirror policy, full launch-path
      pack consumption, policy-refusal CLI integration coverage, and
      snapshot/warm derivation.)
  ...
  [~] Workstream I — Revocation, mirrors, and offline policy.
      ...
      fetching; and prepare/cache/bootstrap CLI policy inputs. Remaining:
      enterprise mirror source resolution and full revocation/mirror docs.)
