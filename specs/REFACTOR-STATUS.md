...
- [ ] **PLAN 193** — rvproxy network substrate (replace gvproxy/passt) · 🟡 in progress — WS-1 transport proven; WS-1.5 parity scaffold + CI lane live; rvproxy R2 shipped; mvm-side native config emission/launch, native flow-audit refeed, and binary-discriminating native enforcement witnesses are landed. Current cleanup removed the dead `BridgeConfig.policy` / production `AllowAll` open-policy slot so the bridge has one live source of enforcement (`bundle` or threaded `NetworkPolicy`, otherwise fail-closed deny-all), macOS/libkrun now selects native by default when `MVM_GATEWAY_BIN` names the rvproxy candidate while preserving gvproxy/no-candidate fallbacks, and admitted libkrun/Vz workload starts already thread the signed plan by default. Remaining: add the transparent terminator requirement, make the parity gate required, then delete the splice + Plan-141 `on_packet` hooks.
...
PLAN 193 — rvproxy network substrate (replace gvproxy/passt)  🟡 WS-1 proven; WS-2.2d mvm-side native parity landed; terminator/parity-required/splice deletion remain
  ...
  WS-2.2d (mvm-side, #1009): native rvproxy audit feed + native-enforcement parity, no splice.
    ...
    - remaining: add the transparent terminator requirement, make the parity gate required, then
      delete the splice + Plan-141 on_packet hooks.
...
4. Continue Plan 193 only after rvproxy cross-repo slices are ready: add the transparent terminator requirement, make the parity gate required, then delete the splice and remove Plan-141 hooks.
