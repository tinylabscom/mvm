...
**Additional 2026-06-17 — Plan 202 Phase 2a landed:** `mvm_hostd::audit_signer::helper::TenantSignerHelper` introduces the resident per-tenant signer-helper core. One key-holding helper owns a `vm_id → Chain` map, opens/owns each VM's workload JSONL plus secondary head at registration, drops the slot at deregistration, and refuses tenant/workload identity mismatches before signing. Phase 2b-2d still move host-agent forwarding, admission signing, and restart recovery onto that helper.
...
PLAN 202 — Host services daemon (per-tenant, not per-VM spawn)   🟡 IN PROGRESS (ADR-084, #977)
  ...
  [ ] Phase 2 — audit-signer daemon (2a helper core landed; 2b forwarding,
