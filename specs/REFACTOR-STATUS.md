...
- [ ] **PLAN 204** — Builder VM resident control plane · 🔴 proposed ([ADR-089](adrs/089-builder-vm-resident-control-plane.md), draft PR #1082) — keep `mvmctl` as the single host UX, keep host Nix optional, and move builder execution behind a resident internal `mvm-builderd` service inside the builder VM over typed vsock requests. Stable API = allowlisted build/eval operations with structured progress/provenance; no user-facing builder shell; guest images do not include host/builder tools.
...
PLAN 204 — Builder VM resident control plane   🔴 PROPOSED (ADR-089)
  Plan: specs/plans/204-builder-vm-resident-control-plane.md
  [ ] Add builder request/response protocol types with version/refusal tests.
  [ ] Add resident `mvm-builderd` in the builder VM with Handshake/Probe.
  [ ] Add host-side `mvmctl` builder client over vsock.
  ...
  [ ] Add structural tests proving workload guest images do not include `mvmctl`
      or `mvm-builderd`.
