...
**Additional 2026-07-09 — Plan 236 execution started by refreshing the Phase 2A port-handler slice on branch `feat/vsock-port-handler-registry`.** The synced `scripts/check-plan-236-go.sh` result is still `NO-GO`: `fix/agent-verb-grant-delivery`, `feat/plan-202-native-host-services`, `worktree-vsock-only-egress-cutover`, and `feat/plan-216-s0-mvm-client` are dirty and/or behind `main`. Rather than open a duplicate lane, execution now starts from the cleanest prerequisite branch already carrying real no-guest-NIC work: PR `#1599` / `feat/vsock-port-handler-registry`. That branch is rebased onto current `main`, its invoke-path env tests now use the shared `mvm_core::util::test_env::TestEnv` guard so parallel workspace runs stay stable, and the refreshed host validation is green again (`cargo check --workspace`, `cargo test --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`).
...
- [ ] **PLAN 219** — Agent verb grant delivery · 🟡 unit-testable delivery landed; live Vz/Firecracker proof remains — core token encode/decode, sidecar mint, backend cmdline token, config-drive host-signer pubkey delivery, guest pinning against config-drive key, and chain-signed denial audit are implemented and targeted-test green. Remaining = live boot proof that `/run/mvm/{host-signer.pub,verb-grant.json}` arrive before agent fork, grant-listed verbs succeed, unlisted ProdSafe verbs return `VerbNotAuthorized`, and `verb_denied` verifies on the real chain.
...
PLAN 219 — Agent verb grant delivery            🟡 UNIT DELIVERY LANDED; LIVE BOOT PROOF REMAINS
  ...
  [ ] Live validation — Vz/macOS and Firecracker/Linux boot proof that files arrive before agent fork, grant-listed verbs succeed, unlisted ProdSafe verbs refuse, `verb_denied` lands and verifies, and `agent_verbs = None` remains byte-identical/no-token.
...
PLAN 238 — Streamed ext4 materialization and OCI/ext4 oracles   ✅ DONE
  Last updated: 2026-07-09
  [x] Add a sparse streamed block-emission API to `mvm-ext4` with dense-vs-streamed byte identity tests.
  [x] Stream the in-process OCI rootfs materialization path directly to the output file instead of building a dense image in memory.
  [x] Add Linux-tool OCI/ext4 differential oracles (`e2fsck`, `debugfs`) and dense-vs-streamed boundary tests in `mvm-build`.
      Tests are in tree, host-side OCI differential coverage is green, and the Linux-only oracle execution passed on a Debian 6.1 host.
