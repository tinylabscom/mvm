...
**Latest update: 2026-06-18** Plan 202 Phase 5 mvmd adoption has started in the companion mvmd Plan 52 workstream while Plan 202 remains open, and Plan 202 vz live verification is complete on top of Phase 4b supervised daemon restart. mvmd now starts/reuses the shared `mvm-host-agent` + `mvm-signer-helper` unit per tenant from instance lifecycle events, registers/deregisters VMs against the resident tenant daemon, binds Firecracker guest-to-host broker traffic on `runtime/v.sock_5300`, and hardens the lifecycle seam so tenant/pool/instance IDs are path-safe validated before deriving daemon registration paths or replaying `host-agent.tenant` marker state. Remaining Phase 5 work is the broader tenant-key/boundary/density proof plus the rest of the cross-VM endpoint/authz surface; Phase 6 still owns retiring `spawn_broker_services_if_admitted`. A direct-boot `audit-probe` VM on vz (`tenant=p202vz`, `vm=p202vzprobe`) ran with no `MVM_GATEWAY_BRIDGE` and no daemon env override, emitted 22 workload entries through `host.audit.v1`, verified clean with `mvmctl trust audit verify --tenant p202vz` (4 lifecycle entries + 22 workload entries), and deregistration left the per-tenant registration journal empty. The flake builder path was blocked by a builder-VM `BadActivate` before workload boot, so the vz runtime proof used a previously built `audit-probe` rootfs copied into isolated state plus `MVM_DIRECT_BOOT=1`. Plan 126 / Plan 200 bookkeeping drift was reconciled. Plan 126's default-closure gate and final measurement are landed: `sigstore`, `opendal`, and `pgp` are out of the `mvmctl` default closure, `opendal` is replaced by `object_store`, D1's forbidden-dep closure ban is wired, and the final measure is recorded in `docs/investigations/dep-baseline.md`; remaining Plan 126 scope is the still-blocked OCI/TLS stack decision (`oci-client` fork/replace/upstream path, `aws-lc-rs` removal, and `reqwest`/`oci-client` major unification) plus documented follow-up sweeps. Plan 200's already-landed machine lifecycle verbs and local image-source tests are status-synced; remaining Plan 200 scope is full manifest-to-durable-machine runtime mapping, SDK parity, scenario/limitations docs, portable artifacts, performance/smoke coverage, and duplicate-major/binary-size budgets. Plan 203 was added as a proposed opt-in forensic network transcript capture follow-on to claim 10. The default claim-10 posture stays metadata-only; raw transcript capture is a separate, explicitly armed/exported forensic mode with its own manifest and encrypted payload store. Plan 200 persistent machine slice also advanced: named machine specs now persist under `<MVM_DATA_DIR>/machines/<name>/machine.json` through `mvm_core::config` helpers with atomic writes, strict JSON, safe-name validation, and `machine create` / `ls` / `inspect` / `rm --yes`; running-VM wrappers `machine exec` / `shell` / `stop` reuse the existing console/down paths and require a persisted spec first; `machine start --name` now boots those specs through the admitted OCI-backed launch path; and the `mvm.toml` parser now accepts the machine-oriented fields (`cpus` with legacy `vcpus` alias, `net`, `[network].allow_hosts`, `[auth].ssh_agent`, `[dev].init`, `[dev].volumes`) with strict validation plus a typed machine-workflow projection. Full manifest-to-durable-machine runtime mapping remains open. Plan 125 is closed/rehome-only: its SDK/CLI veneer work is complete, and the remaining host-services daemon/process-model work now lives in Plan 202.
...
PLAN 202 — Host services daemon (per-tenant, not per-VM spawn)   🟡 IN PROGRESS (ADR-084, #977)
  ...
  [x] Phase 3 — decouple availability from MVM_GATEWAY_BRIDGE (3a default-on + 3b
      `O(active tenants)` cost framing + 3c doctor daemon-state reporting landed; libkrun and vz
      live verification complete).
  [x] Phase 4 — supervision + crash/restart journal (4a registration journal + 4b supervised restart/crash semantics landed).
  [~] Phase 5 — mvmd host agent owns the daemon per tenant (mvmd Plan 52). First slice landed:
      per-tenant daemon reuse, lifecycle register/deregister, Firecracker `runtime/v.sock_5300`
      broker binding, marker replay, and path-safe tenant/pool/instance validation. Remaining:
      per-tenant key-boundary proof, cross-tenant reach/forge refusal tests, broader endpoint/authz
      surface, and fleet-shaped density check.
...
- Plan 202 vz live verification completed after #1054.
- The first Plan 202 Phase 5 mvmd adoption slice has now started in mvmd Plan 52.
- Plan 200 is actively landing in slices. The persistent spec + running-VM wrapper slice, persistent image-backed `machine start`, admitted persistent boot helper, and manifest parser are now landed; the remaining machine-workflow mapping is the next open slice.
...
2. Continue Plan 202 next: complete the remaining mvmd Phase 5 tenant-boundary/density proof, then retire `spawn_broker_services_if_admitted`.
3. Clean stale worktrees: several are old/behind or already landed (`mvm-202-3c-doctor`, `mvm-pr1009`, `mvm-p200-ociboot`, old status/170/vz100 branches). Do not sequence new work from them.
4. Confirm Plan 125 remains closed/rehome-only; remaining per-tenant daemon work belongs to Plan 202.
...
9. Resolve Plan 126's remaining blocked deps (`oci-client`, `aws-lc-rs`, `reqwest` major unification) rather than treating rehomed `sigstore` or superseded `pgp` work as normal TODOs.
...
- [ ] `mvm.toml` machine mapping: parser/model slice landed for `cpus` with legacy `vcpus`, `net`, `[network].allow_hosts`, `[auth].ssh_agent`, `[dev].init`, `[dev].volumes`, and typed machine-workflow projection; remaining is threading those fields into durable machine specs / admitted launches, preserving read-only volume defaults, rejecting dev init for sealed/prod, and surfacing effective policy in dry-run/receipt output.
