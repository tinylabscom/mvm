...
**Additional 2026-07-11 — Plan 236 OCI prod closeout now has a real live-smoke harness.** The same `feat/plan-236-oci-seal-closeout` branch now carries the missing disabled-by-default macOS witness for `mvmctl run --image --prod`: `tests/oci_image_runner_smoke.rs` requires a signed digest-pinned registry ref plus `cosign` + admitted OCI policy, then asserts a successful boot left `rootfs.verity` + `rootfs.roothash` sidecars in the OCI cache. Focused validation is green with `cargo test --test oci_image_runner_smoke -- --nocapture` and `cargo clippy --test oci_image_runner_smoke -- -D warnings`. The blocker is now purely environment/evidence: this host still lacks `cosign` and `~/.mvm/oci-policy.toml` (or `MVM_OCI_POLICY`), so the live witness has not yet been executed.
