...
PLAN 200 — Machine UX/DX layer  🟢 in progress — `machine run` shipped
  ...
  [x] Implement transient network policy plumbing for image-backed runs. → WS-B shipped
      (#1003): `--net`/`--allow-host` on run/machine-run, one `NetworkPolicy` on
      `VmStartConfig`, uniform FC/libkrun/Vz enforcement via admitted-plan gateway bridge,
      audit/dry-run/receipt, live gateway-bridge proofs.
  [~] WS-B deferred follow-ups (branch `feat/plan-200-deferred`): **DONE** — OCI cache-index
      `schema_version 0` bug fixed; `plan.launched`/`plan.failed` emitted on the transient-run
      path (`LaunchAudit` hook); MCP cold code-run routed through the shared `make_run_admit`
      so its deny-all is bridge-enforced; vestigial `BridgeConfig.policy` field + `AllowAll`
      type removed (`AllowAll` now `#[cfg(test)]`). **REMAINING (need live Vz/builder
      verification):** uniform `host:port` L4 on the libkrun/Vz bare path (admission-time DNS
      pin → `L4PolicyScan`); macOS transient-run guest `eth0`/DHCP bring-up (policy-gated);
      `run --image <oci>` `mvm-meta.json` sidecar on the OCI materialize path; DHCP/ARP
      carve-out under deny-all (decision recorded: loopback-only under deny-all).
