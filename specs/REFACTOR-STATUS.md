...
**Latest update: 2026-06-20** Plan 126's last open item is resolved by decision: `aws-lc-rs` removal + the `reqwest`/`oci-client` major-unify (B4+C1) are rehomed to the dependency roadmap, with the durable fix filed upstream as [oras-project/rust-oci-client#274](https://github.com/oras-project/rust-oci-client/pull/274) (`rustls-tls-no-provider`, validated to drop `aws-lc-rs` from the tree) — refactor-close is no longer gated on Plan 126 (#1117/#1120). Plan 118's default-path Vz warm-claim is fixed (#1112/#1101): dev-rootfs standbys now pair the dev kernel, so `up --dev` / transient `run` consume the standby instead of silently cold-booting. Plan 189 WS-1 `vm save`/`vm restore` aliases landed (#1118). Plan 205 builder-VM residency Step 1 landed (#1114): `MVM_RESIDENCY` now governs the builder VM warm/cold (no-live-VM half; Step 2 is macOS-26-gated). Plan 205 (resident builder control plane + residency model) is substantially complete: WS-A/B/D/F merged (#1090/#1094/#1099/#1103), WS-E via #1102, and WS-C's resident builder daemon delivered by Plan 204's `mvm-builderd` (#1091). The builder-tier trust gate (#1110) extends `xtask check-trust-gradient` to all three daemons (`clean (3 rows)`), so the host→builder→workload trust gradient is machine-checked end to end; `MVM_RESIDENCY` (warm⇄parked, per-host default) and the `mvmctl doctor` residency line are wired, and idle vz standbys demote to `StandbyState::Parked` and resume on claim (libkrun reaps to cold). One correction from execution: workload-standby freshness is the existing `StandbyCompat` (kernel+image sha), not the builder fingerprint (Plan 195's fingerprint is builder-VM scope). Deferred: live macOS-26 proof of demotion + resume timing (gated like the Plan 118/159 live lanes), the OCI `run --image` residency path, and a latent `host_signer` substring false-positive shared with the prod-agent gate. Plan 193 transparent terminator wiring is explicitly blocked on rvproxy schema/support, not an mvm-only code slice: the local rvproxy config and `GatewayConfig` surfaces have no typed transparent `:80`/`:443` interception field or host-terminator destination, so mvm must keep libkrun/Vz at `VsockUdsChannel` and must not claim `supports_transparent_terminator()` until rvproxy lands the contract. Plan 193 already has the required rvproxy parity gate: `.github/workflows/rvproxy-parity.yml` runs on every PR and merge-group SHA, emits a stable `rvproxy gateway parity` job, skips unrelated PRs/merge groups through a cheap Ubuntu changes detector, and requires the macOS `rvproxy vs gvproxy parity` candidate/control run when gateway-contract files change; `rvproxy gateway parity` is in the `main` branch-protection required-status list. Remaining Plan 193 work is rvproxy transparent-terminator contract/implementation, mvm wiring once that schema exists, then splice/Plan-141 `on_packet` deletion. Plan 189 WS-1 save/restore aliases landed: `mvmctl vm save` / `mvmctl vm restore` now reuse the existing `vm-full` checkpoint primitive, fail closed on the Vz `save-restore` snapshot tier before mutation, and are covered by parser, audit-coverage, convergence, docs, and rollup updates; live macOS save→stop→restore acceptance remains open. Plan 189 WS-3 checkpoint/snapshot JSON coverage advanced: the grouped `mvmctl vm checkpoint` surface now has JSON for restore/rm/fork in addition to existing create/ls/diff coverage, `mvmctl vm snapshot rm --json` mirrors existing snapshot list JSON, parser tests pin every new flag, and the public CLI reference now reflects the grouped `vm checkpoint` / `vm snapshot` commands instead of stale top-level examples. Plan 205 / ADR-090 were added as the umbrella over Plans 118/152/159/196/202/204: they remove the per-session builder boot (the top latency pain) without moving authority into a guest, via two load-bearing decisions — a **three-daemon trust gradient** (host control daemon holds keys/admission/audit in the TCB; builder daemon `mvm-builderd` is dev-tier, resident, build-only; workload guest agent is the prod-stripped, zero-authority runt; authority decreases monotonically host→builder→workload and keys never cross the host→builder vsock line) and a **residency slider** (one standby-pool `min`+idle knob spanning always-warm ⇄ parked-snapshot-resume, per-host default). The builder VM is dev-tier so snapshot/resume regresses no claim, and claim-11 prod deps stay safe through host-side content-addressed admit-time re-verification regardless of boot path. Plan 205 consumes Plan 204's typed `BuilderRequest` protocol rather than reimplementing it. Plan 204 / ADR-089 were added in draft PR #1082 as the long-term builder-control-plane design: users keep one host `mvmctl` surface, normal use does not require host Nix, and Nix/Linux-only work moves behind a resident `mvm-builderd` service inside the builder VM over typed vsock requests. Plan 202 is complete. ADR-084 is accepted; local `mvm` landed the full per-tenant daemon migration (Phases 1–4 and 6, including Vz live/default verification), and mvmd PR #162 closed the remaining Phase 5 cross-repo work: per-tenant daemon lifecycle ownership, Firecracker broker substrate, the reusable delegated host-services route/authz surface with five low-risk typed ALPN reads, tenant-key/boundary proof, and `O(active tenants)` density evidence. Plan 200 C2 SDK/CLI admission-input and unknown-key parity proof landed: Rust, Python, and TypeScript now share richer `machine run` argv fixtures that the Rust CLI tests parse through `machine run` plus the `mvmctl run` dry-run/preflight and receipt-summary seam, proving CPU/memory/profile, sorted env-key redaction, volume host-path hashing, timeout propagation, command hashing, effective policy, and receipt-input parity stay CLI-owned without invoking Nix or booting a VM. Python/TypeScript `Machine.create --manifest` shared fixtures now also reach the CLI strict-manifest unknown-key gate. Plan 200 signed-plan auth metadata is closed: `ExecutionPlan` schema v6 carries `auth.mode` (`none` / `ssh_agent_socket`), persistent `machine start` passes `ssh_agent_socket` only after dev-tier profile validation, and admitted policy audit labels report `auth_mode=ssh-agent-socket`. Non-standard-port SSH denial now has runtime ingress banner classification plus Firecracker bridge/TAP banner drop coverage, with Firecracker KVM proof at `4ce7d938`. Remaining C2/auth proof: artifact-verification non-bypass, live admission/non-bypass proof, and live VM SSH-agent round-trip after rerunning the Firecracker in-guest raw probe through the port-5301 runtime-UDS fix. Plan 118 PR-10c advanced from pure substrate to live-gated CLI wiring: `bench microvm-density --count/--max-count` now boots/holds admitted libkrun probe VMs for footprint sampling, `bench microvm-launch --concurrency N` emits a concurrent-wave P50/P95/P99 report, platform footprint accessors are split between Linux `smaps_rollup` PSS and macOS `proc_pid_rusage` `phys_footprint`, and the x86_64/libkrun loader follow-up now threads `KernelFormat` plus extracted ELF `vmlinux` so the remote KVM smoke moves past libkrun's `Kernel doesn't fit in RAM` panic. Baselines remain blocked because the default image still never exposes the guest-agent `vsock-5252.sock` on that host, and the live no-leak/nonce-distinctness proof plus Vz/FC lanes remain open. Plan 159 is closed: VZ/macOS scope is shipped, verb/install/product polish is rehomed to Plan 181 / Plan 200, and signed delta-image distribution is descoped until an owning artifact/distribution plan is opened.

**Additional 2026-06-19 — Plan 205 "instant" bar made a CI gate:** Plan 205's residency acceptance was hardened from "measured, not asserted" into an explicit, CI-gated **latency budget** (new "Latency budget" section): warm (`min≥1`) Nth-command takes the resident-daemon branch with **no boot** and **< 50 ms** control round-trip (deterministic in the PR matrix); parked (`min=0`) resume **< 100 ms** P50 (P95 ≤ 2×) on the backend-bearing Vz/FC live lane; cold second-ever boot is a restore within the parked budget. The first-ever image-download cost is explicitly out of the budget — paid once at install/prefetch time (the proposed `mvmctl bootstrap` prefetch + install-script prefetch), never on the per-command hot path. A regression past either budget fails the build; budgets tighten as the paths land, never loosen silently.
...
- [~] **PLAN 126** — Dependency reduction · 🟢 mvm-side default-closure cuts/gates measured and landed; `sigstore`/`opendal`/`pgp` are out of the default closure, D1 forbidden-dep gate and D2 duplicate-major ratchet are live, and `dep-baseline.md` has the final measure. **Step-0 decision made 2026-06-20: the `aws-lc-rs` removal + `reqwest`/`oci-client` major-unify (B4 + C1) are rehomed to the dependency roadmap, not closed in this refactor cycle.** Rationale: `aws-lc-rs` enters *solely* via `oci-client → reqwest 0.13` (mvm's own reqwest 0.12 is already ring); `oci-client` 0.16 **and** 0.17 hardcode the aws-lc provider in their only rustls option with no ring/no-provider feature, so removal needs a feature on `oci-client` itself, not a config change. That feature is now filed upstream as **[oras-project/rust-oci-client#274](https://github.com/oras-project/rust-oci-client/pull/274)** (`rustls-tls-no-provider` = `reqwest/rustls-no-provider` + `jsonwebtoken/rust_crypto`); validated against upstream `main` that it drops `aws-lc-rs` from the tree (`cargo tree -i aws-lc-rs` empty, builds clean — `rustls-platform-verifier` is provider-agnostic and does *not* re-drag aws-lc). Rehomed rather than carried as a fork: once #274 lands it is a clean `oci-client` bump + feature flip + ring-provider install (which also collapses the reqwest 0.12/0.13 split); a `[patch.crates-io]` bridge to the proven branch is available if we want aws-lc gone sooner. The D1/D2 gates keep the regression closed meanwhile. Refactor-close is **no longer gated** on this item.
...
- [ ] **PLAN 189** — VZ DX parity (post-convergence) · 🟡 in progress — WS-1 `vm save`/`vm restore` aliases landed (#1118 — over the `vm-full` checkpoint, Vz `save-restore`-tier-gated, live macOS save→stop→restore acceptance open); WS-3 `dev status/down/up --json`, checkpoint/snapshot JSON, structured-stdout hardening, Vz dev-shell data-port reachability, and source-checkout Vz helper freshness landed; remaining: cached fast-boot acceptance, linux-native richer JSON detail if needed, live save→stop→restore acceptance, base pinning (spun out of 177; sibling of 159)
...
PLAN 189 — VZ DX parity (post-convergence)        🟡 in progress  (ADR-076 §"Out of scope")
  ...
  [~] WS-1 save/restore aliases · WS-2 cached fast-boot default · WS-3
      linux-native richer detail if needed · WS-4 base pinning
  [x] WS-1 surface save/restore verbs — `vm save` / `vm restore` are thin
      ...
      `save-restore` snapshot_capability tier; live acceptance remains open
  ...
  [ ] WS-3 --json coverage across vz lifecycle verbs (checkpoint/snapshot done;
      linux-native richer detail remains)
...
PLAN 193 — rvproxy network substrate (replace gvproxy/passt)  🟡 WS-1 proven; WS-2.2d mvm-side native parity landed; parity gate required; transparent terminator blocked on rvproxy schema; splice deletion remains
...
PLAN 200 — Machine UX/DX layer  🟢 in progress — `machine run` shipped (#968) + WS-B `--net`/`--allow-host` egress enforcement MERGED (#1003)
  ...
  [ ] Implement SDK parity for Python, TypeScript, and Rust, including non-bypass tests for
      ...
      lifecycle/error tests; deeper parity/non-bypass proof remains.
...
PLAN 205 — Resident builder control plane + residency model (umbrella)   🟢 SUBSTANTIALLY COMPLETE (ADR-090)
  ...
  [~] Session reuse / readiness / crash-recovery — Plan 204 domain (mvm-builderd lifecycle).
  D. Snapshot park/resume — ✅ #1099 (logic; live resume timing deferred)
  ...
  [~] Firecracker leg (Plan 175) — FC/libkrun stay reap-to-cold; live-memory gated.
  ...
  Deferred (tracked in plan docs): live macOS-26 proof of warm→parked demotion + parked
  resume timing (gated like the Plan 118/159 live lanes); OCI `run --image` residency; the
  latent `host_signer` substring false-positive shared with the prod-agent gate.
```
