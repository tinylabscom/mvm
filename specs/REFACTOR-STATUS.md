...
**Additional 2026-07-08 — #1462 arbitrary OCI PID 1 hardening live-proven on Firecracker/KVM.** OCI rootfs injection now copies a static `mvm-oci-init` binary to `/init` instead of generating a shell script, eliminating the previous dependency on `/bin/sh`, busybox, coreutils, or image-provided init behavior. OCI config `Entrypoint`/`Cmd` is translated into a static `mvm-oci-entrypoint` wrapper under `/usr/lib/mvm/wrappers/` plus the existing `/etc/mvm/entrypoint` marker, preserving image argv/env/workdir without shell expansion or `sh -c`. A scratch OCI image containing only `/marker` materialized into an injected rootfs with executable `/init`, no `/bin/sh`, and no `/bin/busybox`; direct Firecracker boot on the Hetzner KVM host reached guest-agent vsock readiness with `net:false`, `allow_host:[]`, and no guest NIC (`eth0` absent). Pause succeeded and Firecracker snapshot warm-start loaded with HTTP 204. Remaining follow-ups are narrow: the direct `machine run --image oci-archive:...` path must propagate kernel-source/cached-kernel selection instead of hitting a missing published checksum URL, and scratch-OCI post-restore VMGenID delivery still reports the guest agent unreachable after warm-start.

**Additional 2026-07-08 — Plan 227 WS-B1/B2 workload cutover advanced.** Firecracker and libkrun workload boots now satisfy the same `vsock + no_guest_nic + host_vsock_proxy` contract HVF already carried: Firecracker no longer provisions a workload TAP/network-interface dataplane, guest IP metadata, or host-side redirect path for workload egress; libkrun no longer uses gvproxy/passt workload networking and now always boots through the host-vsock proxy gate in raw-or-substitution mode. The current workload product surface is HVF + Firecracker + libkrun; stale Vz selectors were also removed from active helper surfaces (persistent-builder rejects `--builder vz`, sandbox GC enumerates the real supported backends, and current docs/specs no longer advertise Vz as a live workload choice or a shipped macOS checkpoint path, and the removed legacy-macos archival specs no longer exist as link targets) while legacy code remains only for non-product cleanup paths.
