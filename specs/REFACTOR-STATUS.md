...
**Additional 2026-07-09 — Plan 239 next kernel-config subtraction pass PROPOSED.** `specs/plans/239-kernel-config-subtraction-pass.md` records the next concrete slim-kernel reduction pass as its own plan instead of leaving it as a loose follow-up under Plan 209. The scope stays inside `mvm`'s generated kernel recipe: inspect the resolved config, audit workload-first removal candidates, trim unused compression and initramfs support, test `CONFIG_CC_OPTIMIZE_FOR_SIZE` on the workload kernel, then refresh the budget gate with measured results. It does not change the block+ext4+dm-verity model or introduce committed hand-edited `.config` files.
...
PLAN 239 — Next kernel-config subtraction pass   🔴 PROPOSED
  [ ] Expose and record the resolved builder/workload kernel configs and baseline metrics.
  [ ] Audit workload-first feature removals and trim unused compression/initramfs support.
  [ ] Test a size-optimized workload kernel and refresh the kernel budget gate with measured results.
