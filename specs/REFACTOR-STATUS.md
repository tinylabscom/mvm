...
**Latest update: 2026-06-19** Plan 118 PR-10c's first pure-substrate slice landed: density and concurrent-launch report shapes plus VM-free footprint/distribution helpers and tests are in place; live density CLI, platform footprint accessors, concurrency orchestration, baselines, and Vz/FC lanes remain open. Plan 159 is closed: VZ/macOS scope is shipped, verb/install/product polish is rehomed to Plan 181 / Plan 200, and signed delta-image distribution is descoped until an owning artifact/distribution plan is opened.
...
PLAN 200 — Machine UX/DX layer  🟢 in progress — `machine run` shipped (#968) + WS-B `--net`/`--allow-host` egress enforcement MERGED (#1003)
  ...
  [ ] Implement SDK parity for Python, TypeScript, and Rust, including non-bypass tests for
      ...
      and receipt/audit summaries. Python and TypeScript now have
      ...
      lifecycle/error tests; Rust builders and deeper parity/non-bypass proof remain.
...
- Plan 200 is actively landing in slices. The persistent spec + running-VM wrapper slice landed in #1048; persistent image-backed `machine start`, manifest-to-machine runtime mapping, policy surfacing, dev-tier SSH-agent forwarding, and Python/TypeScript SDK machine wrappers are now landed. The next open slices are Rust SDK builders plus deeper SDK/CLI parity/non-bypass proof, scenario/limitations docs, portable artifacts, perf/smoke coverage, and dependency/binary budgets.
...
1. Continue Plan 200 with the remaining C2 SDK parity: Rust machine lifecycle builders plus SDK/CLI admission-input, receipt/audit-summary, artifact-verification, default-deny network, unknown-key, and source-conflict non-bypass tests. Python/TypeScript wrappers are landed.
...
- [ ] SDK parity: Python and TypeScript machine wrappers now reuse the CLI path by shelling only to `mvmctl machine ...`, expose structured `MachineError`s, and have fake-CLI lifecycle/error tests. Remaining: Rust machine builders plus SDK/CLI admission inputs, artifact verification, default-deny network, unknown keys, source conflicts, and receipt/audit summary non-bypass/parity tests.
