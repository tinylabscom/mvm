...
- [ ] **PLAN 125** — CLI surface + SDK DX veneer · 🟢 **Phases B + C complete** + D1 TS `exec` parity, in **both** SDKs (imperative `Sandbox` whole; `CodeSandbox` + `BrowserSandbox` presets); Phase E cross-cutting (coherence test, `--secret`, doctor table, profiles, host-services SDK) remains
