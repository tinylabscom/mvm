...
**Additional 2026-06-26 — Plan 213 Workstream B release-pack asset gate implemented (branch `feat/plan-213-release-pack-gate`, stacked on `feat/plan-213-explain-launch-attestation`).** The release workflow now builds runtime-pack archives for `aarch64:vz`, `aarch64:libkrun`, `aarch64:firecracker`, and `x86_64:firecracker`, plus builder-pack archives for `aarch64` and `x86_64`, from the existing default-microVM, runtime-overlay, and builder-VM image outputs. Each pack ships with a manifest, provenance, checksum, SBOM reference, cosign signature bundles for the archive and metadata, and expected version metadata; `verify-release-assets.sh` fails closed when any required pack sidecar is missing or inconsistent. Coverage: `packaging/release/test-release-pack-gate.sh` covers the happy path, missing pack signature bundle, mismatched version metadata, and missing pack input failure, and runs in CI lint. **Plan 213 Workstream B remains open only on the pack rebuild-and-compare reproducibility job**; Plan 213 remains open on default install-time preparation, runtime/builder fast paths, builder-VM artifact preparation, actual setup-cache artifact production/consumption, revocation/mirror policy, metrics, and broader docs/migration workstreams.
...
PLAN 213 — Attested fast-first-boot packs   🟢 IN PROGRESS (ADR-097)
  ...
  [~] Workstream B — Release production and artifact publishing.
      ...
      fail-closed. Remaining: rebuild-and-compare reproducibility job for at
      least one runtime pack and one builder pack.)
