...
**Latest update: 2026-06-17** Plan 202 Phase 4a registration journal landed on top of the Phase 3b cost-framing closeout and Phase 2d signer-helper restart work. `mvm-host-agent` now persists the live per-tenant `RegisterVm` set as a deterministic `registrations.json` snapshot beside `control.sock`, rewrites it after successful register/deregister, and replays it after the signer helper is ready on daemon startup. Phase 3b closed host-services process cost as `O(active tenants)`: one resident `mvm-host-agent` plus one supervised `mvm-signer-helper` per active tenant, while each additional VM adds only a registration, broker socket, and per-VM chain head. Phase 4b supervised daemon restart remains open. Plan 125 is closed/rehome-only: its SDK/CLI veneer work is complete, and the remaining host-services daemon/process-model work now lives in Plan 202.
...
- Plan 202 Phase 2d landed in #1049; Phase 4a is now the active Plan 202 PR.

Open PRs at update time:

- `#1052` / `feat/plan-202-3b-cost-semantics`: current PR; closes Plan 202 Phase 3b cost framing after #1051.
...
2. Continue Plan 202 next: finish Phase 4b supervised daemon restart/crash semantics, vz live-verify, mvmd adoption, and retirement of `spawn_broker_services_if_admitted`.
