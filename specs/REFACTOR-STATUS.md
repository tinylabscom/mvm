...
**Latest update: 2026-06-17** Plan 202 Phase 4a registration journal landed on top of the Phase 2d signer-helper restart work. `mvm-host-agent` now persists the live per-tenant `RegisterVm` set as a deterministic `registrations.json` snapshot beside `control.sock`, rewrites it after successful register/deregister, and replays it after the signer helper is ready on daemon startup. Recovery uses the normal register path, so tenant checks, path-safe `vm_id` validation, broker-socket bind, and signer-helper chain reopen remain fail-closed. Phase 4b supervised daemon restart remains open. Plan 125 is also closed/rehome-only: its SDK/CLI veneer work is complete, and the remaining host-services daemon/process-model work now lives in Plan 202.
...
- `#1051` / `feat/plan-202-4-supervision`: carries Plan 202 Phase 4a registration journal.
...
2. Continue Plan 202 next: land #1051, then finish Phase 4b supervised daemon restart/crash semantics, vz live-verify, mvmd adoption, and retirement of `spawn_broker_services_if_admitted`.
