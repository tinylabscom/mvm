...
- [ ] **PLAN 125** — CLI surface + SDK DX veneer · 🟢 **Phases B + C + D complete** in **both** SDKs (imperative `Sandbox` whole; `CodeSandbox` + `BrowserSandbox` presets; TS `exec` parity); Phase E underway — E2 `--secret NAME:host` + E1 Python⇔TS decorator coherence + E3 doctor backend-capability matrix + E4 named security profiles (`--security-profile`, default `production` = highest-security + only deployable; `dev` is development-only and never deploys, refused under `--prod`) landed (the "one IR" mirror guarantee; `mvm.toml`/flake aren't Workload surfaces — reframed). **Remaining:** E5 host-services SDK (workload→broker, 3-layer) — **E5.1 Layer-1 broker transport** (`mvm-guest::broker_client`: advisory guest client over `mvm_guest::vsock`, `BROKER_PORT` 5300, bare `ServiceCall` with host-side connection-identity binding, typed `BrokerError`; claims 8/12/13) **+ E5.2 typed `host.audit.v1` methods landed** (`mvm-guest::host_audit` `emit`/`emit_batch` reusing `mvm-core::protocol::host_audit` wire types; the host `HostAuditV1Handler` already exists + forces `workload_audit`/4 KiB/20-s — so claim 8 is structural: `EmitRequest` has no `category`; typed `AuditError`) **+ E5.4 typed `host.time.v1`/`host.cost.v1` methods landed** (`mvm-guest::host_time` `now` + `mvm-guest::host_cost` `workload`/`tenant`, `_on` stream variants, over the same `broker_client`; the host time/cost handlers aren't built yet so the wire contract is established in `mvm-core::protocol::host_time` `TimeNowResponse{wall_ms}` + `host_cost` `CostReport{spent_micros_usd}` — int-only, `deny_unknown_fields`, reused unchanged by the future handler; typed `TimeError`/`CostError` incl. `NotImplemented` for the mvmd-delegated `cost.v1::tenant`; scope = verb → empty request body; 18 RED-first tests); E5.3a `BROKER_PORT` reserved in `host_listen_ports` (libkrun + vz, fail-closed). **Remaining E5:** E5.3b per-VM broker/audit-signer subprocess spawn (unblocks the live round-trip) + E5.3 PyO3/napi veneer & live-VM E2E for all three services — and **Phase A — the headline 52-verb → ≤15 nested-group CLI restructure (A1 design + A2 implement + docs; not started)**
