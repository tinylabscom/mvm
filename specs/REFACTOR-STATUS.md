...
- Plan 200 is actively landing in slices. The persistent spec + running-VM wrapper slice landed in #1048; persistent image-backed `machine start` and the admitted persistent boot helper are now landed, while the remaining machine-workflow mapping is the next open slice.
...
1. Continue Plan 200 with manifest-to-machine runtime mapping: thread `net`, `[network].allow_hosts`, `[auth].ssh_agent`, `[dev].init`, volumes, `cpus`, `mem`, and `mem_initial` into durable machine specs and the admitted launch path, rather than leaving them parser-only.
