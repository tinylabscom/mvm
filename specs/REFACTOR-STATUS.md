...
- [ ] **PLAN 126** — Dependency reduction · 🟡 mvm-side default-closure cuts/gates measured and landed; `sigstore`/`opendal`/`pgp` are out of the default closure, D1 forbidden-dep gate and D2 duplicate-major ratchet are live, and `dep-baseline.md` has the final measure. Remaining scope: `oci-client` fork/replace/upstream decision to remove `aws-lc-rs` and unify `reqwest`/`oci-client` majors, plus follow-up sweeps.
...
- [ ] **PLAN 189** — VZ DX parity (post-convergence) · 🟡 in progress — WS-3 `dev status/down/up --json` plus checkpoint/snapshot JSON landed; remaining: save/restore verbs, cached fast-boot acceptance, linux-native richer JSON detail, base pinning (spun out of 177; sibling of 159)
...
- [ ] **PLAN 205** — Resident builder control plane + residency model (umbrella) · 🔴 proposed ([ADR-090](adrs/090-resident-daemon-trust-gradient-and-residency.md)) — umbrella over Plans 118/152/159/196/202/204 that removes the per-session builder boot (the top latency pain) without moving authority into a guest. Two load-bearing decisions: a **three-daemon trust gradient** (host control daemon = keys/admission/audit in the TCB; builder daemon `mvm-builderd` = dev-tier, resident, build-only; workload guest agent = prod-stripped runt, zero authority — authority decreases monotonically host→builder→workload, keys never cross the host→builder vsock line) and a **residency slider** (one standby-pool `min`+idle knob spanning always-warm `min≥1` ⇄ parked-snapshot-resume `min=0`, per-host default). Builder VM is dev-tier so snapshot/resume regresses no claim; claim-11 prod deps stay safe via host-side content-addressed admit-time re-verification independent of boot path.
...
PLAN 126 — Dependency reduction                 🟡 default-closure cuts landed; OCI/TLS stack remains
  [x] A1 re-baseline
...
PLAN 189 — VZ DX parity (post-convergence)        🟡 in progress  (ADR-076 §"Out of scope")
  ...
  [ ] WS-3 remaining: linux-native richer detail
...
PLAN 193 — rvproxy network substrate (replace gvproxy/passt)  🟡 WS-1 proven; WS-2.2d mvm-side native parity landed; parity gate requireable; branch-protection activation/terminator implementation/splice deletion remain
  ...
  WS-2.2d (mvm-side, #1009): native rvproxy audit feed + native-enforcement parity, no splice.
    ...
    - remaining: make the parity gate required, land/wire rvproxy macOS transparent terminator
      support, then delete the splice + Plan-141 on_packet hooks.
...
PLAN 205 — Resident builder control plane + residency model (umbrella)   🔴 PROPOSED (ADR-090)
  ...
  [ ] Codify the three-class model + authority/trust-tier invariant in arch docs.
  [ ] Structural test: workload guest image has no key, no admission, no prod do_exec/console.
  [ ] Structural test: builder daemon links no host-signer key path / admission entrypoint.
  [ ] Check the host control daemon stays per-tenant (no global key holder; claim 12/13 moat).
  B. Residency policy over the standby pool
  [ ] Residency policy (`min` warm + idle) over the Plan 118 pool.
  [ ] warm→parked idle demotion + parked→warm on-demand promotion.
  [ ] Per-host default (AS dev = warm, CI = parked) + explicit override.
  [ ] `mvmctl doctor` reports live residency state + default source.
  C. Resident builder daemon
  [ ] Make `mvm-builderd` long-lived across invocations (consume Plan 204 protocol).
  [ ] Session reuse: second command reuses live builder + hot store/page cache (Plan 196).
  [ ] Readiness / version-skew / crash-recovery for the resident daemon.
  D. Snapshot park/resume
  [ ] Wire Plan 159 (Vz) snapshot into the parked state; sub-second resume.
  [ ] Firecracker leg via Plan 175 (fast-boot fallback until live-memory lands).
  [ ] Key snapshot freshness/invalidation to the builder fingerprint (Plan 195).
  E. Cold acquisition
  [ ] Bake a snapshot on first successful boot so the second boot is a restore.
  [ ] Source-checkout path stays free of mvm-release artifacts (ADR-046/089).
  [ ] Doctor distinguishes never-built / parked-snapshot / warm.
  F. Docs and posture
  [ ] "What runs where" table (host control / builder daemon / workload agent).
  [ ] Document residency default, override, and RAM-vs-latency tradeoff.
  [ ] Threat-model delta: lifecycle changes, trust boundary unchanged; per-claim why.
```
...
4. Continue Plan 193 only after rvproxy cross-repo slices are ready: make the parity gate required, land/wire rvproxy macOS transparent terminator support, then delete the splice and remove Plan-141 hooks.
