...
**Additional 2026-06-20 — Plan 205 parked-resume budget reconciled (#1141):** the parked (`min=0`) resume budget was corrected from `< 100 ms` to **≤ a cold boot of the same closure** (single-digit seconds; ~2.3 s measured for a 512 MiB builder, 2026-06-13). The original figure was physically impossible — a full guest-memory restore is not a 100 ms operation — and contradicted the team's own live measurement, so the acceptance it gated could never pass as written. Parked trades resume latency for zero idle RAM; only the warm path (`min≥1`, no boot, `< 50 ms` control round-trip) is the "instant" bar. ADR-090 §2 was internally inconsistent (diagram `<100ms` vs prose "well under a second") and is now reconciled to match. The live lane asserts `resume-ms ≤ cold-boot-ms` for the same closure. Still deferred: a `bench residency-resume` harness + a force-park hook / idle override so the parked path is reachable without the 20-min TTL (tracked in Plan 205 §"Deferred follow-ups").

**Additional 2026-06-19 — Plan 205 "instant" bar made a CI gate:** Plan 205's residency acceptance was hardened from "measured, not asserted" into an explicit, CI-gated **latency budget** (new "Latency budget" section): warm (`min≥1`) Nth-command takes the resident-daemon branch with **no boot** and **< 50 ms** control round-trip (deterministic in the PR matrix); parked (`min=0`) resume **< 100 ms** P50 (P95 ≤ 2×) on the backend-bearing Vz/FC live lane [**superseded 2026-06-20 — see below**: the parked budget is now `≤ a cold boot of the same closure`; `< 100 ms` was physically impossible for a 512 MiB memory restore (~2.3 s measured) and scoped only to the warm control round-trip]; cold second-ever boot is a restore within the parked budget. The first-ever image-download cost is explicitly out of the budget — paid once at install/prefetch time (the proposed `mvmctl bootstrap` prefetch + install-script prefetch), never on the per-command hot path. A regression past either budget fails the build; budgets tighten as the paths land, never loosen silently.
...
- [ ] **PLAN 175** — Firecracker live-memory warm-start · 🟡 T1 host/unit landed (VMGenID delivery); T2–T4 + live gating remain (live-KVM-gated)
