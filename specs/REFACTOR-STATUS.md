...
**Latest update: 2026-06-17** Plan 202 Phase 2a/2b signer-helper path landed: `mvm-signer-helper` is a resident per-tenant child of `mvm-host-agent`, owns `vm_id → Chain` workload-audit heads, opens/closes per-VM chains on register/deregister, and keeps tenant key material out of the host-agent address space. The shared helper wire (`register_vm` / `deregister_vm` / `append_entry` / `probe`) and host-agent audit forwarding by server-derived `vm_id` landed; remaining Phase 2 work is admission signing through the helper and restart/head rebuild.
...
PLAN 202 — Host services daemon (per-tenant, not per-VM spawn)   🟡 IN PROGRESS (ADR-084, #977)
  ...
  [ ] Phase 2 — audit-signer daemon (2a helper core + 2b forwarding landed;
      2c admission signing and 2d persisted-head restart remain).
