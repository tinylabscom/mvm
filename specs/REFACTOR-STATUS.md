...
**Additional 2026-06-19 — Plan 200 auth-proof live follow-up:** the Firecracker-host proof progressed beyond the earlier libkrun materializer blocker but still did not reach a workload VM. Explicit QEMU builder selection now routes OCI `rootfs.ext4` materialization through the QEMU builder, QEMU virtiofs sockets now use short `/tmp/mvm-vfs-*` paths, and dev-build rename fallback copies now preserve `rootfs.ext4` sparsity instead of relying on Linux `copy_file_range`. The remote proof host then needed reboot to clear ext4/jbd2 `D`-state `mvmctl`/Cargo processes and came back pingable but with SSH refusing connections, so final branch-local rebuild and live SSH-agent round-trip remain gated on SSH/sshd recovery or an equivalent Linux/KVM host. Non-standard-port SSH-banner denial still needs a runtime banner classifier before it can be claimed.
