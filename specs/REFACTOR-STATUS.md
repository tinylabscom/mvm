...
- [ ] **PLAN 124** — Lean guest agent · 🟡 ~65%; D1.2a protocol type stubs + check-stubs CI-gated (Plan 128 C3 Step 2) + D1.2 Step 2a machine-readable req→resp contract; host-side typed RPC client (2b) + signed on-device config remain
...
PLAN 124 — Lean guest agent                     🟡 ~65%
  ...
  [ ] D1.2 Step 2b host-side Rust typed RPC client over the contract (SDKs shell to mvmctl, not vsock)
