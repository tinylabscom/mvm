...
**Additional 2026-07-09 — Plan 239 next kernel-config subtraction pass COMPLETE.** The resolved builder/workload config inspection path is now first-class in the kernel flake, builder-vm flake, CLI docs, and kernel guide; the local `mvmctl build kernel build` path also now realizes a `workload-sizeopt` comparison build. Measured on `aarch64`: builder `1373` built-ins / `17,135,624` raw / `7,640,745` gzip; workload `1329` / `15,929,352` / `7,077,261`; workload-sizeopt `1329` / `13,948,936` / `6,008,471`, with the config diff isolated to `CONFIG_CC_OPTIMIZE_FOR_PERFORMANCE` vs `CONFIG_CC_OPTIMIZE_FOR_SIZE`. Shared dead `RD_*` decompressor support is cut down to gzip-only, the resolved builder/workload configs both keep `CONFIG_INITRAMFS_SOURCE=\"\"` with gzip-only initramfs support, workload-only `BPF_SYSCALL` / perf / profiling / ikconfig / checkpoint-restore removals are landed, the size-mode decision is to keep `workload-sizeopt` comparison-only for now, and the workload budget gate is ratcheted to `1329`. The `--boot-check` helper now boots the pinned workload kernel through the OCI `--image` path, polls guest-agent readiness directly, and is live-proven on this host. `CONFIG_BPF` remains forced in the resolved workload config, so this slice keeps the syscall-facing BPF interface off but does not claim a safe whole-subsystem BPF removal.
