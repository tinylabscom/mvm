...
- [ ] **PLAN 213** — Attested fast-first-boot packs · 🟢 in progress ([ADR-097](adrs/097-attested-downloadable-runtime-and-builder-packs.md)) — Workstream A pack schema/verifier core is implemented; Workstream C local content-addressed pack cache, status, prune, archive promotion, `cache install-pack` local/HTTPS download path, and prepare download resolution are implemented; Workstream D has the first cache-backed `mvm prepare`/`--dry-run` resolver slice; Workstream G has explicit prepare-time OCI tag-to-digest, flake-lock identity, and setup-cache identity/resolution slices; Workstream H has launch-attestation record/log core, launch-boundary emission, and `mvm explain`. Remaining: release publishing, default install-time preparation, runtime/builder fast paths, builder-VM artifact preparation, actual setup-cache artifact production/consumption, revocation/mirror policy, metrics, and broader docs/migration workstreams.
...
PLAN 213 — Attested fast-first-boot packs   🟢 IN PROGRESS (ADR-097)
  ...
  [~] Workstream C — Hardened local artifact cache.
      ...
      release publishing integration and runtime consumption wiring.)
  ...
  [~] Workstream G — OCI and flake artifact preparation.
      ...
      cache-backed pack eligibility, with mutable inputs fail-closed unless
      resolution is requested. Remaining: launch admission wiring, flake lock
      resolution, builder-VM preparation for unpublished/private artifacts,
      setup-cache keying, and broader positive/negative coverage.)
