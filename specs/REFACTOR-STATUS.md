...
- [ ] **PLAN 219** — Agent verb grant delivery · 🟡 Linux Firecracker proof is now broad; macOS + sealed `DevOnly` witness remain — core token encode/decode, sidecar mint, backend cmdline token, config-drive host-signer pubkey delivery, guest pinning against config-drive key, and chain-signed denial audit are implemented and targeted-test green. Linux/KVM now proves pre-service grant staging, grant-listed `RunEntrypoint` success, unlisted `ProdSafe` refusal, allow-listed `UpdateIdleTimeoutAck`, grant-less regression, and live `verb_denied` + audit verification. Remaining = macOS HVF/libkrun parity proof, sealed-image `DevOnly` refusal, and ADR-103 acceptance.
...
PLAN 219 — Agent verb grant delivery            🟡 LINUX LIVE PROOF BROAD; MACOS + DEVONLY WITNESS REMAIN
  ...
  [~] Live validation — Firecracker/Linux now proves pre-service grant staging, fail-closed sealed `require_grant:true`, restricted `VerbNotAuthorized`, allow-listed `UpdateIdleTimeoutAck`, listed sealed-image `RunEntrypoint` success (`"hello ari"` on `examples/python/hello-app`), live caller-side `verb_denied`, and the grant-less regression; remaining: macOS HVF/libkrun witness plus the sealed-image `DevOnly` refusal.
