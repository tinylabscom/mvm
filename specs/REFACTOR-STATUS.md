...
**Additional 2026-07-01 — Plan 217 (agent-verbs population) COMPLETE.** All three tasks merged on `feat-plan-215-agent-verbs-populate`: Task 1 (`GuestRequest::prod_safe_verb_names()` guard-tested), Task 2 (`default_agent_verbs` + `parse_agent_verb_override` in `crates/mvm-cli/src/commands/vm/agent_verbs.rs`), Task 3 (`--agent-verb` flag on `up::Args`, `AdmitPlanForBootParams.{agent_verb_override,is_sealed_prod}` threaded from `PersistentImageStartParams`, `SynthesisInput.agent_verbs` wired at synthesis via `parse_agent_verb_override ?? default_agent_verbs(profile!="dev", has_shares)`, `#[allow(dead_code)]` and unused `names()` helper removed). 6 targeted tests pass; workspace check + fmt + clippy clean.
