...
- Plan 200 is actively landing in slices. The persistent spec + running-VM wrapper slice landed in #1048; persistent image-backed `machine start` is still open and must use an admitted persistent boot helper, not transient `run --image`.
...
1. Continue Plan 200 with persistent image-backed `machine start`: extract/add an admitted persistent boot helper for OCI images rather than routing through transient `run --image`, which tears down by design.
