...
- [ ] **PLAN 199** — `WarmLease` borrow-handle + batched guest exec · 🔴 proposed — DX-ergonomics layer over the Plan 118 standby pool + Plan 169 agent-RPC: an RAII claim/release handle (stops + replenishes a fresh standby, never reuses a dirty VM) + a batched stage→run guest `ExecBuilder` (`dev-shell`-gated argv; prod loops use the no-argv `RunEntrypoint` terminal). Caller-convenience only — no new backend/transport, admission + audit untouched. Docs-only so far (#937).
...
PLAN 199 — `WarmLease` borrow-handle + batched guest exec  🔴 proposed
  RAII claim/release over the Plan 118 standby pool (`SupervisorStandbyPool` + `VmBackend::claim_standby`,
  which re-verifies the signed plan — claim 8 untouched) + a batched stage→run guest exec over the Plan
  169 agent-RPC transport. Caller-convenience only; no new backend/transport, admission + audit path intact.
  [ ] WS-A — WarmLease acquire/release/Drop: stops + replenishes a fresh standby, never reuses a dirty VM
      (the security-correct inverse of the borrow-pool prior art); cold-boot fallback on an empty pool
  [ ] WS-B — ExecBuilder Tier 1: one-stream stage→run pipelining over VsockTransport (no guest change)
  [ ] WS-C — ExecOutcome gains duration + peak_rss_kib (host-side Tier 1; getrusage agent-side Tier 2)
  [ ] WS-D — GuestRequest::ExecBatch Tier 2: one round-trip, dev-shell-gated argv, joins the fuzzed surface
  [ ] WS-E — facade re-export + examples/verification_loop.rs (the embedded-library tier)
  Plan: specs/plans/199-warm-lease-and-batched-exec.md  (docs-only PR #937)
