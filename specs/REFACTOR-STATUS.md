...
1. Merge `#1048` once CI is green, then either fold or supersede it with the landed `machine start` / manifest-parser follow-up work from `feat/plan-200-persistent-machine`.
2. Continue Plan 200 with manifest-to-machine runtime mapping: thread `net`, `[network].allow_hosts`, `[auth].ssh_agent`, `[dev].init`, volumes, `cpus`, `mem`, and `mem_initial` into durable machine specs and the admitted launch path, rather than leaving them parser-only.
