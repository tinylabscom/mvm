...
**Additional 2026-07-08 — Plan 213 §D `mvm prepare` + precise not-instant reasons (runtime-pack scope) on branch `feat/plan-213-dD-prepare`.** `mvm_core::pack_cache::diagnose_pack` scans the promoted-pack cache the same way `resolve_pack` does but, on a miss, surfaces the first captured `PackVerifyError` instead of silently returning `None`; `mvm-cli`'s `runtime_pack::not_instant_reason` maps that (or cache-empty) to a precise human message (architecture/backend mismatch, expired/revoked signature, untrusted key, tampered/corrupt, policy mismatch, mutable OCI ref, …). New `mvm prepare [--dry-run]` (host default runtime pack only — no positional `<image-or-flake>` input yet, see the plan doc's §D Deferred note) reports that same readiness/reason. The default `machine run` build-fallback message (`(None, None)` arm in `build_exec_request`) now states the precise reason instead of a generic "no manifest" notice. Scope note: the sibling `feat/plan-213-eE-autoprefer` branch (unmerged at the time of this change) adds an actual try-runtime-pack-first accelerator ahead of that fallback; this change only explains *why* the fallback is being taken, it does not attempt the pack first — that's eE's contribution, not duplicated here. The workspace no longer carries `colored`, and the stale direct `colored` / `indicatif` edges are gone from `crates/mvm/Cargo.toml` now that `mvm::ui` is only a re-export of `mvm-backend::base::ui`. `mvm-backend` keeps the prior color behavior via a tiny terminal-aware ANSI helper, and `mvm-hostd`'s `web_search` path now builds query URLs explicitly instead of depending on reqwest's optional `.query()` extension method. Final host validation is complete: `cargo test -p mvm-cli --lib` passes under isolated `MVM_DATA_DIR` / `CARGO_TARGET_DIR` / `CARGO_HOME`, so the Slice 2 plan + sprint boxes are now ticked alongside the already-closed slices 1, 3, and 4.
**Additional 2026-07-08 — guest/host reconnect backoff hardening on branch `fix/agent-reconnect-backoff`.** The guest transport layer now treats short restart races as transient across both directions: host→guest `connect_to_port` and guest→host `connect_host_vsock` retry `ENOENT`/`ECONNREFUSED`/`ECONNRESET`/timeout-class setup failures with a bounded exponential backoff (100 ms base, 500 ms cap, 4 attempts total) instead of retrying only the narrow CONNECT-read timeout case. The per-tenant host-agent control client (`send_control`) now does the same while a wrapped worker is restarting, including truncated/early-EOF reply failures. The retry boundary stays intentionally narrow: typed daemon refusals still fail immediately, and nothing replays a request after a stream has been established and the request body may already have been delivered. Coverage now includes deterministic restart witnesses on both sides: `test_connect_to_port_retries_across_listener_restart_before_connect_ack` in `mvm-guest` and `register_vm_retries_across_control_socket_restart` in `mvm-backend`, plus worktree-local clippy and serialized workspace validation.
**Additional 2026-07-08 — current-surface docs/specs scrubbed after Vz deletion.** `specs/02-roadmap.md` and the contributor development guide now describe HVF + Firecracker + libkrun as the live workload/backend surface, stop advertising Vz as a current backend choice, and align the macOS contributor story around HVF/libkrun instead of the deleted Apple-Virtualization path.
**Additional 2026-07-08 — Plan 231 P7 Slice 2 dependency cut CLOSED on branch `codex/dep-drop-inquire`.** The workspace no longer carries `colored`, and the stale direct `colored` / `indicatif` edges are gone from `crates/mvm/Cargo.toml` now that `mvm::ui` is only a re-export of `mvm-backend::base::ui`. `mvm-backend` keeps the prior color behavior via a tiny terminal-aware ANSI helper, and `mvm-hostd`'s `web_search` path now builds query URLs explicitly instead of depending on reqwest's optional `.query()` extension method. Final host validation is complete: `cargo test -p mvm-cli --lib` passes under isolated `MVM_DATA_DIR` / `CARGO_TARGET_DIR` / `CARGO_HOME`, so the Slice 2 plan + sprint boxes are now ticked alongside the already-closed slices 1, 3, and 4.

**Additional 2026-07-08 — gvproxy-specific tests retired from `mvm-hostd` and `libkrun-sys`.** The remaining gvproxy-only bridge witnesses are removed from `crates/mvm-hostd/src/supervisor/gateway_bridge.rs`, and the dedicated `gvproxy::tests` module is removed from `crates/deps/libkrun-sys/src/gvproxy.rs`. Passt and native-rvproxy coverage remain; only dead gvproxy-specific fixtures were dropped. Validation: focused `gateway_bridge` / `libkrun-sys` test runs plus green `cargo check --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, and serial `cargo test --workspace -- --test-threads=1`.
...
**Additional 2026-07-08 — Plan 156 `mvm-sdk` feature audit progressed on branch `chore/dep-slim-httpmock`.** `mvm-sdk` now depends on derive-only `schemars`, and the workspace `tree-sitter` dependency now uses only `std`. Targeted `mvm-sdk` / `mvm-cli` check, full `mvm-sdk` library tests, and targeted clippy are green. The current provisional stubbed local `mvmctl` size stayed flat at 21,108,960 bytes, but the dependency graph is cleaner and confirms the remaining regex Unicode pull on the default path comes from `tree-sitter`'s `std` path itself rather than its broader default feature set.

**Additional 2026-07-08 — Plan 156 direct serde-stack audit progressed on branch `chore/dep-slim-httpmock`.** The workspace direct `serde` / `serde_json` declarations now use explicit `default-features = false` settings with only `derive/std` and `std`, and validation passed via workspace check, workspace clippy, plus focused `mvm-core` protocol, `mvm-cli` JSON output, and `mvm-hostd` framing tests. The current provisional stubbed local `mvmctl` size stayed flat at 21,108,960 bytes, and `cargo tree` still shows transitive default-feature users through `reqwest`, `schemars`, and build-time `tree-sitter`, so this slice is recorded as dependency-surface cleanup rather than an additional measured binary win.

**Additional 2026-07-08 — Plan 156 `clap` feature trim landed on branch `chore/dep-slim-httpmock`.** The workspace `clap` dependency now uses `default-features = false` with only `derive`, `help`, `std`, and `usage`, and `cargo tree -p mvmctl -e features -i clap` confirms the default CLI path no longer carries `color`, `error-context`, or `suggestions`. Focused `mvm-cli` check, clippy, and help-surface unit tests (`top_level_help_hides_infra`, `machine_help_lists_run_first`, `builder_flag_appears_in_help`) are green. The current provisional stubbed local `mvmctl` size dropped from 21,158,544 to 21,108,960 bytes (`-49,584`), making this the first measured non-zero Plan 156 C2 binary win on this branch while the serde-stack audit remains open.

**Additional 2026-07-08 — Plan 156 direct-regex feature audit progressed on branch `chore/dep-slim-httpmock`.** The workspace `regex` dependency now uses `default-features = false` with only `perf`, `std`, and `unicode-perl`, and `mvm-core` now consumes that workspace entry instead of re-enabling `regex` defaults locally. Focused `mvm-core` / `mvm-hostd` check, clippy, and regex-heavy suites (`secrets_scanner`, `injection_guard`, `pii_redactor`) are green. The current provisional stubbed local `mvmctl` size is 21,158,544 bytes, which only removes the earlier temporary `+16` byte drift from the Tokio slice; `tree-sitter` still pulls the full regex Unicode feature set transitively on the default path, so this is recorded as dependency-surface tightening rather than a completed Plan 156 C2 win.

**Additional 2026-07-08 — Plan 156 secondary-binary baseline refreshed on branch `chore/dep-slim-httpmock`.** `docs/investigations/binary-size-baseline.md` now measures the release-bundled helper set that `release.yml` actually ships instead of the stale historical sidecar list in the plan. Recorded bundle: Linux-shared `mvm-bridge` + `mvm-substitution-endpoint`, plus macOS-only `mvm-vz-supervisor`, `mvm-hvf-supervisor`, and `mvm-libkrun-supervisor`. Each row now carries an exact byte count from `/tmp/mvm-size-target/release/*` and a `cargo bloat --release --crates -n 8` snapshot. The current largest sidecar is `mvm-substitution-endpoint` at 8,052,440 bytes, with `aws_lc_sys` and `rustls` prominent in the crate table; the regex stack remains a repeated top contributor across the bridge and the heavier supervisors. Validation used offline release builds for `mvm-vm-host` and `mvm-hostd`, `stat -f '%N %z'`, and per-binary `cargo bloat` runs against the shared `/tmp/mvm-size-target` target dir.

**Additional 2026-07-08 — source-checkout machine-start / Stage 0 dev-DX hardening prepared on branch `fix/dev-kernel-fallback`.** The `mvm-cli` build script no longer emits the zero-byte-stub Cargo warning on every `cargo run -- ...`, so the developer console matches normal CLI output. More importantly, source checkouts now honor the local-build invariant end to end on the workload-kernel / Stage 0 paths: `machine start` chooses a local workload-kernel build on a cold cache instead of fetching the versioned release kernel, and stubbed embedded host binaries fall back to an on-demand source-checkout `cargo zigbuild` for Stage 0 / builder bootstraps instead of forcing `MVM_EMBED_BINARIES=1`. Validation on the branch: `cargo run -- machine ls`, focused `mvm-cli` regression tests, `cargo check --workspace`, and `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test --workspace` still ends on the same three unrelated `mvm` failures already present on `main`.
...
**Additional 2026-07-08 — Plan 213 §I mirror config + fetch-policy modes LANDED on branch `feat/plan-213-iI-enterprise`.** The operator's `pack-trust.json` (`mvm_core::pack_trust::PackTrustConfig`) now carries `mirror: Option<String>` and `fetch_mode: PackFetchMode` (`online` default / `offline_pinned` / `mirror_only` / `local_rebuild_required`), both additive `#[serde(default)]`. The pure resolver `resolve_pack_download_base` picks the mirror-or-default host for `online`, requires a configured mirror for `mirror_only` (loud `PackTrustError::MirrorRequired` on misconfiguration), and returns `None` for the two no-network modes. The attested builder-pack download (`fetch_release_builder_pack_staging`/`fetch_and_promote_release_builder_pack` in `crates/mvm-cli/src/commands/env/dev_vz/bootstrap.rs`) now resolves its base URL through this instead of a hardcoded release host; a forbidding mode logs and falls through to the existing cache/builder-VM path unchanged. Deferred (named in the plan §I progress note): the key-rotation validity window (item 3, design sketch only), the plain non-pack builder-VM image download's hardcoded host, and rerouting the revocation-list fetch through the same resolver.
**Additional 2026-07-08 — OCI `--image` NIC-less vsock-egress activation landed in the macOS cutover lane.** The runtime-injected guest `/init` now includes and launches `mvm-egress-client`, exports SOCKS proxy env vars, and the embedded/cacheable guest-runtime pipeline carries the third guest binary end-to-end for OCI `--image` runs. The final activation is now tied to the **requested outbound policy** (`--net` / `--allow-host`), not a hidden opt-in env var: the OCI exec/preflight/receipt path injects and records proxy env vars automatically, and backend selection now requires a backend that honestly advertises `{ vsock, no_guest_nic, host_vsock_proxy }`. Today that means `hvf`; incapable backends are refused up front instead of silently degrading to a guest NIC. The same gate now applies to persistent OCI-backed machine boots (`machine run -d --image ...` and `machine start` on an image-backed machine), so the stored lifecycle can no longer fall back to the legacy NIC/gateway path after a safe transient preflight. Scope is intentionally TCP/HTTP-only: ICMP is still unsupported, so `ping` remains an invalid smoke for `--allow-host`. This advances Plan 226 WS-N / Plan 227 WS-B activation plumbing but does not yet flip every backend to vsock-only egress or delete gvproxy/passt globally.
...
- [x] **PLAN 234** — Vsock port-handler registry production-readiness · ✅ COMPLETE — the host-I/O loop is readiness-driven rather than 5 ms polling, the active libkrun/HVF runtime path no longer depends on gvproxy, `run --json --receipt` keeps stdout machine-readable with chrome on stderr, and the required post-fail-closed Hetzner builder smoke is green.
...
PLAN 236 — Host-authority runtime roadmap   🟡 IN PROGRESS
  ...
  [x] Phase 1 §1.4 — bake the sealed-image verb-trust policy: the OCI `--prod` path now has a real builder-VM dm-verity seal fallback on branch `feat/plan-236-oci-seal-closeout`; remaining work moved to live-proof closeout rather than a missing code path.
