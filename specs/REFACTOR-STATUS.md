...
**Additional 2026-06-19 — Plan 200 B2 phase timing (measure-first):** the first B2 task landed — `commands::vm::phase_timing` (`RunPhaseMarks`→`RunPhaseTimings`, pure + unit-tested) is wired at the `exec::run_inner` seams (resolve, drives, admit, backend-start, run, teardown) and emits one greppable stderr line behind `MVM_PHASE_TIMING=1` (default off, zero behavior change). The boot micro-benchmark substrate (`bench microvm-launch`) is reused, not rebuilt. The frozen "Stage 0 install cache / faster-first-boot" pivot was rejected as redundant with Plan 196 (warm-store/kernel-cache, WS-2 descoped) + Plan 198 (build cache) + the existing persistent /nix store. Remaining B2: split vsock-ready from command-exit, capture the upstream OCI cache-resolve span, the cached-`machine run` benchmark, the `<200 ms` acceptance bar, macOS measurements, and cache/elide-empty-drives.
