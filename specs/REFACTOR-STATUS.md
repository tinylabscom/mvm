...
- [ ] **PLAN 200** — Machine UX/DX layer · 🔴 planned, docs strategy synced — first-class `mvmctl machine` surface over existing runtime primitives; starts with no-host-Nix binary-install DX, current image-backed one-shot docs, future `machine run`, explicit network opt-in, persistent named machines, SDK parity, verified portable artifacts, measured hot-start latency, friendly exec/shell wrappers, `mvm.toml` schema v1 with strict security defaults, and default-binary-closure dependency budgets
