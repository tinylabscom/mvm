...
- [ ] **PLAN 213** — Attested fast-first-boot packs · 🟢 in progress ([ADR-097](adrs/097-attested-downloadable-runtime-and-builder-packs.md)) — Workstream A pack schema/verifier core is implemented; Workstream B signed runtime/builder release pack publishing, fail-closed release asset verification, and pack rebuild-and-compare reproducibility are implemented; Workstream C local content-addressed pack cache, status, prune, archive promotion, `cache install-pack` local/HTTPS download path, and prepare download resolution are implemented; Workstream D has the first cache-backed `mvm prepare`/`--dry-run` resolver slice; Workstream G has explicit prepare-time OCI tag-to-digest, flake-lock identity, and setup-cache identity/resolution slices; Workstream H has launch-attestation record/log core, launch-boundary emission, and `mvm explain`. Remaining: default install-time preparation, runtime/builder fast paths, builder-VM artifact preparation, actual setup-cache artifact production/consumption, revocation/mirror policy, metrics, and broader docs/migration workstreams.
