...
- [ ] **PLAN 125** — CLI surface + SDK DX veneer · 🟢 **Phase B complete** (imperative `Sandbox`: create/exec/copy/forward/id/info, sync+async, dev-tier-gated) in **both** SDKs, + D1 TS `exec` parity; Phase C helpers, Phase E cross-cutting remain
...
- [ ] **PLAN 193** — rvproxy network substrate (replace gvproxy/passt) · 🟡 WS-1 PROVEN (libkrun-unixgram live `dev up` through rvproxy, ~540k connections relayed, builder rootfs built — rvproxy #38/#42/#53) + WS-1.5 parity-gate scaffold `scripts/rvproxy-gateway-parity.sh` (gvproxy↔rvproxy conformance, refuses non-conforming binary); WS-2 native-flow port (the biggest win — replaces the in-line claim-10 datapath wrapper, Plan 141) + WS-3/4 remain, cross-repo
...
- [ ] **PLAN 200** — Machine UX/DX layer · 🔴 planned, docs strategy synced — first-class `mvmctl machine` surface over existing runtime primitives; starts with no-host-Nix binary-install DX, current image-backed one-shot docs, future `machine run`, explicit network opt-in, persistent named machines, SDK parity, verified portable artifacts, measured hot-start latency, friendly exec/shell wrappers, `mvm.toml` schema v2 with strict security defaults, and default-binary-closure dependency budgets
...
PLAN 200 — Machine UX/DX layer  🔴 planned, docs strategy synced
  ...
  `mvm.toml` schema v2, verified portable artifacts, measured hot-start claims, managed
  macOS virtualization as the safer default, custom kernels as signed runtime/artifact
  payloads, SDKs that mirror the CLI, and dependency weight as a default-binary-closure
  DX/security goal.
  ...
  [x] Record that `image` and `flake` are mutually exclusive source selectors in schema v2.
  ...
  [ ] Implement schema-v2 parser/tests with unknown-key and `image`/`flake` conflict rejection.
  [ ] Implement SDK parity for Python, TypeScript, and Rust.
  [ ] Implement product-level portable artifact pack/run docs and tamper tests.
