...
**Additional 2026-06-27 — Plan 213 Workstream E prepared runtime artifact resolution implemented (branch `feat/plan-213-runtime-pack-launch-preflight`, stacked on `feat/plan-213-runtime-pack-select`).** `mvm_core::packs::cache` now resolves a selected verified runtime pack into concrete cached launch artifact paths by matching semantic runtime output hashes (`kernel_hash`, `rootfs_hash`, `initramfs_hash`, `agent_rootfs_hash`) to manifest file entries. The resolver requires at least one boot payload and refuses unsafe manifest file paths before launch code can consume the pack. Coverage: focused `mvm-core` tests for artifact path resolution, missing boot-payload refusal, and unsafe-path refusal. **Plan 213 Workstream E remains open** on per-run CoW sandboxes, snapshot/warm-claim launch paths, guest-agent dispatch, and live latency evidence.

**Additional 2026-06-26 — Plan 213 Workstream E prepared launch selection implemented (branch `feat/plan-213-runtime-pack-select`, stacked on `feat/plan-213-prepare-ux-closeout`).** `mvm_core::packs::cache` now has a prepared-launch selection API that reuses `PackCache::prepare_report` and returns a launchable pack only when the report is ready, verified, fast-path eligible, and carries pack hash, pack kind, and cache root. Cache misses, setup-cache misses, and malformed ready reports fail closed before any builder preparation callback exists. Coverage: focused `mvm-core` tests for verified runtime-pack selection, cache-miss refusal, setup-cache-miss refusal, and malformed ready-report refusal. **Plan 213 Workstream E remains open** on binding the selected pack into per-run CoW sandboxes, snapshot/warm-claim launch paths, guest-agent dispatch, and live latency evidence.

**Additional 2026-06-26 — Plan 213 Workstream D prepare UX output contract covered (branch `feat/plan-213-prepare-ux-closeout`, stacked on `feat/plan-213-mirror-sources`).** Human `mvm prepare` report output now routes through a pure line renderer with focused tests for ready fast-path, cache-miss next-step, and policy-refusal detail cases. `mvmctl machine run --dry-run` preparation diagnostics use the same testable line-construction shape, with coverage for mutable OCI and manifest cache-miss blockers. The CLI reference documents the human output fields for prepare and machine-run preparation diagnostics. **Plan 213 Workstream D remains open** only on install-time/default-pack preparation and builder/snapshot derivation work that depends on the runtime and builder fast paths.

**Additional 2026-06-26 — Plan 213 Workstream I enterprise mirror source resolution implemented (branch `feat/plan-213-mirror-sources`, stacked on `feat/plan-213-revocation-fetch`).** `mvm prepare`, `mvmctl cache install-pack`, and bootstrap pack preload now resolve relative pack and revocation metadata source names under `--pack-mirror-base` / `MVM_BOOTSTRAP_PACK_MIRROR_BASE`. Explicit local and HTTP(S) sources pass through unchanged, parent-directory components are refused for mirror-relative sources before any download or file read, and the existing HTTP opt-in still gates plain-HTTP mirror downloads. Coverage: focused cache helper tests for HTTPS/local mirror joins, explicit-source passthrough, and traversal refusal; prepare/cache parser tests for mirror base flags; bootstrap config parsing; plus CLI/install docs and Plan/Sprint status updates. **Plan 213 Workstream I is closed** for the policy, revocation, mirror, and offline-operation scope currently represented in the plan.

**Additional 2026-06-26 — Plan 213 Workstream I revocation metadata fetching implemented (branch `feat/plan-213-revocation-fetch`, stacked on `feat/plan-213-revocation-freshness`).** `mvm prepare`, `mvmctl cache install-pack`, and bootstrap pack preload now accept local/HTTPS revocation metadata sources through `--revocations-source` and `MVM_BOOTSTRAP_PACK_REVOCATIONS_SOURCE`, then parse the same fail-closed revocation schema used by local files. Ambiguous local-file plus source configurations are refused, and plain-HTTP metadata downloads stay opt-in behind the existing HTTP escape hatch. Coverage: focused cache helper tests for HTTPS fetch, plain-HTTP refusal, and local/source conflict handling; parser tests for prepare/cache source flags; bootstrap config conflict coverage; and docs/status updates. **Plan 213 Workstream I remains open** on enterprise mirror source resolution and full revocation/mirror operations docs.

**Additional 2026-06-26 — Plan 213 Workstream I offline revocation freshness implemented (branch `feat/plan-213-revocation-freshness`, stacked on `feat/plan-213-pack-policy-modes`).** Local pack revocation metadata can now declare `fetched_at` plus `max_age_seconds`; stale metadata fails closed during pack verification and `mvm prepare` reports `stale_revocation_metadata`. Incomplete freshness metadata is refused at parse time, preventing an operator from thinking a local revocation file is freshness-bound when it is not. Coverage: focused core verifier test for stale revocation refusal, focused cache/prepare test for stale metadata mapping, focused CLI parser tests for stale, fresh, and malformed local revocation metadata, plus docs/status updates. **Plan 213 Workstream I remains open** on enterprise mirror source resolution and full revocation/mirror operations docs.

**Additional 2026-06-26 — Plan 213 Workstream I pack policy modes implemented (branch `feat/plan-213-pack-policy-modes`, stacked on `feat/plan-213-bootstrap-prepare`).** Pack verification now carries typed local policy modes (`online-default`, `offline-pinned`, `mirror-only`, and `local-rebuild-required`), per-channel signing-key pins, verifier-level key-rotation windows, and mirror identity checks. `mvm prepare`, `mvmctl cache install-pack`, and bootstrap pack preload share the same policy construction through `--policy-mode`, `--channel-signing-key`, `--mirror-identity`, and `MVM_BOOTSTRAP_PACK_*` env equivalents. Coverage: focused core verifier tests for missing offline pins, wrong signer, closed rotation windows, mirror mismatch, and local-rebuild refusal; focused cache test for local-rebuild-required prepare routing; focused CLI/bootstrap parser tests for policy mode, channel pin, mirror identity, and malformed inputs. Docs: CLI reference, install script env knobs, installation guide, Plan 213, and sprint status updated. **Plan 213 Workstream I remains open** on enterprise mirror source resolution and full revocation/mirror docs.

**Additional 2026-06-26 — Plan 213 Workstream D explicit bootstrap pack preloading implemented (branch `feat/plan-213-bootstrap-prepare`, stacked on `feat/plan-213-prepare-reasons`).** `mvmctl bootstrap` now installs attested runtime and/or builder packs into the local pack cache when an operator provides explicit `MVM_BOOTSTRAP_RUNTIME_PACK_SOURCE` / `MVM_BOOTSTRAP_BUILDER_PACK_SOURCE` values plus local policy metadata (`MVM_BOOTSTRAP_PACK_POLICY_HASH`, backend, channels, optional host capabilities, trust store, revocations, and HTTP opt-in). The hook reuses the existing quarantine-first cache installer, honors `MVM_SKIP_PACK_PREFETCH=1`, emits cache-install audit entries without storing full source URLs or paths, and is a no-op when no pack sources are configured. Coverage: focused `mvm-cli` bootstrap parser tests for absent config, opt-out, required policy metadata, runtime+builder source parsing, channel/capability splitting, trust/revocation paths, HTTP opt-in, and unknown backend refusal. Docs: install-script env knobs, CLI reference env table, installation guide, Plan 213, and sprint status updated. **Plan 213 Workstream D remains open** on automatic channel/mirror policy, prepared runtime launch consumption, policy-refusal CLI integration coverage, snapshot/warm derivation, and broader install-time size/cache-location docs.

**Additional 2026-06-26 — Plan 213 Workstream D machine-run preparation reasons implemented (branch `feat/plan-213-prepare-reasons`, stacked on `feat/plan-213-release-pack-gate`).** `mvm prepare` now exposes shared `PackPrepareReason` detail and next-step text, and `mvmctl machine run --dry-run` emits a pre-dispatch preparation diagnostic for instant-launch blockers without mutating cache or booting. The diagnostic reports stable reason codes for mutable OCI tags, digest-pinned OCI inputs without a selected verified pack, manifest sources, local/private flakes, and remote flakes requiring builder preparation. Coverage: focused `mvm-cli` tests pin the reason-name/help table and the machine-run source classifier for mutable OCI, missing-pack OCI/manifest, private local flake, and builder-required remote flake cases. **Plan 213 Workstream D remains open** on install-time/default pack preparation, full prepared-pack launch consumption, policy-refusal CLI integration coverage, snapshot/warm derivation, and broader installation/getting-started docs.

**Additional 2026-06-26 — Plan 213 Workstream B release-pack publishing implemented (branch `feat/plan-213-release-pack-gate`, stacked on `feat/plan-213-explain-launch-attestation`).** The release workflow now builds runtime-pack archives for `aarch64:vz`, `aarch64:libkrun`, `aarch64:firecracker`, and `x86_64:firecracker`, plus builder-pack archives for `aarch64` and `x86_64`, from the existing default-microVM, runtime-overlay, and builder-VM image outputs. Each pack ships with a manifest, provenance, checksum, SBOM reference, cosign signature bundles for the archive and metadata, and expected version metadata; `verify-release-assets.sh` fails closed when any required pack sidecar is missing or inconsistent. The release workflow also rebuilds one representative runtime pack and one builder pack from tag source pins and compares archive hashes with the published sidecars. Coverage: `packaging/release/test-release-pack-gate.sh` covers the happy path, missing pack signature bundle, mismatched version metadata, and missing pack input failure, and runs in CI lint. **Plan 213 Workstream B is closed**; Plan 213 remains open on default install-time preparation, runtime/builder fast paths, builder-VM artifact preparation, actual setup-cache artifact production/consumption, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-25 — Plan 213 Workstream H explainability implemented (branch `feat/plan-213-explain-launch-attestation`, stacked on `feat/plan-213-launch-attestation-emit`).** `mvmctl explain <run-id>` now verifies the local hash-chained launch-attestation log before lookup, fails closed on tamper or missing run ids, and prints either a human explanation or JSON containing the verified launch record plus sequence, previous-hash, entry-hash, and log-head metadata. The CLI reference documents the new command. Coverage: focused `mvm-cli` tests for lookup, missing records, human rendering, parser coverage, and machine-readable stdout routing. **Plan 213 Workstream H is now closed**; Plan 213 remains open on runtime-pack fast-path consumption, builder fallback/source precision, release publishing, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-25 — Plan 213 Workstream H launch-path attestation emission implemented (branch `feat/plan-213-launch-attestation-emit`, stacked on `feat/plan-213-setup-cache-key`).** The existing launch audit boundaries now append typed launch attestation records to the local hash-chained log after admitted success/failure, deriving source input, runtime pack hash, policy hashes, backend identity, command digest, result outcome, and error digest from the admitted signed plan. Emission is non-fatal like the existing chain audit path, so local log IO cannot block an already accepted launch. Coverage: focused `mvm-cli` tests for success, launch failure, refusal, builder fallback, verification failure, and interrupted outcome records, plus the earlier `mvm-core` tamper/reorder detection tests. This left the explain command as the remaining Workstream H item, which is closed by the follow-up entry above.

**Additional 2026-06-25 — Plan 213 Workstream H launch attestation core implemented (branch `feat/plan-213-launch-attestation-log`, stacked on `feat/plan-213-setup-cache-resolver`).** `mvm_core::launch_attestation` now defines typed launch records covering source input, builder identity, pack identity, local verification, snapshot/warm derivation, policy admission, command identity, result metadata, backend identity, launcher version, and timestamps. The local launch-attestation log is hash-chained JSONL under the state log tree; appends first verify the existing chain, and verification detects modified or reordered records. Coverage: focused `mvm-core` tests for serde roundtrip, builder validation, append/read chain state, modified-record tamper detection, and reordered-record detection. The later launch-boundary emission and explain entries above close Workstream H.

**Additional 2026-06-25 — Plan 213 Workstream G setup-cache prepare resolver implemented (branch `feat/plan-213-setup-cache-resolver`, stacked on `feat/plan-213-setup-cache-key`).** `mvm_core::packs::cache::PackPrepareRequest` can now declare required setup-cache layer identities, and prepare resolution reports `setup_cache` hit/miss state after pack verification. A verified matching pack remains fast-path eligible when all required setup-cache layer keys are declared by the manifest; missing or invalidated setup-cache layers now return `RequiresBuilder` with `setup_cache_miss` while preserving verified trust state. Coverage: focused `mvm-core` prepare/cache tests for older request default compatibility, required-layer hits, required-layer misses, and invalidation across image digest, flake lock hash, setup command hash, environment hash, mount shape hash, runtime pack hash, and policy hash. **Plan 213 remains open** on actual setup-cache artifact production/consumption, remote builder-VM flake lock resolution, release publishing, default install-time preparation, runtime/builder fast paths, builder-VM artifact preparation, launch attestation/explainability, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-25 — Plan 213 Workstream G setup-cache identity implemented (branch `feat/plan-213-setup-cache-key`, stacked on `feat/plan-213-flake-lock-prepare`).** Pack manifests now carry typed setup-cache layer identities and derive deterministic layer keys over OCI image digest, flake lock hash, setup command hash, environment hash, mount shape hash, runtime pack hash, and policy hash. Verification rejects source-less setup-cache layer declarations that specify neither an image digest nor a flake lock hash, while older manifests deserialize with an empty setup-cache layer list. Coverage: focused `mvm-core` pack tests for serde default compatibility, stable key derivation, per-dimension invalidation, and invalid setup-cache metadata refusal. **Plan 213 remains open** on setup-cache hit/miss runtime wiring, remote builder-VM flake lock resolution, release publishing, default install-time preparation, runtime/builder fast paths, builder-VM artifact preparation, launch attestation/explainability, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-25 — Plan 213 Workstream G local flake lock identity implemented (branch `feat/plan-213-flake-lock-prepare`, stacked on `feat/plan-213-input-eligibility`).** `mvmctl prepare --resolve-flake-lock <flake>` now hashes local `flake.lock` files before cache-backed pack eligibility and carries the hash through `PackPrepareInput`, requiring cached pack manifests to match both the resolved flake reference and lock hash. Remote flake lock resolution fails closed until the builder-VM resolver path owns that work; non-flake inputs reject the flag. Coverage: focused `mvm-cli` prepare tests for local lock hashing, remote resolver refusal, and non-flake flag refusal; focused `mvm-core` cache tests for lock-hash match and mismatch behavior. **Plan 213 remains open** on remote builder-VM flake lock resolution, release publishing, default install-time preparation, runtime/builder fast paths, builder-VM artifact preparation, setup-cache runtime hit/miss wiring, launch attestation/explainability, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-25 — Plan 213 Workstream G prepare OCI digest resolution implemented (branch `feat/plan-213-input-eligibility`, stacked on `feat/plan-213-prepare-resolver`).** `mvmctl prepare --resolve-oci-digest <oci-image>` now resolves mutable OCI tags to Linux platform manifest digests before cache-backed pack eligibility checks, reusing the existing OCI reference parser/canonicalizer, manifest fetcher, platform selector, and registry bearer-token lookup. Digest-pinned OCI inputs canonicalize without network access; non-OCI inputs reject the flag; mutable OCI inputs remain fail-closed unless resolution is explicitly requested. Coverage: focused `mvm-cli` prepare tests for digest-pinned canonicalization, explicit mutable-tag resolution, default mutable-input behavior, resolver failure, non-OCI flag refusal, and top-level CLI parsing. **Plan 213 remains open** on release publishing, default install-time preparation, runtime/builder fast paths, flake lock resolution, builder-VM artifact preparation, setup-cache runtime hit/miss wiring, launch attestation/explainability, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-25 — Plan 213 Workstream C/D prepare resolver implemented (branch `feat/plan-213-prepare-resolver`, stacked on `feat/plan-213-pack-download`).** `mvmctl prepare <image-or-flake>` now resolves attested pack readiness from the local pack cache, verifies matching cached packs through the existing manifest/hash/signature/expiry/revocation/policy checks, and can install a local/HTTPS `--pack-source` through the existing quarantine-to-atomic-promotion cache path before resolving. `--dry-run` reports trust state, cached size when known, download requirement, builder-VM requirement, and expected fast-path eligibility without mutating the cache. Coverage: focused `mvm-core` resolver tests for ready, missing, mutable OCI, expired trust metadata, revoked signer, unsupported backend, and local-rebuild-required states; focused `mvm-cli` parser/helper tests for `mvm prepare`; and `audit_total_coverage` for the new top-level command. **Plan 213 remains open** on release publishing, default install-time preparation, runtime/builder fast paths, snapshot/warm derivation, OCI/flake lock resolution/building, launch attestation/explainability, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-25 — Plan 213 Workstream C cache install/download implemented (branch `feat/plan-213-pack-download`, stacked on `feat/plan-213-pack-promote`).** `mvmctl cache install-pack <SOURCE>` now reads local or HTTPS attested pack tar archives, refuses plain HTTP unless explicitly opted in, builds local policy from CLI arguments, reuses the existing trusted-publisher key store, accepts an optional local revocation JSON file, and installs through the existing quarantine-to-atomic-promotion pack cache path. Successful installs emit `CachePackInstall` without recording full source URLs or paths. Coverage: focused `mvm-core` trust-store adapter test and focused `mvm-cli` parser/helper tests for install-pack flags, required channel, source classification, local file reads, HTTP refusal, interrupted download propagation, bad policy hashes, and revocation schema/match behavior. **Plan 213 remains open** on release publishing, prepare UX download resolution, runtime/builder fast paths, OCI/flake preparation, launch attestation, revocation/mirror policy, metrics, and broader docs/migration workstreams.

**Additional 2026-06-24 — Plan 213 Workstream C archive promotion implemented (branch `feat/plan-213-pack-promote`, stacked on `feat/plan-213-pack-prune`).** `mvm_core::packs::cache` now installs pack tar archives by extracting into fresh quarantine directories, refusing unsafe archive paths, symlink/hardlink/special entries, duplicate files, missing manifests, and poisoned file contents before content-addressed promotion. Archive installs preserve the existing fail-closed verification ladder — staged manifest/file hashes/pack hash/signature/expiry/revocation/policy compatibility — then atomically promote and clean partial quarantine directories on failure. Coverage: focused `mvm-core` cache tests for archive atomic promotion, missing-manifest cleanup, duplicate-entry partial extraction cleanup, unsafe path refusal, symlink entry refusal, poisoned archive refusal, permission hardening, and the existing policy-aware prune path; full workspace `cargo test`, full workspace `cargo check`, and full workspace `cargo clippy --workspace --all-targets -- -D warnings`. **Plan 213 remains open** on release publishing, prepare UX download resolution, runtime/builder fast paths, OCI/flake preparation, launch attestation, revocation/mirror policy, metrics, and docs/migration workstreams.

**Additional 2026-06-24 — Plan 213 Workstream C cache prune implemented (branch `feat/plan-213-pack-prune`, stacked on `feat/plan-213-pack-status`).** `mvmctl cache prune` now routes attested pack cache deletion through `mvm_core::packs::cache` policy logic: default prune removes expired or structurally invalid pack entries, supports dry-run reporting, retains metadata-ready packs, and refuses to delete any pack protected by snapshot or warm-standby sidecars discovered under the checkpoint store and supervisor standby pool. Malformed protection sidecars fail closed. Coverage: focused `mvm-core` prune tests for expired/invalid deletion, ready-pack retention, dry-run behavior, protection-ref serde, and protected-pack retention; focused `mvm-cli` sidecar parsing/discovery tests; full workspace `cargo test`, full workspace `cargo check`, and full workspace `cargo clippy --workspace --all-targets -- -D warnings`. **Plan 213 remains open** on release publishing, prepare UX download resolution, runtime/builder fast paths, OCI/flake preparation, launch attestation, revocation/mirror policy, metrics, and docs/migration workstreams.

**Additional 2026-06-24 — Plan 213 Workstream C cache status implemented (branch `feat/plan-213-pack-status`, stacked on `feat/plan-213-pack-cache`).** `mvmctl cache status [--json]` now reports local attested pack cache inventory from the content-addressed index: readiness, kind, architecture, backend compatibility, channel, expiry, size, file count, last-used time, last-verified time, revocation-check state, and instant-launch eligibility state. The status path is read-only and does not consume packs; metadata-ready packs still report `requires_verification` until the launch path reruns trust/revocation/policy verification. Coverage: focused `mvm-core` status-entry tests for ready/expired/missing-index/malformed-index/hash-mismatch entries, focused `mvm-cli` parser and status-row tests, full workspace `cargo test`, full workspace `cargo check`, and full workspace `cargo clippy --workspace --all-targets -- -D warnings`. **Plan 213 remains open** on release publishing, prepare UX download resolution, runtime/builder fast paths, OCI/flake preparation, launch attestation, revocation/mirror policy, metrics, and docs/migration workstreams.

**Additional 2026-06-24 — Plan 213 Workstream C cache foundation implemented (branch `feat/plan-213-pack-cache`, stacked on `feat/plan-213-pack-schema`).** `mvm_core::packs::cache` now installs already-verified pack roots into a content-addressed cache under `mvm_cache_dir()/packs`, stages copied contents in per-pack quarantine directories before atomic promotion, writes restrictive permissions, records cache indexes with pack hash, kind, architecture, backend compatibility, channel, expiry, size, file count, last-used time, and last-verified time, and reverifies manifest contents, file hashes, aggregate pack hash, signatures, expiry, revocation status, and local policy before every cached use. Coverage: 7 focused cache tests for promotion, reverify-on-use, tampered source refusal, poisoned cache refusal, stale quarantine cleanup, permission hardening, and symlink source refusal, plus the CLI cache-recognition regression. **Plan 213 remains open** on release publishing, prepare UX download resolution, runtime/builder fast paths, OCI/flake preparation, launch attestation, revocation/mirror policy, metrics, and docs/migration workstreams.
...
- [ ] **PLAN 213** — Attested fast-first-boot packs · 🟢 in progress ([ADR-097](adrs/097-attested-downloadable-runtime-and-builder-packs.md)) — Workstream A pack schema/verifier core is implemented; Workstream B signed runtime/builder release pack publishing, fail-closed release asset verification, and pack rebuild-and-compare reproducibility are implemented; Workstream C local content-addressed pack cache, status, prune, archive promotion, `cache install-pack` local/HTTPS download path, and prepare download resolution are implemented; Workstream D has the cache-backed `mvm prepare`/`--dry-run` resolver slice, machine-run dry-run preparation reason diagnostics, tested human output fields, and explicit bootstrap runtime/builder pack preloading from operator-provided sources/policy; Workstream E has the core prepared-launch selection gate for verified ready runtime packs plus cached runtime artifact-path resolution; Workstream G has explicit prepare-time OCI tag-to-digest, flake-lock identity, and setup-cache identity/resolution slices; Workstream H has launch-attestation record/log core, launch-boundary emission, and `mvm explain`; Workstream I has explicit policy modes, channel signing-key pins, key-rotation windows, mirror identity checks, offline revocation freshness refusal, local/HTTPS revocation metadata fetching, and enterprise mirror source resolution. Remaining: runtime/builder fast paths, builder-VM artifact preparation, actual setup-cache artifact production/consumption, metrics, and broader docs/migration workstreams.
...
PLAN 213 — Attested fast-first-boot packs   🟢 IN PROGRESS (ADR-097)
  Plan: specs/plans/213-attested-fast-first-boot-packs.md
  [x] Workstream A — Pack identity, schema, and verification core.
      (`mvm_core::packs` strict typed runtime/builder/image-project manifests,
      verifier for schema/file hashes/pack hash/signatures/expiry/revocation/
      local policy, and tamper/refusal tests.)
  [~] Workstream C — Hardened local artifact cache.
      (Done: content-addressed cache under `mvm_cache_dir()/packs`, restrictive
      permissions, reverify-on-use, status, prune with protection refs, archive
      quarantine/atomic promotion, and `mvmctl cache install-pack` for local/HTTPS
      pack archives, including prepare UX download resolution. Remaining:
      runtime consumption wiring.)
  [x] Workstream B — Release production and artifact publishing.
      (Done: release workflow builds runtime and builder pack archives from
      release image outputs, emits manifests/provenance/checksums/SBOM refs,
      signs pack archives and metadata, and verifies the published asset set
      fail-closed, then rebuilds one representative runtime pack and one
      builder pack from source pins to compare archive hashes.)
  [~] Workstream D — Install and prepare UX.
      (Done: cache-backed `mvm prepare`/`--dry-run` resolver, local/HTTPS
      pack-source install before prepare resolution, stable reason detail and
      next-step text, and `machine run --dry-run` preparation diagnostics for
      mutable OCI, missing pack, private/local flake, remote flake builder
      preparation, and manifest sources; explicit `mvmctl bootstrap` runtime
      and builder pack preloading from operator-provided sources and policy
      metadata; tested human output fields for prepare and machine-run
      diagnostics. Remaining: install-time/default-pack preparation plus
      builder and snapshot derivation work that depends on Workstreams E/F/G.)
  [~] Workstream E — Runtime pack launch path.
      (Done: core prepared-launch selection gate on top of
      `PackCache::prepare_report`, returning a launchable verified pack only
      for ready fast-path reports and failing closed for cache misses,
      setup-cache misses, and malformed ready reports; cached runtime artifact
      path resolution by semantic output hash with fail-closed missing-payload
      and unsafe-path checks. Remaining: per-run CoW sandboxes, snapshot
      derivation, warm-claim/snapshot/cold launch binding, guest-agent command
      dispatch, secret-injection ordering tests, and live latency evidence.)
  [ ] Workstream F — Fast builder VM path.
  [~] Workstream G — OCI and flake artifact preparation.
      (Done: explicit prepare-time OCI tag-to-digest resolution before
      cache-backed pack eligibility, local flake lock hashing, setup-cache layer
      identity keying, and prepare-time setup-cache hit/miss resolution.
      Remaining: launch admission wiring plus builder-VM preparation for
      unpublished/private artifacts and actual setup-cache artifact
      production/consumption.)
  [x] Workstream H — Launch attestation and explainability.
      (Done: typed launch records, local hash-chained log, launch-boundary
      emission for success/failure/refusal/fallback/interruption, tamper
      detection, and `mvmctl explain <run-id>` human/JSON output.)
  [x] Workstream I — Revocation, mirrors, and offline policy.
      (Done: typed policy modes for online default, offline pinned,
      mirror-only, and local-rebuild-required operation; per-channel signing-key
      pins; verifier-level key-rotation windows; mirror identity checks;
      offline revocation freshness refusal; local/HTTPS revocation metadata
      fetching; relative enterprise mirror source resolution for pack and
      revocation metadata; prepare/cache/bootstrap CLI policy inputs; and
      revocation/mirror docs.)
  [ ] Workstream J — Metrics, evidence, and rollout.
  [~] Workstream K — Documentation and migration.
      (Done: CLI reference entries for prepare, cache status/prune, and
      explain, plus explicit install/bootstrap policy-mode, channel-key, and
      mirror identity docs. Remaining: architecture, troubleshooting,
      revocation/offline/mirror guides, and broader migration docs.)
