...
**Additional 2026-07-06 — Plan 226 R1P1 (Vz workload/dev/builder deselection) LANDED on branch `worktree-plan-226-r1p1-vz-deletion` (Option B split).** Makes the in-house HVF VMM the default and sole *selectable* macOS backend; removes Vz from every selection path. Commits: T1 checkpoint `vm_full` descoped + `fs_quick` kept (7328cee4); T2 macOS-26 dev VM → in-house HVF, `dev_vz` lifecycle removed (db28be9b); T3 one-shot `VzBuilderVm` selection removed, libkrun fallback kept (82f71fad); T4 `AnyBackend::Vz`/catalog/candidates removed, `vz.rs` retained (84ec085e); T5 resolve.rs Vz test assertions dropped (6878571a); T6 verified macOS `check`/`clippy` + `check-linux` all CLEAN, plus an incidental unblock of a pre-existing Plan 224/225 gap (`SubprocessBackend` missing `MvmClient::reconfigure_machine`) (f3b250a5). **Vz is now unselectable** (`AnyBackend::Vz`/`DevBackend::Vz`/`BuilderBackendChoice::Vz` all gone; `--hypervisor vz`/`--builder vz` fall back). **KEY:** `create_linux_env()` still returns `VzDevEnv` on macOS-26, so the persistent Vz builder VM remains the live build substrate — the Vz code files (`vz.rs`, `vz_control.rs`, `vz_objc.rs`, `vz_builder.rs`, `mvm-vz-supervisor`) are RETAINED. ADR-098 ratified workload-scoped (partial). `machine checkpoint/fork` full-VM mode temporarily unsupported (→ 226-R1E HVF SaveRestore). **NOT merged; no v0.17.0 bump this round.** NEXT: write + execute **226-R1P1b** (flip `create_linux_env`→libkrun persistent builder, validated with a real build, then delete the Vz files and cut v0.17.0); then 226-R1P2 (libkrun→vsock + gvproxy), 226-R1E, 226-R2. Aligns with Plan 227 WS-E ("hvf is the only workload backend with zero snapshot support; vz has SaveRestore but is sunset").

**Additional 2026-07-06 — Plan 227 (instant-resume sandboxes over a vsock-only auditable data plane) OPENED, docs-only.** A five-axis audit of main @ `db659875` (snapshot/pause/resume per backend; warm-start machinery; cold-boot critical path; MvmClient facade coverage; nix-run + kernel + network-surface inventory) produced `specs/plans/227-instant-resume-vsock-only-sandboxes.md`. Headline findings: the new macOS-26 default backend (hvf) is the only workload backend with **zero snapshot support** (FC has pause/resume/vm_full/fork/`resume --warm` ~0.5 s; vz has SaveRestore but is sunset); pause/resume/checkpoint/restore/fork/warm ops are **entirely absent from the MvmClient facade** (CLI-only `vm` verbs; CLI itself bypasses the facade — Plan 216 S2 deferred); the hvf workload path is already **vsock-pure** (no NIC device) while libkrun/FC workloads and ALL builder VMs still ride gvproxy/passt/TAP (Plan 214 S4/S6/S7 designed, unmerged); `--flake`-built machines boot on hvf with the shared slim kernel; the remaining kernel strips (VIRTIO_NET, builder netfilter) are blocked on the vsock cutover, after which builder+workload converge on ONE kernel config. Plan 227 workstreams: WS-A in-house agent no-reply fix (prereq), WS-B vsock-only cutover incl. builder-as-policy-profile (allowlisted + fetch-audited nix egress → supply-chain manifest), WS-C no-NIC CI claim, WS-D single-kernel convergence, WS-E hvf snapshot/restore/fork + FC UFFD tail + warm pools, WS-F facade completeness (lifecycle ops on the trait, CLI routes through facade, conformance fixtures), WS-G PII masking on the gating endpoint, WS-H docs/claims. Owner directive captured: networking is never enabled — all data in/out crosses the audited vsock seam (secrets substitution + PII masking hang off it); no competitor names in any artifact.
