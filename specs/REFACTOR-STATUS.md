...
- [ ] **PLAN 209** — Per-VM host-process model convergence · 📋 PLANNED ([ADR-094](adrs/094-vm-host-process-model-convergence.md)) — converge all workload backends on the split *VMM-process + thin shared bridge sidecar* model: fold `mvm-firecracker-bridge` + `mvm-vz-drainer` into one `mvm-bridge` binary (uniform jailer-lite confinement — closes the vz-drainer gap), and strip the bridge out of `mvm-libkrun-supervisor` so it is a thin krun launcher whose backend spawns the shared sidecar via socketpair + `AttachedBridgeGuard` (the pattern FC/vz already use). 4 per-VM host bins → 3; deletes the factory-closure + thread-reaped-by-`exit()` complexity; `spawn_bridge_thread` enforcement + claims 10/12/13 unchanged. Does **not** drop Firecracker. Tasks 1–6 specced; fd-passing (Task 3) is the crux; FC acceptance live-KVM-gated.
