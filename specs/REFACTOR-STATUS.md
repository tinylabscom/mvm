...
PLAN 193 — rvproxy network substrate (replace gvproxy/passt)  🟡 WS-1 proven; WS-2.2d mvm-side native parity landed; parity gate required; terminator implementation/splice deletion remain
  ...
  WS-2.2d (mvm-side, #1009): native rvproxy audit feed + native-enforcement parity, no splice.
    ...
    - remaining: land/wire rvproxy macOS transparent terminator
...
4. Continue Plan 193 only after rvproxy cross-repo slices are ready: land/wire rvproxy macOS transparent terminator support, then delete the splice and remove Plan-141 hooks.
