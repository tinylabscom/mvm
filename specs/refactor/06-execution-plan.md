...
- [~] 1i `mvm-cli`: delete direct reaches into runtime internals; route through `mvm-client`. _(Was: CLI used `mvm_client` in 0 files, reached `mvm_runtime` across 59. **Slice 1 landed** `250b42219`+`d830af307`: `mvmctl ps` routes through `LocalBackend::list_machines` (host-wide registry∪backend view), dropped the `mvm_runtime::vm::name_registry` reach; 7555/7555, reviewed. Remaining slices (design doc §sequence): down/stop, remove, pause/resume, checkpoint/restore, up/run; then the optional `check-cli-runtime-surface` lint.)_
