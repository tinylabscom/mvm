...
- [~] 1i `mvm-cli`: delete direct reaches into runtime internals; route through `mvm-client`. _(Was: CLI used `mvm_client` in 0 files, reached `mvm_runtime` across 59. **Slice 1** `250b42219`+`d830af307`: `mvmctl ps` → `list_machines`. **Slice 2** `af4456123`+`690e62f6a`: `mvmctl down` → `stop_machine` (facade owns owning-backend resolution + registry deregister; CLI keeps readiness/audit; review caught + fixed a Critical — `down --all` must reap crashed dead-with-pid VMs, so it iterates the backend-observed `list_stop_targets` set, not the registry-folded listing). 7556/7556, reviewed. Remaining slices (design doc §sequence): remove, pause/resume, checkpoint/restore, up/run; then the optional `check-cli-runtime-surface` lint.)_
