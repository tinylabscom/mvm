...
The target crate map, binary model, feature model, directory model, backend/egress model, and top-level repo layout. This is the _design target_ — see [07-progress-and-decisions.md](07-progress-and-decisions.md) for how far execution has gotten against it.
...
Kept as-is: `crates/deps/libkrun-sys` (FFI), `xtask`. **Dropped/folded:** `mvm-ext4`, `mvm-network`, `mvm-verify`, `mvm-guest-helpers`, `mvm-vm-host`, `mvm-host-services-ffi` (folded into `mvm-sdk`), `mvm-mcp` (folded into `mvmctl serve` behind an `AgentProtocol` trait — MCP now, ACP later, no per-protocol crate; see [06-execution-plan.md](06-execution-plan.md) WS7), the orphan Swift supervisor dir, dead deps (`colored`, `names`, `hickory-server`, stale `mvm-egress-proxy` path). (The `qemu` backend was _not_ dropped — kept as an opt-in Tier-2 dev substrate; see [07-progress-and-decisions.md](07-progress-and-decisions.md).)
...
Note: the crate map and the deviations recorded in [07-progress-and-decisions.md](07-progress-and-decisions.md) are now aligned on `mvm-host-services-ffi`; it is folded into `mvm-sdk` and the standalone crate is removed.
