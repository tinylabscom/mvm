...
- **1h/1i** `mvm-client` facade completion + `mvm-cli` routed entirely through it — IN FLIGHT. Design + scope boundary + contract-faithfulness rule in [13-mvm-client-facade.md](13-mvm-client-facade.md); the pause/resume and machine-boot slice designs are [14-facade-pause-resume.md](14-facade-pause-resume.md) / [15-facade-boot-admission.md](15-facade-boot-admission.md). The facade existed but the CLI bypassed it entirely (0 uses, 59-file `mvm_runtime` reach). Landed slices, each routing a command group through `LocalBackend` and ending green:
  ...
  Doc 13 categorized the full remaining boundary: **category A** (lifecycle, still to migrate) vs **B–E** (legit-direct, allowlisted). Remaining category-A slices, all test-gated: **`up`/machine-boot** — the capstone (`up.rs` is 4107 lines of claim-8 admission logic, not a thin command; per doc 15 it **must** be `nextest`-verified because it touches the signed-plan admission gate, never clippy-only); **`sandbox`**; the **`machine`** command group's non-boot registry reads; `readiness.rs`. checkpoint/snapshot deliberately **stay direct** (store surfaces, allowlisted). The **`check-cli-runtime-surface`** lint lands LAST, once category A is drained, banning `mvm_runtime::vm::name_registry` + `AnyBackend` in mvm-cli with B–E allowlisted.
