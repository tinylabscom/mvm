...
- **1h/1i** `mvm-client` facade completion + `mvm-cli` routed entirely through it — STARTED. Design + scope boundary + contract-faithfulness rule in [13-mvm-client-facade.md](13-mvm-client-facade.md). The facade existed but the CLI bypassed it entirely (0 uses, 59-file `mvm_runtime` reach). Slice 1 (`ps`, `250b42219`+`d830af307`) proves the pattern: route through `LocalBackend::list_machines`, enrich the shared contract to stay faithful (`MachineStatus::Paused`, `MachineState.status_detail`), and deliberately shift the CLI's `--json` from internal `VmStatus` to contract types. Remaining slices are the rest of the vm-lifecycle command group (down/stop, remove, pause/resume, checkpoint, up/run), each an independent task ending green.
