...
| I5 | Resolved: self-update verifies in-process, and a fresh install authenticates the selected archive against an installer-carried or independently supplied SHA-256 before using its `mvmctl` to verify the mandatory tag-pinned bundle | `install.sh`, `crates/mvm-cli/src/update.rs` |
...
- [x] Have `install.sh` prefer `mvmctl env verify-release` once a binary exists
      ...
      `mvmctl` as a temporary verifier, and require its tag-pinned bundle. There
...
- [x] Close the fresh-install residual without a new dependency: the release
      deploy step bakes each published target archive's SHA-256 into the stable
      installer, which authenticates the temporary in-archive verifier before
      its mandatory Sigstore check.
