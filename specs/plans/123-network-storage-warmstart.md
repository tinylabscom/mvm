...
**Phase C — the `PostRestore` sender gap — CLOSED (prerequisite built).** The guest has always handled `GuestRequest::PostRestore` (SIGUSR1 → PID 1 → remount config/secrets drives + restart services), but nothing on the host sent it. Now wired: `mvmctl resume` calls `instance_snapshot::signal_post_restore` after `verify_and_resume`, routing `GuestRequest::PostRestore` through the backend-agnostic `vsock_transport::for_vm` dispatcher (`VsockPostRestoreSignal`); an unacknowledged signal fails closed (the VM is up but degraded → re-run resume). Testable seam (`PostRestoreSignal` trait + mock; the orchestration is unit-tested without a guest). **What remains for C2** is the *VMGenID-carrying* variant: have `PostRestore` carry a `GenerationToken` and call `GenIdReseeder::on_genid` in the guest handler (122 D shipped that substrate) — still live-KVM-gated. So the sender exists; the reseed-on-resume *payload delivery* is the next C2 increment, no longer blocked on a missing sender.
...
- [x] **Step 1:** Author `mvm-network` and the trait:
  ```rust
...
- [x] **Step 2 (Linux LUKS2 arm) — DEFERRED (see follow-ups):** block-level LUKS2 over a loop device. Un-buildable + un-verifiable on a macOS dev host (needs Linux + root + loop devices; `cargo zigbuild`/zig 0.16 can't even compile the crate here). Land on Linux CI per the `mvm_core::rotate_luks_slot` precedent (direct `cryptsetup`, `MVM_LIVE_LUKS=1`-gated).
...
> **Status (2026-06-08):** **C1 + C4 landed; the `PostRestore` sender prerequisite is in (#734).** C1 shipped the capability enum + per-backend disposition. **C4** (this slice) added the warm-start *operation* seam: a typed `WarmStartError` (ADR-053, recovery hint) + `SnapshotCapability::{label,satisfies}` + a fail-closed `VmBackend::warm_start` default (over-request → `Unsupported`, never a silent cold boot), libkrun's disk-only override (boots a private writable clone of the golden rootfs via Phase B3 `SnapshotUpper::materialize_image`, base immutable), and the `mvmctl doctor` warm-start matrix (per-backend tier + Linux NBD/HugeTLB substrate probe; N/A off Linux). All pure logic is unit-tested locally; the libkrun boot path it delegates to is unchanged (mvm-backend run-tests ride Linux CI). **C2/C3 remain deferred** — gated lanes this dev host can't run (live-KVM for Firecracker UFFD/NBD; macOS-26 for Vz save/restore). No CLI verb yet *invokes* `warm_start`, so a live disk-snapshot resume + agent_ping rides the snapshot-RPC wiring that lands with C2/C3.
...
- [x] **Step 1:** libkrun has no memory snapshot — warm-start is a fast re-boot from the overlay/rootfs disk snapshot (Phase B3). Implement that path; the capability stays `DiskOnly`; a request for live-memory returns the typed unsupported error with the recovery hint.
- [x] **Step 2:** `doctor` reports the per-backend warm-start capability + probes the Linux substrate (NBD module loaded, HugeTLB reservation). Failing test on the doctor lines. Commit.

## Acceptance

- [x] `mvm-network` exists (17th crate); `NetworkProvider` provisions **TAP (L1) + gvproxy/passt (L3-A)** behind the trait; egress default-deny is **enforced behind the `EgressEnforcer` trait (A2)** on Firecracker and via the per-tenant `PlanFlowPolicy` on the libkrun gateway-bridge; **DNS sink-hole (A4) + flow audit (Plan 141)**; the egress proxy carries the substitution + leak-scan seams (A3, no-op until 129). **Phase A is complete** — L3 slice B landed (#664) and the L2 microvm_nix question is resolved (QEMU is a dev/test-only Tier-2 backend, no enforcement; ADR-002 + CLAUDE.md).
...
- [x] **Phase A claims-gated lift (A1/L1, A2, A3, A4, L3-A) — DONE** (#626/#632/#633/#635/#637/#639/#642/#643/#644). Each move carried its claim witness; `xtask check-claim-catalog` stayed green. What's left below is *not* lift — it's per-tenant wiring + two decisions:
  - [x] **L3 slice B — DONE (#664).** `resolve_networking_mode` was made macOS-safe (`MVM_NETWORKING=passt` on macOS → gvproxy + warn; passt is Linux-only) and the workload site (`mvm_backend::libkrun::build_supervisor_config`) re-pointed from `cfg!(target_os = "macos")` to it, so workloads honor `MVM_NETWORKING` consistently with the builder. macOS behaviour unchanged. (The deeper `NetworkProvider`/`NetHandle` impedance — a config-production result vs. the stringly `tag` round-trip — is left as-is; the selection-site reconciliation was the load-bearing part.)
  - [x] **A2/A4 per-tenant enforce wiring — DONE.** `PlanFlowPolicy::from_effective` (`gateway_bridge.rs`) maps the admitted plan's resolved `EffectivePolicy` onto a deny-by-default `FlowPolicy`: an egress flow opens only when the policy admits some egress (an L4 allow rule, an egress allow-list entry, or `mode == open`); deny-all drops the egress flow at open (ingress, a reply, always opens). `run_bridge_inner` derives it from the *same* `resolve()` that feeds `build_egress_scan`, so the coarse flow gate never drops a flow the packet scan would admit, and it replaces the supervisor bin's `AllowAll` whenever a bundle resolves (no-bundle → `AllowAll` fallback, still under the always-on mandatory-deny + placeholder-leak scans). This is the libkrun analogue of the Firecracker `install_default_deny` — both backends derive default-deny from the same policy through their respective seams. `ObserverWiring.scan = DnsSinkholeScan(allow_list)` was already threaded per-tenant (Slice 3). The `MVM_ACK_UNRESTRICTED_NETWORK` escape lives at admission (`up.rs`), before the supervisor spawns. Tests: 4 `plan_flow_policy_*` unit tests + 2 live-bridge (`run_libkrun_gvproxy_bridge`, no VM) deny/allow tests.
  - [x] **L2 `microvm_nix` egress policy — DECIDED: option (b), no enforcement (2026-06-08).** QEMU/microvm_nix is a `mvm`-only dev/test backend (Plan 166), never reached by `mvmd`, so it carries no untrusted multi-tenant workload and there is no admission flow to source a policy from — forcing `deny_all()` onto its start path would just break all egress for a local dev loop. Documented as Tier-2 dev/test (no claim-10 egress enforcement) in ADR-002 §"Per-backend tier matrix" (claim-10 egress-enforcement note + the QEMU row) and CLAUDE.md §"Key Design Decisions". Option (a) — threading `network_policy` through `VmStartConfig` for uniform whole-fleet claim-10 — is a deliberate *future* decision if QEMU is ever promoted to a workload-bearing tier, not a Phase A gap.
...
- [x] **B2 Linux LUKS2 arm — DONE.** `crates/mvm-storage/src/encrypted_linux.rs` — block-level `EncryptedStorage` (`cryptsetup luksFormat`/`luksOpen` + `mkfs.ext4`/`mount` over a loop-backed image), gated `#![cfg(target_os = "linux")]`, mounted on `lib.rs` as `pub mod encrypted` via `#[path]` so the type name + `new(root, key)` constructor match the non-Linux arm (callers never branch on `target_os`). Per-volume size from a new `VolumeSpec::with_size_mib` (Optional → no call-site churn; non-Linux/Local arms ignore it). DEK staged in a 0600 tempfile for `cryptsetup --key-file --keyfile-size 32` (byte-exact, unlike newline-truncating stdin). Tests: a runs-anywhere fail-closed test (`attach` on a non-LUKS backing → Err) + `MVM_LIVE_LUKS=1`-gated live tests (ciphertext-at-rest roundtrip + wrong-key-fails-to-open) — **verified green on a Linux aarch64 VM** (real cryptsetup + loop + root, cross-built via `cargo-zigbuild`). Integrity scope: plain LUKS2 authenticates the keyslot (confidentiality), not data blocks; block-level tamper *detection* stays dm-verity's job (claim 3). **Still deferred:** dedup the cryptsetup shell-out with `mvm/src/security/encryption.rs` + `mvm_core::rotate_luks_slot` (three call sites now; a shared helper is a follow-up).
- [ ] **C4 warm-start CLI/RPC wiring** — `VmBackend::warm_start` exists + libkrun implements disk-only, but no CLI verb invokes it yet (parallels C1's "snapshot RPC not wired"). The user-facing `mvmctl resume`/warm-start verb + the live disk-snapshot boot → `examples/agent_ping` validation land with the C2/C3 snapshot RPC.
- [ ] **C4 reflink clone** — `SnapshotUpper::materialize_image` is a plain byte copy today; on a reflink-capable FS (APFS `clonefile`, Linux `FICLONE`) it should be a CoW clone so disk-only warm-start stays O(1). COW *semantics* (immutable base) already hold.
- [ ] Cloud-Hypervisor snapshot parity (if CH stays a backend).
- [x] Soften the gap-analysis "live-memory resume" line to the per-backend matrix (Firecracker + Vz live-memory; libkrun disk-only).
