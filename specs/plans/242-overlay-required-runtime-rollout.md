# Plan 242 — Overlay-required guest runtime rollout
...
**Additional 2026-07-11 — Plan 242 implementation remains complete, and the current worktree candidate has been revalidated end to end.** The runtime-overlay contract itself remains complete on the branch: the workload-side required-overlay path is proven on every admitted block-backed workload backend that consumes `RequiredOverlay` today, and the builder side remains production-safe because Linux-native auto-detect routes to qemu while explicit steady-state Linux rootfs-backed libkrun builder attempts fail closed with `BuilderVmError::LibkrunUnavailable(...)`. The release surface is explicit too: the runtime-overlay assets are versioned release artifacts under `~/.cache/mvm/runtime-overlay/<version>/<arch>/`, the release workflow publishes the matching `.ext4`, `.verity`, `.roothash`, `.VERSION`, and checksum files, and the operator docs/release note state the real rollout contract for fresh starts, stopped VMs, already-running VMs, and rollback. The current closeout slice also fixes the exact-candidate witnesses: the SDK/fixture receipt posture tests now match the vsock-only/libkrun contract, the `mvm-build` library tests are isolated from shared env/cache races, `cargo check --workspace --offline`, `cargo clippy --workspace --all-targets --offline -- -D warnings`, and `cargo test --workspace --offline` are green on the host execution boundary, and the Linux/KVM `cargo test -p mvm-build --lib --offline` witness remains green on `88.99.197.234` from a fresh source snapshot. This note tracks implementation and validation state only; branch/PR synchronization remains an external release step, not part of the technical contract described here.
