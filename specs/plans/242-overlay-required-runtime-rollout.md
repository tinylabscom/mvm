...
**Additional 2026-07-10 — Plan 242 implementation is complete, and the refreshed ship candidate is now pushed and revalidated at exact head `29fb2c2466316201bc34f81567921a4b8856eb38`.** The runtime-overlay contract itself remains complete on the branch: the workload-side required-overlay path is proven on every admitted block-backed workload backend that consumes `RequiredOverlay` today, and the builder side remains production-safe because Linux-native auto-detect routes to qemu while explicit steady-state Linux rootfs-backed libkrun builder attempts fail closed with `BuilderVmError::LibkrunUnavailable(...)`. The release surface is explicit too: the runtime-overlay assets are versioned release artifacts under `~/.cache/mvm/runtime-overlay/<version>/<arch>/`, the release workflow publishes the matching `.ext4`, `.verity`, `.roothash`, `.VERSION`, and checksum files, and the operator docs/release note state the real rollout contract for fresh starts, stopped VMs, already-running VMs, and rollback. The exact-current candidate has now been refreshed end to end after the latest closeout fixes: the OCI cache-rematerialization test fixtures recreate the verity sidecars they rely on, the gateway-bridge export test fails on bridge-task errors instead of ignoring them, `cargo check --workspace --offline`, `cargo clippy --workspace --all-targets --offline -- -D warnings`, and `cargo test --workspace --offline` are green on the merged head, and the Linux-native builder witnesses `builder_backend_select::tests::auto_detect_default_for_linux_native_picks_qemu` plus `libkrun_builder::tests::linux_native_rootfs_builder_support_guard_matches_platforms` are green on `88.99.197.234` from an exact source snapshot of that head. What remains external is the normal reviewed integration path: PR `#1613` now points at this exact head and is mergeable, but GitHub correctly blocks merge until the fresh required checks finish. Treat this branch state as a validated ship candidate awaiting CI completion, not as already shipped production state.
