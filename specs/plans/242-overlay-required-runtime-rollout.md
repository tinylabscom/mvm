...
**Additional 2026-07-11 — Plan 242 implementation remains complete, and the refreshed ship candidate is now pushed and revalidated at exact head `96900bc8ebf74d19afd4e3ff9bb10d619561df0f`.** The runtime-overlay contract itself remains complete on the branch: the workload-side required-overlay path is proven on every admitted block-backed workload backend that consumes `RequiredOverlay` today, and the builder side remains production-safe because Linux-native auto-detect routes to qemu while explicit steady-state Linux rootfs-backed libkrun builder attempts fail closed with `BuilderVmError::LibkrunUnavailable(...)`. The release surface is explicit too: the runtime-overlay assets are versioned release artifacts under `~/.cache/mvm/runtime-overlay/<version>/<arch>/`, the release workflow publishes the matching `.ext4`, `.verity`, `.roothash`, `.VERSION`, and checksum files, and the operator docs/release note state the real rollout contract for fresh starts, stopped VMs, already-running VMs, and rollback. The exact-current candidate has now been refreshed end to end after the latest closeout fixes: the SDK/fixture receipt posture tests now match the current vsock-only/libkrun contract, the `mvm-build` library tests are isolated from shared env/cache races, `cargo check --workspace --offline`, `cargo clippy --workspace --all-targets --offline -- -D warnings`, and `cargo test --workspace --offline` are green on this head on the host execution boundary, and `cargo test -p mvm-build --lib --offline` is green on `88.99.197.234` from a fresh source snapshot of this exact head. What remains external is the normal reviewed integration path: PR `#1613` now points at this exact head, is mergeable, and is only blocked on the fresh required GitHub checks completing. Treat this branch state as a validated ship candidate awaiting CI completion, not as already shipped production state.
