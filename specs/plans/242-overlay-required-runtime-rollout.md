...
**Additional 2026-07-10 — Plan 242 implementation is complete, but the ship candidate still needs refreshed branch/PR proof.** The runtime-overlay contract itself remains complete on the local branch: the workload-side required-overlay path is proven on every admitted block-backed workload backend that consumes `RequiredOverlay` today, and the builder side remains production-safe because Linux-native auto-detect routes to qemu while explicit steady-state Linux rootfs-backed libkrun builder attempts fail closed with `BuilderVmError::LibkrunUnavailable(...)`. The release surface is now explicit too: the runtime-overlay assets are versioned release artifacts under `~/.cache/mvm/runtime-overlay/<version>/<arch>/`, the release workflow publishes the matching `.ext4`, `.verity`, `.roothash`, `.VERSION`, and checksum files, and the operator docs/release note now state the real rollout contract for fresh starts, stopped VMs, already-running VMs, and rollback. What is still in progress in this worktree is the exact-current ship-candidate refresh after the latest closeout fixes: the OCI cache-rematerialization test fixtures now recreate the verity sidecars they rely on, the gateway-bridge export test now fails on bridge-task errors instead of ignoring them, and the branch/docs must only claim the narrower truth until the refreshed head is pushed and rerun through PR checks. Treat this branch state as an implemented local ship candidate, not as shipped or currently mergeable production state.
