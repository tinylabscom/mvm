...
- [x] **Step 2:** On seal, compute `sha256` over the sealed bytes and sign `(sha256 ‖ epoch)`. HMAC kept as cheap local integrity; the Ed25519 signature is the authentication gate. **Deviation:** signed by a dedicated `snapshot-signer.ed25519`, NOT the plan host-signer — that key lives behind the claim-8 subprocess moat and snapshot sealing runs host-side in mvm-backend; loading it there would weaken the moat.
