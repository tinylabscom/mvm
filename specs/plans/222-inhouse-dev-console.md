# Plan 222 — dev-only interactive `-it` console on the hvf VMM
...
Once `-it` hvf is live-proven, flip `auto_select` (`backend.rs:566`) + builder `auto_detect_default_for` (`builder_backend_select.rs:91`) to hvf, collapse `hvf`→runner, then delete Vz (`vz.rs`, `vz_control.rs`, `mvm-vz-supervisor`, `is_vz_default_tier`, `vz_builder.rs`, Vz cases) — coordinating the `mvmctl::runtime::*` deletion with mvmd's build.
