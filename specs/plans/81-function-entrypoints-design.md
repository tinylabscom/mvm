...
> **Post-flip note (2026-05-06):** the embedded ADR-0009 + Plan-0003
> drafts below treat `mkPythonFunctionService` / `mkNodeFunctionService`
> as mvmforge-side factories. ADR-0010 §3 was later amended back to
> Option A, and the factories themselves landed in **this** repo at
> [`nix/lib/factories/`](../../nix/lib/factories/) (exposed via
> `mvm.lib.<system>` in [`nix/flake.nix`](../../nix/flake.nix)).
> Quoted text below is preserved as the historical proposal.
...
This is mostly Nix-factory work; the factories landed in this repo
post-Option-A flip (`nix/lib/factories/`), and mvm still exposes the
tier-loading mechanism (already there per W2.4).

#### M16. Wrapper distribution / supply chain

The wrapper itself ships in this repo post-Option-A flip
(`nix/wrappers/` reference templates + `nix/lib/factories/` Nix
factories) and is baked into the rootfs at `mvmforge compile` time.
Two questions:

1. **How is the user sure the wrapper is what the substrate says it
   is?** The wrapper sources in `nix/wrappers/` are reviewed in this
   repo and the factories produce them deterministically. mvm's W5.3
   reproducibility guarantee covers them; mvmforge's downstream build
