...
| Boot fast from a pre-built image | yes | Templates + snapshots — `restore_snapshot` in `crates/mvm/src/vm/instance/snapshot.rs:220` |
| Stop / teardown a VM | yes | `cmd_down` |
| Per-instance ext4 data disk + file injection into a drive | yes | `ensure_data_disk()` and `drive_file_inject_commands()` in `crates/mvm/src/vm/instance/disk.rs` |
...
4. Boot via `backend.start(&VmStartConfig)`. Prefer snapshot restore when the template has one (`template_snapshot_info` in `crates/mvm/src/vm/template/lifecycle.rs:325`); fall back to cold boot.
...
- `crates/mvm/src/vm/image.rs` and `crates/mvm/src/vm/instance/disk.rs` — small helper to build a read-only ext4 image from a host directory (tarball populate then mark RO). Likely reuses what `drive_file_inject_commands()` already does.
