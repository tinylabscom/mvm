...
- [x] **HVF guest `BROKER_PORT` bridge.** Guest-originated `host.audit.v1` now reaches the broker on HVF, at parity with libkrun/vz. Threaded the per-VM broker UDS (the `hvf-broker.sock` the host-agent daemon already binds) through `HvfSupervisorConfig.broker_socket` → the supervisor → `HostChannels`/`RunInputs` → the in-VMM vsock device, and added a `BROKER_PORT` route in the dispatcher that relays the stream byte-for-byte to that UDS — reusing the generic guest-stream↔UDS relay the egress path uses (fail-closed with no socket, sharing the run-loop heartbeat). Guest side unchanged. Live-verified: an HVF machine binds + accepts on `hvf-broker.sock` with the device wired and no relay errors. Files: `crates/mvm-backend/src/vmm/vsock.rs`, `hvf_backend.rs`, `hvf/kernel_boot.rs`, `driver/hvf.rs`, `builder_runner/inject.rs`, `crates/mvm-build/src/hvf_supervisor.rs`, `crates/mvm-vm-host/src/bin/mvm-hvf-supervisor.rs`.
- [x] **HVF supervisor teardown reliability — verified not reproducing.** A live HVF repro (isolated `MVM_DATA_DIR`) shows `machine stop --yes` cleanly reaps both the `mvm-hvf-supervisor` and the per-VM substitution endpoint — no orphan. The prior symptom was fixed by the reconcile-reaping fix (#1521); the note predated it. Since restart = stop+start and stop is clean, phase-C restart is unaffected.

### Adjacent lifecycle bugs found during the teardown repro (separate tracks)

- [x] **OCI cache self-heal.** `machine run --image` failed hard when a cached image's `rootfs.ext4` had vanished but its unpacked layers survived; it now re-materializes network-free (the same seal `image pull` performs). Live-verified. File: `crates/mvm-cli/src/commands/image/mod.rs`.
- [ ] **`machine run -d -- <cmd>` ignores `-d`.** The argv branch of `run_persistent_post_start` foreground-streams the command via `console::run` even with `-d`, so a detached run hangs. Needs a design decision on detached-workload delivery (host-side output-draining holder vs. a guest-agent detached-workload verb) — the guest exec is connection-scoped and output-cap-kills an unattended workload, so it isn't a one-line fix.
