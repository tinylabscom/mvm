...
- [ ] **Step 1 — rename the concept, not the behavior.** Replace or wrap
...
- [ ] **Step 2 — add descriptor-facing helpers.** Export a small registry API:
...
- [ ] **Step 3 — add human-facing metadata while staying factual.** Add a stable
...
- [ ] **Step 4 — keep the macro narrow.** `backend_catalog!` may expand to build
      the descriptor table and related wiring, but it must remain one flat
      declarative table, not a layered DSL.
- [ ] **Step 5 — green.** `cargo test -p mvm-backend catalog`,
      `cargo test -p mvm-backend backend`,
      `cargo clippy -p mvm-backend --all-targets -- -D warnings`.
- [ ] **Step 6 — commit.** `git commit -m "refactor(backend): promote catalog into descriptor registry"`
...
- [ ] **Step 4 — green.** `cargo test -p mvm-backend backend`,
      `cargo clippy -p mvm-backend --all-targets -- -D warnings`.
- [ ] **Step 5 — commit.** `git commit -m "refactor(backend): add descriptor-driven backend constructors"`
...
- [ ] **Step 1 — classify call sites honestly.** Split callers into:
      1. descriptor-only,
      2. `VmBackend`-only,
      3. genuinely enum-specific.
      Record the remaining enum-specific sites in a short closeout note.
- [ ] **Step 2 — migrate the easy generic sites.** Where a caller only invokes
...
- [ ] **Step 3 — keep backend-specific code explicit.** Leave call sites that
      ...
      or a second trait.
- [ ] **Step 4 — green.** Run the targeted CLI/runtime tests touched by the
      migration, then `cargo clippy -p mvm-cli -p mvm-backend --all-targets -- -D warnings`.
- [ ] **Step 5 — commit.** `git commit -m "refactor(cli): migrate generic backend callers onto descriptor registry"`
...
- [ ] **Step 3 — keep one obvious escape hatch.** Retain `as_vm_backend()` for
      callers that already have an enum and only need the shared trait methods.
- [ ] **Step 4 — add a boundary test.** Add or update a test that asserts the
      enum still exists for backend-specific flows while descriptor-only
      consumers no longer need to instantiate it.
- [ ] **Step 5 — green.** `cargo test -p mvm-backend`,
      `cargo clippy -p mvm-backend --all-targets -- -D warnings`.
- [ ] **Step 6 — commit.** `git commit -m "refactor(backend): narrow AnyBackend to enum-specific behavior"`
...
- [ ] **Step 1 — remove stale “future registry” wording.** Update the supervisor
      ...
      registry.
- [ ] **Step 2 — document the ownership split plainly.** In the architecture
      ...
      operations.
- [ ] **Step 3 — note the non-goal explicitly.** Document that the registry is
      compile-time/static today and is not a runtime plugin system.
- [ ] **Step 4 — commit.** `git commit -m "docs(architecture): describe backend descriptor registry"`
...
- [ ] `cargo clippy --workspace --all-targets -- -D warnings` green in the required environment(s).
- [ ] `public/src/content/docs/reference/architecture.md` matches the code.
...
- [ ] Record, in the closing update to this file, that the shipped end state is:
