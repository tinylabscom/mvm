...
## Status: Rip-out landed (2026-06-05) — nix seed is the ONLY Stage 0 path; Alpine/apk/pgp/`init.sh`/the Alpine release key deleted; `pgp` dropped from the tree (`cargo tree -i pgp` empty). Bootstrap trust model captured in **ADR-071**. Remaining (own follow-ups, not blockers): **x86_64 support** (mvmctl is aarch64-guest only today — the embed toolchain is single-target, a pre-existing ADR-065 limit; **its own workstream → Plan 164**), persistent ext4 `/nix` store (a RAM optimization — tmpfs holds today). In-process xz decode is **done**.
...
- [x] **In-process xz decode (done 2026-06-05).** `extract_nix_store_tarball` decodes `.xz` via `lzma-rs` (pure Rust) + the `tar` crate, with `lift_single_top_level` for the strip-components-1 lift. No more host-`tar` shell-out; round-trip test (`extract_nix_store_tarball_decodes_xz_and_strips_wrapper`) covers it.
