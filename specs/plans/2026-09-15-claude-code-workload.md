...
- [x] A reachable CLI surface binding a managed secret to a run: `--secret
      ...
      destination-binding semantics. Landed 2026-09-15 — see "W5 findings"
      below for the shape it took.
...
#### W5 findings (2026-09-15)

The `--secret` surface, as landed:

- **Grammar**: `--secret NAME[:HOST,...]`, repeatable, on the shared
  `RunArgs` — so `run` and `machine run` accept it identically. `NAME` is
  the stored secret's name; the optional hosts are a per-run declaration
  validated against the stored binding's allow-list (a host outside it
  refuses at admission). Enforcement on the wire stays the binding's
  literal allow-list fixed at `secret set` time; the flag can never widen
  it. The dead `up::Args` field was removed (moved, not forked).
- **Guest variable naming**: the catalog gained an optional `env_var`
  field (`crates/mvm-contract/src/service_catalog.rs`) — `anthropic` →
  `ANTHROPIC_API_KEY`, `openai` → `OPENAI_API_KEY`, `github` →
  `GITHUB_TOKEN`, `stripe` → `STRIPE_API_KEY`; `aws-s3` stays `None`
  (SigV4's credential is two halves). A secret authored under a provider
  surfaces its placeholder under that name; an uncatalogued binding
  derives the variable from the secret name (uppercased, `-` folded to
  `_`). Naming only — no destination or substitution decision reads it,
  which keeps the resolved-at-authoring-time rule for hosts intact. This
  was chosen over a `--secret NAME:ENV_VAR` spelling because the existing
  shapes already carried everything but the name: `SecretBinding.name` is
  the guest var, `MachineSecretRef.placeholder_var` records it, and the
  binding meta records the authoring provider.
- **Resolution path** (`crates/mvm-cli/src/commands/vm/run_secrets.rs`):
  parse → `SecretService::validate_for_admission` (the same fail-closed
  check persistent-machine references run) → `SecretBinding` list +
  `MachineSecretRef` list. The transient path threads the bindings into
  `admit_plan_for_boot`; the signed plan carries them
  (`admitted_plan_carries_the_secret_bindings_and_planbound_release`)
  and the endpoint spawn path already decodes exactly that
  (`secrets_from_signed_json` → `EndpointConfig.secrets` →
  `SubstitutionService::from_plan` → placeholder handshake).
- **Persistent machines**: `machine.json` gained a raw `secrets` list
  (recreate-on-change, like `allow_host`); every start re-resolves
  fail-closed and records the reference set, so `mvmctl secret rm`
