...
- [ ] **Task E3.1 — egress-tier roaming for the unified builder image (ADR-060/065 carryover).** ADR-065 collapsed the builder and dev images into one rootfs. That rootfs now runs under three distinct egress postures: **Dev tier** (`mvmctl dev up`, interactive shell — full egress; gateway audit is intentionally not required at this tier per `feedback_dev_vm_vs_prod_security_tiers`); **Build-job tier** (submitted builder jobs / mvmd-driven pipelines — restricted egress per the `NetworkProvider` default-deny policy from Plan 123 A1–A2); **Workload tier** (production microVMs that pick this overlay up via the runtime-overlay path — per-deployment `EgressPolicy`). The selection mechanism that picks the right tier at launch is implicit today. Make it explicit: (a) add `EgressTier { Dev, BuildJob, Workload }` to `mvm-core::policy`; (b) every launch path explicitly selects one — refuse to launch on `EgressTier::Unset`; (c) extend the E3 `doctor` table with an "egress tier" column; (d) cross-reference Plan 129 D–E so the leak detector knows which tier it's inspecting. Failing tests: each launch path picks the expected tier; an unset tier fails closed; the doctor row populates. Fail-closed enforcement still lives in mvmd's admission gate; this is the mvm-side selection + observability layer only. Commit.
- [ ] **Task E4 — named security profiles.** `--profile <name>` selects a named capability matrix over the seams (seccomp tier, egress posture, snapshot allowance), and the authoring-side `mvm.toml [security].preset = <name>` declares the same matrix at the workload level (CLI `--profile` overrides the file). Failing tests: a profile resolves to the expected per-seam dispositions; `[security].preset` parses to the same matrix; `--profile` overrides the file value; an unknown profile errors. Fail-closed *enforcement* of a preset stays in mvmd (the `--prod` admission gate is mvmd's) — this is the mvm-side authoring + resolution only. Commit.
...
- [ ] `--secret NAME:host` parses to a `SecretRef`; `doctor` shows the per-backend capability table; `--profile` selects a named matrix and `mvm.toml [security].preset` resolves to the same matrix (CLI overrides file).
