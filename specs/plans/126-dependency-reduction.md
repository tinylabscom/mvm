...
- [x] **Step 0 (decision, 2026-06-20):** **rehome + upstream** — chosen over fork/replace-now. B4 + C1 move to the dependency roadmap; the durable fix is option 3 (upstream a `rustls-tls-ring` feature to `oci-client`, then bump). Why not the bounded fork now: `oci-client` 0.16/0.17 both hardcode aws-lc in their only rustls option (no ring/no-provider feature), and even reqwest 0.13's `rustls-no-provider` path pulls `rustls-platform-verifier`, which can re-drag aws-lc — so a multi-hour fork + reqwest-major unify could land and *still* leave `cargo tree -i aws-lc-rs` non-empty. Low expected value for the last/lowest-value target; the D1/D2 gates already hold the regression closed. Refactor-close is no longer gated on this.
...
- [~] `aws-lc-rs` gone (`cargo tree -i aws-lc-rs` empty, C/cmake build removed).
      **Rehomed (Step-0 decision 2026-06-20):** not achievable by config; `oci-client`
      ...
      roadmap behind an upstream `oci-client` `rustls-tls-ring` FR; D1/D2 gates hold the
