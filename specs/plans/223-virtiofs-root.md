...
ADR-002 (security posture — claim 3, tier matrix), Plan 214 (HVF VMM),
...
supermachine model and the Plan-214 hvf-HVF end state; it deletes rootfs
...
Plan 214's hvf-HVF backend exposing a virtiofs **root** device. Pick this up
...
`VZVirtioFileSystemDeviceConfiguration`); the HVF backend (Plan 214)
...
- [ ] HVF backend: attach a read-only virtiofs device for the root tree.
...
- **Live boot.** `mvmctl run --image <ref>` on the HVF backend (and Vz)
...
A1–A2 are backend + guest plumbing (Plan-214-adjacent; the HVF backend
...
accepted, so the design is settled; scheduling waits on Plan 214's hvf-HVF
