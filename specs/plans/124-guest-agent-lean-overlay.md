...
- [x] **Foundation fix (D1.0).** `gen_stubs.rs` ran `cargo run -p mvm-ir …` — a crate Plan 121 deleted (folded into `mvm-sdk::ir`) — so `gen-stubs`/`check-stubs` were **unrunnable**, and the committed stubs had silently **drifted** (missing `NetworkMode::Custom` from Plan 123 A5 and `SecretRef.allowed_hosts`/`auth_type` from Plan 129 A). Fixed the invocation to `-p mvm-sdk` (+ stale doc comments) and regenerated the stubs; `check-stubs` is GREEN. The codegen runs locally (`uvx datamodel-codegen` + `npx json-schema-to-typescript`, version-pinned).
  - [ ] **CI no-drift gate — deferred.** Wiring `check-stubs` into `ci.yml` needs the Python (`uv`) + TS (`node`) toolchains in the Lint job (currently cargo-only); the codegen's cross-platform determinism is unverified from a macOS dev host, so a CI-tested job belongs with the Plan 128 gate wiring (the plan already assigns it there). Don't land an untested red gate.
...
- [ ] **Step 1 (the real work):** a **protocol schema** SSOT — derive `JsonSchema` on `GuestRequest`/`GuestResponse` + the broker verb payload types, add an `emit_protocol_schema` bin. Round-trip test (generated client ↔ generated agent types).
- [ ] **Step 2:** `xtask gen-sdk` → the **RPC client method surface** (one RPC → one method) into `sdks/{python,typescript}/_generated/` + the Rust agent request enum from the same schema, committed + gated.
- [ ] **Step 3:** Replace the hand-written `vsock` request enum + the SDK wire dicts with the generated ones; the 125 veneer sits over `sdks/*/_generated/`. Commit.
