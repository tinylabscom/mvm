...
- [ ] **Step 2b (D1.2):** generate the typed **host-side Rust** RPC client (one method per verb, return type from `response_contract()`, unary vs frame-stream) over `Verb`/`response_contract()`, replacing the scattered hand-encoding at the CLI call sites; round-trip test (client ↔ agent dispatch — this is also where "agent dispatch honors the contract" gets asserted end-to-end). Generating a Rust enum *from* `protocol-v0.json` is circular (Rust is the schema's source), so that sub-goal is dropped.
