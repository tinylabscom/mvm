...
- [ ] **Step 2 (D1.2):** the **RPC client method surface** (one RPC → one method) over the generated `_protocol` types + the Rust agent request enum from `protocol-v0.json`, committed + gated. Round-trip test (generated client ↔ generated agent types).
