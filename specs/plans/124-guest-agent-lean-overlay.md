...
- [ ] **Step 2c (D1.2):** migrate the remaining `send_request` call sites in `mvm-cli` (`exec.rs`, `commands/vm/{wait,console,proc,cp,session}.rs`, `commands/shared/vsock.rs`) onto `call_unary` / `call_streaming`. Gated on the Plan 189 `--json` CLI work settling (those files are hot). End-to-end "agent dispatch honors the contract" gets its live assertion here (against a real guest, not a socket-pair fake).
