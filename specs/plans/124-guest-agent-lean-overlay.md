...
- [ ] **Step 2 (D1.2):** `xtask gen-sdk` → the **RPC client method surface** (one RPC → one method) into `sdks/{python,typescript}/_generated/` + the Rust agent request enum from `protocol-v0.json`, committed + gated. Round-trip test (generated client ↔ generated agent types).
- [ ] **Step 3 (D1.3):** Replace the hand-written `vsock` request enum + the SDK wire dicts with the generated ones; the 125 veneer sits over `sdks/*/_generated/`. Commit.
