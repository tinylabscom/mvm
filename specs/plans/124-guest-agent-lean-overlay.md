...
- [ ] **Wire the resolver into the boot path.** Every production `VmStartConfig` leaves `runtime_overlay_{path,verity_path,roothash}` at `None` (`..Default::default()`); nothing calls `RuntimeOverlayResolver::resolve`, so **the overlay is built but never attached** at `mvmctl up`/`run`. Populate the fields in the up/run flow (`up.rs`, `exec.rs`, `start.rs`). This is **Firecracker-only** (the only backend with overlay attach + verity-init initramfs), the resolver's nix-build path returns `HostUnsupported` on macOS, and the acceptance test (agent runs from `/mvm/runtime/agent`; a tampered overlay panics the kernel on the dm-verity roothash — claim 3 lineage) needs a **real verity-enabled Firecracker/KVM boot**. Land + validate on the Hetzner Debian-12/KVM box, not the macOS dev host.
