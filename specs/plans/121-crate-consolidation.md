...
| B4 | dedup | framing → `mvm-core::framing` (#581, 4 real copies); paths → existing `mvm-core::config` helpers (#583, 15 sites); config_envelope + subprocess **descoped** (no genuine cluster — already unified / distinct by design) | see B4 |
...
- [x] **Step 1 (framing) — Option A shipped (#581):** the "6 framing impls" were not 6 duplicates — only the **four byte-identical no-auth length-prefixed-JSON** copies (supervisor proxy + broker / host-signer / audit-signer UDS servers) were genuine duplication. Collapsed them into `core::framing` (`read_json_frame` / `write_json_frame`: 4-byte BE length + serde body, cap-before-alloc; round-trip + oversize-reject + truncated + malformed unit tests). Call sites keep thin error-context adapters. **Option B (the `FramedMessage<T>` pluggable-auth retrofit) is descoped — see the deferred follow-up below; do NOT treat this Step 1 as covering it.**
- [x] **Step 2 (config_envelope) — generic loader DESCOPED (no genuine cluster).** Investigation found the file-loaders each parse a *different* type with *materially different* semantics: `PolicyBundle::load_bundle_from_path` distinguishes `NotFound` as a recoverable error + gates `schema_version`; `RuntimeConfig::load_from` returns `Ok(None)` on missing + runs post-parse `validate()` (pool-size/mode); the registry loaders (`LocalVolumeCatalog`/`VolumeMountRegistry`/`TemplateTags`) fall back to `Ok(default())`; `MvmConfig::load` is deliberately fail-*open* (defaults on any error). A single `load_envelope<T>(path)` can't honor those distinct missing-file/validation/error contracts — forcing it would erase semantics for ~6 lines saved (mirrors the framing Option-B call). The only near-identical trio is the 3 registry loaders, a minor optional DRY not worth a `core::` module. **Real win extracted into a follow-up, not this task:** 5 file-loaded types lack `#[serde(deny_unknown_fields)]` (`MvmConfig`, `Manifest`, `ArtifactManifest`, `VmNameRegistry`, `VmRegistration`) — a fail-closed *hardening* gap (per-type risk; `MvmConfig` is intentionally lenient), tracked under deferred follow-ups, NOT folded into a low-risk dedup.
- [x] **Step 3 (paths) — done as a call-site dedup, NO new `core::paths` module.** The canonical XDG/data helpers already live in `mvm_core::config` (`mvm_data_dir`/`mvm_cache_dir`/… + the `MVM_*_DIR` overrides), so a `core::paths` module would be pure rename churn with zero dedup. Instead: added `config::mvm_data_dir_strict()` (honors `MVM_DATA_DIR`, errors instead of `/tmp` on unset `$HOME` for secret/bundle/trust/audit state) and collapsed **15 inline `$HOME/.mvm` / `$HOME/.cache/mvm` re-derivations** across mvm-core (6), mvm-cli (6), mvm-hostd (1), mvm-build (2) onto the canonical resolvers. This also **fixed a real bug**: those 15 sites silently ignored the `MVM_DATA_DIR`/`MVM_CACHE_DIR` overrides that isolate parallel sessions. Tests assert override-honoring + the no-`/tmp`-fallback contract.
- [x] **Step 4 (subprocess) — DESCOPED: the scaffold already exists, nothing to extract.** The "three signer-subprocess templates" are not duplicates. The spawn/wait/probe/teardown scaffold is *already* one keyless impl — `mvm_hostd::supervisor::services::spawn` (`ProcessSpawner` + `RestartSupervisor` + `probe::wait_for_uds`, from Plan 104 W1b), generic over any stdin-config + UDS-binding binary. The transport is *already* one impl — `mvm_core::framing` (#581 / Step 1). What's left per-signer is genuinely distinct and must stay so: the three proxy clients have different wire protocols + error types (`SignRequest` / `AppendEntryRequest` / `ServiceCall`), and the three binaries have different startup + concurrency (host-signer `Arc<Keystore>`, audit-signer `Mutex<Chain>`, broker `Arc<Registry>`). A `core::subprocess` "scaffold" would either duplicate the existing spawner or erase the proxies' type safety. The spawner lives in `mvm-hostd` (host-daemon-specific), not `mvm-core` — correct per ADR-066 §3. Optional micro-win (a shared `SubprocessConfig` shape) left to a later plan, not this consolidation.
- [x] **Step 5:** Workspace green + clippy + `check-forbidden-deps` — gated per-PR by CI on #581 (framing) and #583 (paths); no banned dep introduced (the dedups added zero third-party deps).
...
- [x] Workspace is the consolidated set — 15 Rust crates under `crates/` (`mvm-core, mvm-sdk, mvm-sdk-macros, mvm, mvm-build, mvm-guest, mvm-guest-helpers, mvm-cli, mvm-mcp, mvm-oci, mvm-backend, mvm-storage, mvm-hostd, mvm-vm-host, mvm-verify`) + `mvm-vz-supervisor` (Swift) + `xtask` (root) + `crates/deps/libkrun-sys` + the fuzz crates. (`mvm-verify` is the +1 the original "16" undercounted — see the E1 note; `mvm-network` = next, plan 123.) Landed in #573.
- [x] `cargo test --workspace`, `cargo clippy --workspace -- -D warnings`, `cargo fmt --all -- --check` green — gated per-PR by CI on #573 / #581 / #583.
- [ ] **`mvm-core` is NOT async-free — pre-existing, not introduced by this plan.** `tokio` (`io-util` base + feature-gated `rt`/`net`/…) has been a normal `[dependencies]` of `mvm-core` since the first verbatim workspace import (`1c3e00c3`), before any B-fold. B1–B3 added no runtime dep; `core::framing` (#581) used the already-present `tokio::io`. The literal `cargo tree -p mvm-core \| grep tokio == empty` check was aspirational and is **not CI-enforced** (no workflow runs it). Making `mvm-core` genuinely runtime-free (move the async surfaces out) is a **separate follow-up**, not a plan-121 deliverable — tracked below.
- [x] The **full ADR §8 rename-break checklist** applied same-commit (3 fuzz dirs + `-p mvm-libkrun` lane + sealed-prod-allowlist `-p mvm-core`/`-p mvm-guest` + the `policy::security::tests::*`/`vsock::tests::*` count-locks + `check-audit-positional` exempt path + `check-mvm-host-binaries-sync` manifest path); all existing xtask claim-gate lints green. Landed in #573 + the merge-conflict resolution (architecture.yml allowlist repoint).
- [x] `mvmctl` facade + `mvm-core` module map current; the old→new ident table is published for mvmd plan 51. Landed in #573 (E1).
- [x] ADR-066 + CLAUDE.md reflect the consolidated reality; `mvm-vm-sidecar`→`mvm-vm-host` fixed. Landed in #573 (E1).

### deferred follow-ups

- [ ] **B4 Option B — `core::framing` pluggable auth/encryption retrofit (NOT a duplicate dedup; a unification).** Today the host↔guest Ed25519-signed `AuthenticatedFrame` (session-id + sequence replay protection, separately fuzzed; backs claims 8/12/13) and the builder channel that inherits it live as a **single** impl in `mvm_core::crypto` / `mvm_guest::vsock` — not a copy of `core::framing`, so folding them onto a generic `FramedMessage<T>` buys **zero** dedup. It was descoped from #581 on that basis: it would churn a claims-8/12/13 fuzzed wire format for no payload reduction. Revisit **only** if a real new untrusted-boundary channel needs the auth/encryption stage (e.g. confidential computing — out of scope per ADR-002 today, see ADR-066 §5 / §"Encryption seam"). **Gate before landing:** a dedicated cargo-fuzz pass on the unified frame parser **and** a live host↔guest boot round-trip (`examples/agent_ping`) proving the authed path is byte-for-byte unchanged — never a pure refactor commit. The `core::framing` doc-comment (`AuthStage` seam, `NoAuth` the only impl) is the shape this slots into. Not blocking any plan; tracked here so the descope is not mistaken for "done".
- [ ] **B4 config fail-closed hardening (surfaced during the Step-2 descope, NOT dedup).** Five types are loaded from disk WITHOUT `#[serde(deny_unknown_fields)]`, so a typo'd key is silently dropped instead of failing loud: `MvmConfig` (`~/.mvm/config.toml`), `Manifest` (`mvm.toml`), `ArtifactManifest` (`manifest.json`), `VmNameRegistry` + its nested `VmRegistration` (`vm-names.json`). Adding the attribute is a per-type *behavior* change (e.g. `MvmConfig::load` is deliberately fail-open — tightening it would make a typo silently fall back to defaults, arguably worse), so each needs its own judgement + a forward-compat check, not a blanket sweep. Own commit/PR, gated on a read of each type's intended leniency. Not blocking.
- [ ] **Make `mvm-core` genuinely runtime-free (the aspirational "no async in core" invariant).** `tokio` is a normal dep today (pre-dates plan 121); CLAUDE.md still claims core is runtime-free "in the default build". To make it true: audit what in `mvm-core` actually needs `tokio` (`core::framing`'s async I/O — whose only callers are the 4 mvm-hostd UDS channels — plus any feature-gated `rt`/`net` users), move those surfaces to the consuming crate (`core::framing` → `mvm-hostd`, since all callers live there), and add the `cargo tree -p mvm-core -e no-dev \| grep -qv tokio` gate to CI so it can't regress. Own plan; **not** a plan-121 deliverable. Until then, reconcile CLAUDE.md's "no runtime deps" wording with reality.
