...
**One accepted out-of-scope caveat:** the Acceptance item *"no async/runtime dep in `mvm-core`"* is **not met** — `tokio` has been a normal dep since the first workspace import (`1c3e00c3`), pre-dating every B-fold; B1–B3 added none. Not CI-enforced, not introduced here. Making core genuinely runtime-free is now **folded into plan 126 Task B5**, not a plan-121 deliverable. The granular per-Step boxes below are the historical execution recipe — the work shipped in the three PRs named above.

**The three spawned follow-ups are folded into active plans** (so a closed plan's deferred list isn't their only home): framing Option B → **plan 122 Task A0**; `deny_unknown_fields` config hardening → **plan 128 Task C4**; runtime-free `mvm-core` → **plan 126 Task B5**. See the deferred-follow-ups list at the foot for the pointers.
...
- [x] **Step 3 (paths) — done as a call-site dedup, NO new `core::paths` module.** The canonical XDG/data helpers already live in `mvm_core::config` (`mvm_data_dir`/`mvm_cache_dir`/… + the `MVM_*_DIR` overrides), so a `core::paths` module would be pure rename churn with zero dedup. Instead: added `config::mvm_data_dir_strict()` (honors `MVM_DATA_DIR`, errors instead of `/tmp` on unset `$HOME` for secret/bundle/trust/audit state) and collapsed **15 inline `$HOME/.mvm` / `$HOME/.cache/mvm` re-derivations** across mvm-core (6), mvm-cli (6), mvm-hostd (1), mvm-build (2) onto the canonical resolvers. This also **fixed a real bug**: those 15 sites silently ignored the `MVM_DATA_DIR`/`MVM_CACHE_DIR` overrides that isolate parallel sessions. Tests assert override-honoring + the no-`/tmp`-fallback contract. *(Extended by #587 (a parallel session, fix #582): kept `mvm_data_dir_strict()` and added typed helpers `vm_state_dir` / `vm_vsock_port_socket` / `mvm_keys_dir` / … so the `~/.mvm` centralization this step began is now complete across the dev-up / core-demo paths.)*
...
- [x] **B4 Option B — `core::framing` auth/encryption retrofit → folded into plan 122 Task A0** (the encryption seam; 122 A0 is the first plan that needs the pluggable `FramedMessage<T>` stage, and its prior text wrongly assumed 121 had built it). Full rationale + the cargo-fuzz + live-boot gate now live there.
- [x] **B4 config fail-closed hardening (`deny_unknown_fields` on 5 file-loaded types) → folded into plan 128 Task C4** (fail-closed config + a `check-config-deny-unknown-fields` gate). Per-type judgement noted there (`MvmConfig` is intentionally fail-open).
- [x] **Make `mvm-core` genuinely runtime-free → folded into plan 126 Task B5** (drop `tokio` from core's default closure by relocating `core::framing` → `mvm-hostd`, + the `cargo tree -p mvm-core | grep tokio` assertion wired into 126 D1's gate). Coordinates with 122 A0 (both touch `framing`).
