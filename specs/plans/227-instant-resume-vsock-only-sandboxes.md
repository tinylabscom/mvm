...
- [ ] **F5 (SDK thin-wrapper parity):** the language SDKs (Python/TS/Rust) gain the new lifecycle ops as thin wrappers only — subprocess argv locally, gateway DTOs remotely, both fixture-pinned; no orchestration logic in any SDK. The build-time decorator SDKs are explicitly out of the facade: their language-neutral contract stays the canonical `Workload` IR (`mvm-sdk::ir`), so a new language = IR-emitting decorators + a facade wrapper, nothing more. Three seams total (decorators→IR, lifecycle→MvmClient, in-guest→agent vsock protocol), each versioned + conformance-tested.
- [ ] **F7 (guest-ops through the facade):** the agent protocol already carries the full runtime-SDK semantic set (Exec/ExecBatch, RunCode, Fs{Read,Write,List,Stat,Mkdir,Remove,Move}, Proc{Start,Signal,SendInput,Wait,Kill}, StartPortForward/StartUnixSocketForward, UpdateIdleTimeout, and the snapshot hooks SleepPrep/PostRestore/Primed) — but none of it is facade-reachable. Add typed `MvmClient` ops for the common set (exec-streamed, run_code, files read/write/list, proc lifecycle, port-forward, idle timeout) so SDKs get the object model (`sandbox.files.*`, `sandbox.run_code(...)`, `sandbox.commands.*`) as thin wrappers. Profile-gated end-to-end: dev tier = full set; sealed prod = entrypoint-only (the facade surfaces the same `VerbNotAuthorized` refusals the agent enforces — no fail-open at the client layer).
- [ ] **F6 (entrypoint × snapshot semantics):** production workloads have exactly one work surface — the entrypoint (claim 15 / Plan 165; `RunEntrypoint`, `/etc/mvm/entrypoint`, sealed-prod no-interactivity). Define and test the snapshot point relative to it: the golden freezes at the *primed* barrier (entrypoint initialized, awaiting input — Plan 206 T2 heritage), so restores/forks resume inside a warm entrypoint instead of re-running init. `SnapshotState` records the entrypoint phase; restoring a pre-primed snapshot is refused in prod.
