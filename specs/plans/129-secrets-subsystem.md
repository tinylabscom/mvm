...
> - [ ] **Host side remaining:** `SubstitutionService::serve` takes a `UnixListener` today (right for Firecracker's per-port UDS); QEMU needs a **host AF_VSOCK listener** on `SUBSTITUTION_PORT`. Add a backend-selected listener (UDS vs AF_VSOCK accept loop) — then the 4 wiring pieces.
