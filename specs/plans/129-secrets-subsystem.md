...
- [ ] **#4 remainder (needs the routing-architecture decision + a real boot):** the guest-side routing — how a workload's HTTP reaches the endpoint over vsock. **Fork:** (a) a guest-local forward proxy the workload's `HTTP_PROXY`/`HTTPS_PROXY` points at (language-agnostic, works with Python `requests`/Node `http`), relaying `WireRequest` over `vsock::connect_to_port` to the host `SubstitutionService`; vs (b) an SDK-specific HTTP client. Then: the Python `mvm.secret("name")` surface returns the admission-minted placeholder (`sdks/python/mvm/_dsl.py`), `#1b` bin glue injects the `HandedPlaceholders` into guest env + exposes the endpoint vsock port, the `sign` wire op, and retiring the legacy `mvm-sdk/src/runtime_substitution.rs` (`mvm-secret://`, dead ADR-049 scaffolding superseded by ADR-062) so there is one opaque format (`mvm-secret-<hex>`; ADR-067 §4 requires opacity).
