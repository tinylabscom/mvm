...
- [ ] **#4 remainder (boot-validated):** the HTTP-proxy *front* in the guest (parse the workload's proxied request → `WireRequest`; the HTTPS terminate/re-originate model so the proxy sees the request) + `HTTP_PROXY`/`HTTPS_PROXY` env wiring; the Python `mvm.secret("name")` surface returns the admission-minted placeholder (`sdks/python/mvm/_dsl.py`); `#1b` bin glue injects `HandedPlaceholders` into guest env + exposes `SUBSTITUTION_PORT` (incl. the host proxy port-allowlist W1.3); the `sign` wire op; retiring the legacy `mvm-sdk/src/runtime_substitution.rs` (`mvm-secret://`, dead ADR-049 scaffolding superseded by ADR-062) so there is one opaque format (`mvm-secret-<hex>`; ADR-067 §4 requires opacity).
