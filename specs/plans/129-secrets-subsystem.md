...
3. **`#1b` endpoint spawn** — **DECISION (2026-06-08, user): a dedicated per-VM moat process holds plaintext secrets, not the per-backend supervisor bin.** Rationale: the one process that decrypts secrets should be minimal + confinable (ADR-002 process-moat), separate from the general VM supervisor. The host (mvm-local) holds the encrypted secrets in `~/.mvm`; the endpoint opens them by path. QEMU-first (the only on-box-validatable backend; it has no supervisor bin anyway).
   ...
   > - [x] **On-box boot e2e CLOSED (2026-06-10, QEMU) — #745/#749/#755:** a secret-declaring workload (`examples/python/secret-egress`) launched via `up --flake` through admission; the destination (httpbin) reflected the **real** `Authorization: Bearer …` while the guest held only `mvm-secret-…`; `substitution.pid` lived for the run + was reaped on stop; an unbound host was refused (claim 12). The live guest→host vsock path (CID 2:5253) + http forward are proven end-to-end. (https + Firecracker is the separate Stage 2 layer — see `specs/notes/plan-129-stage2-https-ca-tdd-plan.md`; S2.7 box e2e gated on FC agent bringup.)
...
- [x] `secret.substituted` / `secret.placeholder_dropped` audit emit (Phase E2) wired into the endpoint's success/refusal paths. `secret.substituted` on the `process` forward-success path; `secret.placeholder_dropped` on the `process` claim-12 refusal path (a placeholder-bearing request bound for an unbound destination — `emits_placeholder_dropped_audit_on_unbound_refusal`). Metadata only; the value and the secret name never reach the chain. *(Remaining: the `:80`/`:443` terminator path's `handle_request` fuses substitution + forward under one opaque `anyhow::Error`, so it can't separate a claim-12 refusal from a network forward-failure without a typed-error refactor — deferred to the #1b bin-glue pass that wires the terminator into a booted FC/libkrun backend. No audit gap meanwhile: terminator refusals still close the socket and the always-on `PlaceholderLeakScan` packet backstop covers side-channel placeholders.)*
...
- [x] **Step 3:** `secret.placeholder_dropped` wired **live** into the `SubstitutionService` `process` refusal path: a placeholder-bearing request refused for an unbound destination (claim 12) emits the dedicated event (destination only — no value, no name). Best-effort, never fails the request; mirrors the `secret.substituted` success emit. Committed.

> **Deferred:** wiring `secret.placeholder_dropped` into (a) the `:80`/`:443` terminator refusal path — blocked on a typed-error refactor of `terminator::handler::handle_request` (today it fuses substitution + forward under one `anyhow::Error`, so a refusal is indistinguishable from a forward-failure) — and (b) the live packet pipeline backstop (the drop is already audited via the generic `gateway.flow_observer_fault` with `by="placeholder-leak"`; the dedicated event is a refinement). Both land with the #1b bin-glue pass that wires those paths into a booted FC/libkrun backend.
