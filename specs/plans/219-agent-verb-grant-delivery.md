# Plan 219 — Plan-bound Agent Verb Grant: Out-of-Band Delivery + End-to-End Wiring
...
**Architecture:** The staged core proved the *types and the guest verify seam* with unit tests, but left three things unwired: (1) the guest has no host-signer pubkey to verify against; (2) the guest has no way to learn the admitted plan's `nonce`; (3) nobody mints the grant on the launch path or hands it to the agent. This plan closes all three with two existing per-launch out-of-band channels to a sealed guest: the read-only `mvm-config` drive carries the host-signer public key, and the kernel cmdline hex-token mechanism (`mvm.egress_ca=`, `mvm.secret_env=`) carries the signed `VerbGrantEnvelope` + nonce that `/init` decodes into `/run/mvm/`. The grant reaches the agent before it binds vsock, and no host↔guest wire type changes.
...
**Refresh 2026-07-13:** the current-main follow-on branch now carries a
disabled-by-default macOS witness harness in `tests/oci_image_runner_smoke.rs`
behind `MVM_AGENT_VERB_GRANT_SMOKE=1`. It compiles the checked-in
`examples/python/hello-app`, boots a sealed HVF `machine run --entrypoint -d`
session with restricted `ping` + `run-entrypoint` grants, asserts listed
`RunEntrypoint` success (`"hello ari"`), checks `console.log` ordering for
`mvm-init: provisioned verb-grant` before
`mvm-guest-agent: control plane ready`, expects the operator-facing
`mvmctl machine set-timeout <vm> 349` call to fail with
`update-idle-timeout`, and verifies both the per-VM `verb_denied` audit chain
and `mvmctl trust audit verify --tenant local`. Focused validation of the
harness is green with `cargo test --test oci_image_runner_smoke -- --nocapture`
and `cargo clippy --test oci_image_runner_smoke -- -D warnings`. The live macOS
proof checkbox remains open until that witness is executed on a host with the
real runtime prerequisites and the evidence is captured.
...
`mvmctl trust audit verify --tenant local` still passed. The remaining live gap
is macOS proof only; the listed sealed-image `RunEntrypoint` witness and the
sealed-image `DevOnly` refusal witness are now both covered on Linux.
**macOS narrowing 2026-07-10:** a real source-checkout regression in the
libkrun Stage 0 helper resolver is now fixed. A `/tmp/.../debug/mvmctl`
source build previously fell through to a stale `$PATH`
`mvm-libkrun-supervisor`, which rejected the current `SupervisorConfig`
(`network_provider = disconnected`) before Stage 0 could build the workload
kernel. The resolver now treats standalone `debug`/`release` cargo layouts as
source-checkout builds and rebuilds the helper into that same target dir,
closing the stale-helper trap. A second real macOS blocker also landed on the
same refresh branch: the builder VM's production egress allowlist had never
admitted `crates.io`, `index.crates.io`, or `static.crates.io`, so Cargo
vendoring in the no-guest-NIC builder path could fail with host-generated
...
### Fork 2 — delivery vehicle: **config-drive pubkey + kernel cmdline grant token, NOT extending `ProtocolHello`.**

The workload boot path uses no authenticated `ProtocolHello`; the readiness probe has neither the admitted plan nor the host-signer keystore. The signed grant is pre-computed before boot and rides the existing cmdline token channel, while the trust anchor rides the existing per-launch config drive (`mvm-config`). `/init` stages both files into `/run/mvm/` before the agent forks, so the grant is pinned by the time `wait_for_guest_agent` probes. Extending `ProtocolHello` was rejected: it would require adding an `Option<VerbGrant>` to a `deny_unknown_fields` wire type, re-plumbing the unauthenticated readiness probe to carry a per-workload token, and threading plan/keystore material into a readiness poller with neither. A dedicated post-handshake RPC verb is worse still because the agent would answer verbs before it is pinned.

### Fork 3 — Gap 1 (guest provisioning + `HOST_SIGNER_PUBKEY_PATH` reconciliation): **copy config-drive `host-signer.pub` into `/run/mvm/host-signer.pub`.**

`HOST_SIGNER_PUBKEY_PATH = /etc/mvm/host-signer.pub` cannot stay on the dm-verity-sealed read-only rootfs. Decision: the launcher adds `host-signer.pub` to `VmStartConfig.config_files` when a plan has `agent_verbs`; `/init` copies `/mnt/config/host-signer.pub` to `/run/mvm/host-signer.pub` and decodes the grant to `/run/mvm/verb-grant.json`, both before the agent forks. The guest verifier loads the key from `/run/mvm/host-signer.pub`; `VerbGrantEnvelope.pubkey_hex` is compatibility/diagnostic data only and is not trusted.
...
- **Reuse first.** Grant delivery reuses the cmdline-token pattern (`encode_secret_env_cmdline` shape) and the `/init` hex-decode idiom verbatim. Host-signer pubkey delivery reuses `VmStartConfig.config_files` / `mvm-config`. Mint reuses `mint_verb_grant` (already landed). Verify reuses `pin_verb_grant` + `load_host_signer_verifying_key` (already landed). Do NOT add a new wire type or a canonicalizer.
...
- **Docs upkeep:** on completion, tick Plan 219 in `specs/REFACTOR-STATUS.md` and reflect status in `specs/SPRINT.md` in the same change; flip ADR-103 `Status: Proposed → Accepted` if the maintainer approves.
...
- [x] **Step 1: Write the failing roundtrip test** (mirror `encode_secret_env_cmdline_round_trips_pairs_as_single_token`): build a signed `VerbGrant` (reuse the `verb_grant` test helpers' pattern — `SigningKey::from_bytes`, `Signer`), wrap in `VerbGrantEnvelope`, `encode_verb_grant_cmdline` → assert single space-free token starting `mvm.verb_grant=`, strip prefix, `decode_verb_grant_cmdline` → assert the envelope (pubkey_hex, nonce_hex, grant fields) round-trips. Add a `decode_rejects_malformed_hex` and `decode_rejects_unknown_field` negative test.
- [x] **Step 2: Run** `cargo nextest run -p mvm-core verb_grant_cmdline` — expect FAIL (helpers absent).
- [x] **Step 3: Implement** `VerbGrantEnvelope`, `encode_verb_grant_cmdline` (hex the `serde_json::to_vec`), `decode_verb_grant_cmdline` (un-hex → `serde_json::from_slice`). Keep byte-determinism (fixed-field-order struct, no maps — same rule the landed `VerbGrant::signing_bytes` follows).
- [x] **Step 4: Run** `cargo nextest run -p mvm-core verb_grant_cmdline` — expect PASS. **Unit-testable in full** (pure encode/decode). **Verified 2026-07-04:** `cargo test -p mvm-core verb_grant_cmdline` and `cargo test -p mvm-core decode_rejects`.
- [x] **Step 5: Commit** `feat(core): mvm.verb_grant cmdline token encode/decode for out-of-band grant delivery`.
...
**Files:**
- Modify: `nix/lib/mk-guest.nix` `/init` — add a stage (after 2.47 secret-env, before the agent fork) that `sed`-extracts `mvm.verb_grant=` from `/proc/cmdline`, hex-decodes it (same `printf '%b' | sed 's/../\\x&/g'` idiom as stages 2.46/2.47), writes the raw JSON to `/run/mvm/verb-grant.json` (mode 0644), and copies `/mnt/config/host-signer.pub` to `/run/mvm/host-signer.pub` (mode 0644). Absent token ⇒ whole block is a no-op (byte-identical boot).
...
- Modify: `crates/mvm-cli/src/commands/vm/up.rs` — when an admitted plan has `agent_verbs`, read the same host-signer public key used for admission and add it as `host-signer.pub` in `VmStartConfig.config_files` so the config drive carries the trust anchor.
...
- [x] **Step 1: Write the failing mint-sidecar test** in `plan_admission.rs` (sibling to `stash_plan_for_bridge_writes_both_files_when_present` at `:1032`): a `VmStartConfig` whose `plan_json` decodes to a plan with `agent_verbs = Some([...])` ⇒ `verb-grant.json` written and its envelope verifies under the loaded signer key + the plan nonce; a plan with `agent_verbs = None` ⇒ **no** sidecar. This is the **mint/provision assembly** — unit-testable.
- [x] **Step 2: Write the failing token-builder test** in `microvm.rs` (sibling to the existing cmdline-token tests): given a `verb-grant.json` in a tempdir state dir, `verb_grant_cmdline_token` returns `Some("mvm.verb_grant=…")`; absent ⇒ `None`. Unit-testable. **Also done 2026-07-04:** `up.rs` config-drive host-signer pubkey attachment tests cover grant-present and grant-absent paths.
...
- [x] **Step 1: Read** `emit_oci_provenance` (`:206`-ish) to find the exact private append/chain helper + the emitter's test helpers (`test_signing_key`, `sample_plan`, verify/chain-path helpers). Match them; don't invent.
- [x] **Step 2: Write the failing test** `verb_denied_entry_is_chained_and_verifies` (mirror the `emit_oci_provenance` test): `emit_admitted` then `emit_verb_denied(&plan, "update-idle-timeout")`; assert the chain still verifies, the log contains `verb_denied` + the verb, and a byte-flip breaks `verify_audit_chain`. Unit-testable.
...
- [ ] Update `specs/REFACTOR-STATUS.md` (tick Plan 219) + `specs/SPRINT.md` same commit.
- [ ] Flip ADR-103 `Status: Proposed → Accepted` if the maintainer approves; update its `Sequenced by:` to reference Plan 219 for the delivery leg.
