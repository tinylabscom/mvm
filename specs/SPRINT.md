...
**WS0.4 — dead deps + orphans**
- [ ] Remove `colored`, `names`, `hickory-server`, the stale `mvm-egress-proxy` path dep, and the orphan `crates/mvm-vz-supervisor` dir. Add `cargo machete` + `cargo deny` to CI.
- Gate: `cargo machete` clean; lockfile shrinks.

**WS0.5 — finish worktree cleanup**
- [x] Remove clean/unlocked stale worktrees (19 removed, 77 → 58; branches preserved).
- [ ] Per-tree review the remaining locked/dirty worktrees; `git worktree unlock` + force-remove the confirmed-stale ones (branches preserved regardless).
- Gate: only the active working set remains.
...
- [ ] Merge `setup`/`bootstrap` into one first-run `bootstrap`; add `upgrade` (self-update) and `clean` (single-dir cleanup, replacing the fragmented `env cleanup` / `cache prune` / `pack prune` / `storage gc`).
