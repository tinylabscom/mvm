...
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) WS-B deferred follow-up — **transient-guest eth0 enabler ticked + live-validated** (stacked on the DHCP/ARP PR). The eth0 bring-up itself landed in **#1020** (unconditional/host-enforced — guest-side gating isn't a boundary; deny-all is enforced host-side + the static fallback keeps `eth0` up without hanging). This PR ticks the plan checkbox and records the **live result**: the `examples/egress-probe` workload booted on libkrun and reached BOTH external probe targets (`up --wait` verdict 0), proving the guest now gets a working `eth0` (link-up → DHCP → static gvproxy fallback). The Vz VM-level smoke is gated on `up --wait` being libkrun-only (Plan 152 WS-A); the Vz *bridge* enforcement is covered by the deterministic `*_live_vz_bridge` tests. **Surfaced a tracked follow-up** (added to the plan's deferred list): `up --network-allow` does not enforce egress on the libkrun direct-boot path (verdict 0 = no enforcement, not even the default deny-all) — `up.rs` builds `VmStartConfig { ..Default::default() }` without setting `.network_policy` (the transient `run` path in `exec.rs` does set it, which is the path PR2's bare L4 targets), and the libkrun gvproxy splice interposition on the `up` path needs confirming. Pre-existing, not introduced by the WS-B egress work.
