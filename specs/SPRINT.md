...
- [x] Continued
      ...
      dry-run reporting, and refuses to delete any pack referenced by snapshot
      or warm-standby protection sidecars discovered under the checkpoint store
      and supervisor standby pool. Malformed protection sidecars fail closed.
      Coverage: focused `mvm-core` prune tests for expired/invalid deletion,
      ready-pack retention, dry-run behavior, protection-ref serde, and
      protected-pack retention; focused `mvm-cli` sidecar parsing/discovery
      tests; full workspace `cargo test`, `cargo check`, and
...
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-1.5 required-check shim**. `.github/workflows/rvproxy-parity.yml` now runs on every PR and merge-group SHA, and emits a stable `rvproxy gateway parity` job suitable for branch protection. A cheap Ubuntu changes job skips unrelated PRs/merge groups without spending macOS minutes; gateway-contract PRs/merge groups must pass the existing macOS `rvproxy vs gvproxy parity` candidate/control run. The stable `rvproxy gateway parity` context is now required by `main` branch protection.
