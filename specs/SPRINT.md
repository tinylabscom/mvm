...
- [ ] **In review (PR #868, CI-green)** — Firecracker `stop_vm` brought to the same **sub-second down** behavior as the Vz path: the unconditional `sleep(2s)`-then-kill is replaced by a 250 ms graceful window polled at 25 ms (ACPI `SendCtrlAltDel`, then `firecracker::is_vm_running` until exit), escalating to the hard `kill $(cat pid)` only when the guest ignores the graceful request — a headless workload guest has no power-button handler, so the old path paid 2 s on every `down` for nothing. `crates/mvm-backend/src/microvm.rs`; `cargo clippy -p mvm-backend --lib -- -D warnings` clean (the `stop_vm` shell-into-VM path has no unit test, matching existing coverage).
