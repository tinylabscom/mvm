...
- [ ] 1d `mvm-net`: fold `mvm-network` + host tunnel/gateway/dns + guest net; vsock/UDS transport + egress seam. _(crate rename `mvm-network`→`mvm-net`, the bounded/fuzzed DNS codec, strict answer guard, pins-first `MVM_DNS/1` host handler + async/Linux-blocking dispatch, metadata-only chain-signed per-query audit, per-workload DNS rate/concurrency guards, the loopback UDP+TCP guest DNS stub, loopback resolver seeding across OCI, Nix-busybox, and Stage 0 guest init paths, three live DNS BDD scenarios, and claim-10 witness wiring landed; live scenario execution remains assigned to the KVM/HVF witness host, while tunnel/guest-net absorption remains pending; 3,104 core+agentd-addons+hostd tests plus the serialized workspace suite green; claim catalog clean at 16 claims / 41 witnesses)_
...
- [x] **CROSS-LANGUAGE SDK INTEROP** (2026-07-22): one meaning-identical workload is emitted by the shipped Python and TypeScript declaration SDKs, deserialized through the Rust `Workload` contract, and compared with the native Rust declaration. All three pin `sha256:b7106af4133c7d678744adb3b617e7289bc3f4c131b2df03a8e9cc49aac90037`. `xtask gen-ir-parity` regenerates the committed SDK outputs; `check-ir-parity` executes both language sources and fails CI on drift. The harness caught and fixed Python emitting `hooks`/`files` as JSON `null` even though those Rust fields are non-optional and skip-serialized when empty. Focused gates: Python 15/15, xtask 231/231, Rust parity 1/1, clippy clean; full `cargo test --workspace -- --test-threads=1` green. **Still open:** compare against an independently published UOR-ADDR conformance vector; no external UOR implementation is claimed by this SDK-only witness.

<<<<<<< HEAD
- [ ] **UOR FRAMEWORK EXPLORATION** (`specs/research/uor-framework-integration-exploration.md`, 2026-07-22): surveyed the broader UOR Foundation ecosystem beyond `uor-addr`. Findings: only `uor-addr` is a strong fit; the UOR Framework ontology, Prism substrate, and PrimeShield crypto are not recommended for mvm. Recommended follow-ups: (1) add a conformance test comparing the existing `SemanticAddress` golden vector against UOR-ADDR published vectors and TS/Python SDK output; (2) prototype a `SemanticAddress` for `BuildProvenance` as an optional field using existing `serde_jcs` + `sha2`; (3) defer `uor-addr` crate adoption to WS11-P4/browser where cross-impl equality is load-bearing.
=======
- [x] **UOR FRAMEWORK EXPLORATION** (`specs/research/uor-framework-integration-exploration.md`, 2026-07-22): surveyed the broader UOR Foundation ecosystem beyond `uor-addr`. Findings: only `uor-addr` is a strong fit; the UOR Framework ontology, Prism substrate, and PrimeShield crypto are not recommended for mvm. Recommended follow-ups: (1) add a conformance test comparing the existing `SemanticAddress` golden vector against UOR-ADDR published vectors and TS/Python SDK output; (2) prototype a `SemanticAddress` for `BuildProvenance` as an optional field using existing `serde_jcs` + `sha2`; (3) defer `uor-addr` crate adoption to WS11-P4/browser where cross-impl equality is load-bearing.
>>>>>>> 7b4363ec7 (docs(research): add UOR Framework integration exploration)
