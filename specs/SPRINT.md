...
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-1.5 required-check shim**. `.github/workflows/rvproxy-parity.yml` now runs on every PR and emits a stable `rvproxy gateway parity` job suitable for branch protection. A cheap Ubuntu changes job skips unrelated PRs without spending macOS minutes; gateway-contract PRs must pass the existing macOS `rvproxy vs gvproxy parity` candidate/control run. The stable `rvproxy gateway parity` context is now required by `main` branch protection. Remaining Plan 193 work is rvproxy macOS transparent terminator support/wiring, then splice/Plan-141 hook deletion.
