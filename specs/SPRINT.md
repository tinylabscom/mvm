...
- [ ] Plan 200 auth/security proof follow-up: persistent-machine `ssh_agent` is explicit in signed/admitted policy metadata, receipts, dry-run, and audit, while remaining dev-tier/socket-only with no SSH key or known-host material copied or mounted. Live Firecracker-host proof progressed the builder path but remains unclaimed: QEMU OCI `rootfs.ext4` materialization selection, short QEMU virtiofs socket paths, and sparse-preserving `rootfs.ext4` fallback copies are implemented; the remote proof host then required reboot to clear ext4/jbd2 `D`-state proof processes and returned pingable but with SSH refusing connections. Remaining live gate is SSH/sshd recovery on that host or an equivalent Linux/KVM host, then rerun the throwaway-agent raw protocol probe; non-standard-port SSH-banner denial still needs a runtime classifier.
...
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) C1 live auth-proof attempt recorded — the 2026-06-19 Firecracker-host run built branch-local `mvmctl`, `mvm-libkrun-supervisor`, and `mvm-firecracker-bridge`, created a throwaway host `ssh-agent` plus throwaway key outside the repo, prewarmed the isolated guest-agent and builder-image caches, and attempted a dev-profile `alpine:latest` persistent machine with `[auth].ssh_agent = true`. It did not reach guest boot: `python:3.12-alpine` first hit the expected OCI hardening `MalformedHeader` refusal, and the `alpine:latest` path then blocked during OCI `rootfs.ext4` materialization because `mvm-build::rootfs::materialize_ext4` hardcodes the libkrun builder VM and the libkrun supervisor fails on that Firecracker box with `rc -22`. Therefore the live SSH-agent round-trip remains open until run on a working libkrun-materializer host or until OCI materialization honors the QEMU/builder backend seam. Non-standard-port SSH-banner denial also remains unclaimed because current enforcement has TCP/22 and template-material bans, not a runtime banner classifier.
