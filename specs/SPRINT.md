...
- [x] 2026-07-07 workload-egress fail-closed contract: policy-carrying workload boots now require backend capabilities `vsock + no_guest_nic + host_vsock_proxy`. HVF already satisfied the contract; the follow-on cutover now brings Firecracker and libkrun onto the same workload path instead of silently falling back to TAP/gvproxy/passt. The named capability contract landed in `mvm-core`, workload-backend enforcement + selection wiring landed in `mvm-backend`, the transient/persistent workload boot helpers now check it, and the CLI reference/networking guide plus Plan 227 WS-B were updated together.
- [x] 2026-07-08 workload-egress cutover completion: Firecracker and libkrun workload traffic now egress only through the host-side vsock gate. Firecracker no longer provisions workload TAP/network-interface state or guest IP metadata on the workload path; libkrun no longer stands up gvproxy/passt workload networking and now always wires the host-vsock proxy gate in raw-or-substitution mode. Current workload product backends are HVF, Firecracker, and libkrun; stale Vz selectors were also scrubbed from the active helper/docs surface (`persistent-builder` rejects `--builder vz`, sandbox GC scans the real supported backends, and current macOS/runtime docs no longer advertise Vz as a live workload choice).
- [x] 2026-07-08 #1462 arbitrary-OCI PID 1 hardening: OCI runtime injection now installs a static `mvm-oci-init` binary at `/init` instead of a shell script, so boot no longer depends on `/bin/sh`, busybox, coreutils, or an image-provided init system. OCI config `Entrypoint`/`Cmd` is translated into a shell-free static `mvm-oci-entrypoint` wrapper under the existing `/etc/mvm/entrypoint` security contract, preserving image argv/env/workdir without `sh -c`. Live Firecracker/KVM proof used a `FROM scratch` image containing only `/marker`; the materialized rootfs had executable `/init`, no `/bin/sh`, no `/bin/busybox`, booted with `net:false`/`allow_host:[]`, no guest NIC (`eth0` absent), and guest-agent vsock readiness on port 5252. Pause and Firecracker snapshot warm-start succeeded (`/snapshot/load` 204); post-restore VMGenID delivery on scratch OCI still reported guest-agent unreachable and the direct `--image` path still needs kernel-source override propagation, so those remain follow-ups.
...
- [x] Closed `removed legacy macOS archival spec`
...
## Sprint 55 — `Virtualization.framework` backend (`vz`) — ✅ COMPLETE (closed 2026-06-13; vz at full macOS-libkrun parity)  `removed legacy macOS archival spec` | `removed legacy macOS archival spec`
...
- ✅ **Phase D** — `removed legacy macOS archival spec` filed; ADR-002
...
### Plan 98 — finishing slices  `removed legacy macOS archival spec`
...
Full audit lives in `removed legacy macOS archival spec` §"Can we still make all
nine ADR-002 security claims?". Summary: all
