...
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-1 — per-VM `mvm-audit-signer` spawn + chain wiring** (stands up the workload chain's sole writer; the chain it writes is now verifiable via E5.3b-0). New `mvm-backend::audit_signer_spawn` — **one impl for libkrun + vz** because the audit-signer (unlike the substitution endpoint) has no backend-shaped fields: it always speaks a host UDS. Mirrors `substitution_spawn`: resolve the binary (`MVM_AUDIT_SIGNER_PATH` → sibling-of-exe → workspace `target/{release,debug}`), hand-build the `SubprocessConfig` JSON on stdin (mvm-backend sits **below** mvm-hostd in the dep graph and can't import the type — exactly why substitution hand-builds its config too; a drift surfaces fail-closed as an early signer exit caught by the bind wait), detach via `setsid`, wait for the UDS to bind (the bin writes no stdout handshake → poll the socket path + `try_wait` for early exit, SIGKILL + bail on timeout), persist a PID. Config: `workload_id`=vm_name (signer log label; authoritative per-entry id comes from the broker ctx in -2), `software_chain_key_path` = `compute_audit_substrate().signing_key_path` (the **host signer key** → one trust root with the claim-8 chain), `audit_jsonl_path` = `mvm_core::config::workload_audit_path(tenant, vm)` (the per-VM path E5.3b-0's verifier reads), UDS + secondary-head under `<state_dir>/services/` (created 0700). **Gated on a tenant** (admitted plan) — a no-tenant `up` is a defused no-op; **fail-closed** (spawn error rolls the launch back — an admitted workload must not run unaudited) via an `AuditSignerGuard` (RAII reaper, armed until the VM is up, mirroring the substitution `EndpointGuard`). Wired into `LibkrunBackend::start`/`stop` **and** the vz start/stop, right next to the substitution spawn/reap. 5 unit tests (no-tenant→defused-no-op-guard with no PID file, config has *exactly* the 6 `SubprocessConfig` keys + right values, UDS/head under `services/`, bad-`MVM_AUDIT_SIGNER_PATH` reject, reap idempotent without a pidfile); the live spawn→Probe round-trip is E5.3b-4. `cargo test -p mvm-backend` (328) green (no codesign SIGKILL this run), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. **Deferred (tracked):** the `mvm-audit-signer` binary must ship beside `mvmctl` in release like `mvm-substitution-endpoint` does (the resolver already looks there); seccomp/setpriv/cgroup/`pdeathsig` + cosign verify-then-exec + signed config envelope ride E5.3b-2/Plan 128. Stacked on E5.3b-0. Remaining E5.3b: -2 broker spawn + `ServiceCallCtx` enrichment (unblocks the round-trip), -3 veneer, -4 live E2E.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-2 — per-VM `mvm-broker` spawn** (the slice that **unblocks the guest→broker→signer round-trip**). New `mvm-backend::broker_spawn` + a shared **`mvm-backend::service_spawn`** core extracted from E5.3b-1 (resolve binary → `setsid`-detach → pipe JSON config on stdin → wait for the UDS bind → PID → `reap_service` + a generic `ServiceGuard` carrying a `fn(&Path)` reaper) — the audit-signer was **refactored onto it** in the same change (reuse-first, no forked spawn logic). The broker binds the **backend-shaped** vsock-port socket the VMM bridges the guest's `connect_host_vsock(BROKER_PORT)` to: `vm_vsock_port_socket(name, BROKER_PORT)` on libkrun (direct-bind — libkrun proxies the dial straight to the socket) / `vm_vz_vsock_port_socket(name, BROKER_PORT)` on vz (the vz supervisor splices to it) — passed by the caller; everything else (config, audit_signer wiring) is backend-agnostic. Hand-built broker `SubprocessConfig` JSON (mvm-backend below mvm-hostd, same pattern) carries `uds_path`=listen socket, `audit_signer_uds_path`=`audit_signer_uds_path(state_dir)` (so `HostAuditV1Handler` forwards `host.audit.v1` to the E5.3b-1 signer — it only registers when that path is set), and the host signer **public** key path (`signing_key_path.with_extension("pub")`; carried for the secrets-dispatcher composition, unused by host.audit.v1). Gated on a tenant, spawned **after** the audit-signer, fail-closed via `ServiceGuard`, wired into `LibkrunBackend` + `VzBackend` start/stop. 10 spawn-module unit tests (no-tenant→defused-no-op for both services; broker config has the required keys + values; audit-signer config has exactly the 6 keys; services-dir paths; generic resolver bad-env reject; `ensure_private_dir` 0700; reap idempotency ×3). `cargo test -p mvm-backend` green (332 passed; the 1 `base::runtime_meta::round_trip_detached_sealed` blip is a pre-existing process-global HOME-env race — passes isolated), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. **Deferred → E5.3b-2b:** `ServiceCallCtx` enrichment — `broker/server.rs` builds a stub ctx (`session_id: "w1a-stub-session"`, `profile: Dev`); the round-trip works (the entry just records the stub session), so threading the real session/profile from the admitted plan + the host-authoritative correlation rewrite is the recorded-entry correctness pass (claim-12 binding-gated dispatch stays Plan 128). Also deferred: ship `mvm-broker` beside `mvmctl` in release; spawn moat hardening (with the audit-signer's). Stacked on E5.3b-1. Remaining E5.3b: -2b ctx enrichment, -3 veneer, -4 live E2E.
