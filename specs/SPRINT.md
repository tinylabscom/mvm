...
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.2 — typed `host.audit.v1` methods** (Layer 2, on the E5.1 transport). **Surprise on grounding: the host side is already built** — `mvm-hostd`'s `HostAuditV1Handler` forces `category: workload_audit`, enforces the 4 KiB per-record cap (→ `BadRequest`) and the 20/s/workload token bucket (→ `RateLimitExceeded`), implements `emit` + `emit_batch`, and chain-signs via `mvm-audit-signer`; the wire payloads (`EmitRequest`/`EmitResponse`/`EmitBatchRequest`/`EmitBatchResponse`) already live in `mvm-core::protocol::host_audit`. So E5.2 is purely the **guest half**: `mvm-guest::host_audit` `emit`/`emit_batch` (+ `_on` stream variants for testing) build the `host.audit.v1` `ServiceCall` over `broker_client` and map the host's typed `ServiceErrorCode` onto a typed `AuditError` (`RateLimited` / `BadRequest` / `Unavailable` / `Service{code}` / `Transport`) the veneer can turn into distinct exceptions. **Claim 8 is structural, not advisory**: `EmitRequest` has no `category` field, so a workload literally cannot express a host category, and the host stamps `workload_audit` regardless — the guest can't spoof it even by bypassing the SDK. The 4 KiB cap + 20/s limit stay **host-enforced** (single source of truth); the guest only surfaces the host's refusal as the typed error (no client-side cap to drift). `correlation_id` is a placeholder (atomic counter, no new dep) the supervisor reassigns at ingress. RED-first, 7 mock-broker unit tests over `UnixStream::pair()`: envelope shape (`host.audit.v1`/`emit`, **no category key**, parses as `EmitRequest`) + chain_head return, `BadRequest`/`RateLimitExceeded`/`Unavailable`/other-code → typed `AuditError`, `emit_batch` envelope + per-entry statuses, distinct minted ids. `cargo test -p mvm-guest` green (0 failures), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Stacked on E5.1. Remaining: E5.3 PyO3/napi veneer + live-VM E2E (box; needs the supervisor guest-facing `BROKER_PORT` listener wired into `host_listen_ports`), E5.4 `host.time.v1`/`host.cost.v1`.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.4 — typed `host.time.v1` / `host.cost.v1` methods** (Layer 2, on the same E5.1 transport, mirroring E5.2). **Grounding surprise the other way: the host side is NOT built** — only `HostAuditV1Handler` exists; `mvm-broker` says the time/cost handler scaffolds "land later", and there are no `host_time`/`host_cost` wire-type modules in `mvm-core::protocol`. The one concrete shape is the committed broker-proxy mock: `host.time.v1::now` → `{"wall_ms": u64}`. So this slice both **establishes the wire contract** and **adds the guest half**: new `mvm-core::protocol::host_time` (`TimeNowResponse { wall_ms }`) + `mvm-core::protocol::host_cost` (`CostReport { spent_micros_usd }`) — int-only money/clock per plan-104's "time/cost are ints", `#[serde(deny_unknown_fields)]`, sole definition today and reused unchanged when the host handler lands; and `mvm-guest::host_time` (`now` + `now_on`) + `mvm-guest::host_cost` (`workload`/`tenant` + `_on` stream variants) building the `ServiceCall` over `broker_client` and mapping `ServiceErrorCode` onto a typed `TimeError` / `CostError` (`NotBound` / `Unavailable` [= `Unavailable`|`NotReady`] / `Service{code}` / `Transport`, plus `NotImplemented` for the mvmd-delegated `host.cost.v1::tenant` verb). **Scope is the verb**, so the request body is empty (`{}`) — a workload can't ask for another workload's or tenant's spend; identity is the supervisor's connection-derived `ServiceCallCtx`, not the payload. Advisory like every broker client (no key, bare `ServiceCall`, host gates everything). RED-first, 18 unit tests: 4 core serde (roundtrip + `wall_ms` wire-shape decode + unknown-field reject for each), 14 guest mock-broker over `UnixStream::pair()` (envelope shape `host.time.v1::now` / `host.cost.v1::workload`+`tenant` with empty payload + value return, `NotBound`/`Unavailable`/`NotImplemented`/other-code → typed error, malformed-payload → `Transport`, distinct minted ids). `cargo test -p mvm-core` + `-p mvm-guest` green, doctests green, clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Stacked on E5.2. The PyO3/napi veneer for all three services (`mvm.audit.*` / `mvm.host.time()` / `mvm.host.cost()`) rides E5.3b-3; the live round-trip is gated on E5.3b (per-VM broker/audit-signer spawn).
