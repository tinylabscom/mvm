...
- [ ] Plan 200 auth/security proof follow-up: persistent-machine `ssh_agent` is explicit in signed/admitted policy metadata, receipts, dry-run, and audit, while remaining dev-tier/socket-only with no SSH key or known-host material copied or mounted. The 2026-06-19 Firecracker-host proof now reaches a running dev-profile `alpine:latest` persistent machine and captures honest auth metadata in dry-run, receipt, and audit output. Host-side raw SSH-agent proof succeeds through both the throwaway `SSH_AUTH_SOCK` and the spawned per-machine proxy UDS, but the in-guest raw probe against `/run/mvm/ssh-agent.sock` still fails with `Connection reset by peer`; the narrowed remaining live gate is Firecracker guest-to-host host-listen forwarding for dev `SSH_AGENT_PORT` 5301. Follow-up code now routes Firecracker through the same per-port runtime UDS transport used by the host-listen bridge and unit-tests that backend selection, but the live in-guest raw probe still needs to be rerun before the smoke is claimed. Non-standard-port SSH-banner denial still needs a runtime classifier.
