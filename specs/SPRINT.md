...
- [~] **P3** — the governed egress seam. Design SIMPLIFIED (`bf3eac389`, doc 11): NO new transport — wasm egress is just a `WireRequest` client (`mvm_core::substitution_wire`) over the existing `Uds` to the SAME substitution endpoint the microVM backends use (faithful by construction). Reachability recon (`127b22e44`) settled it: governance lives in mvm-hostd (unreachable in-process from WasmBackend in mvm-runtime), so route through the subprocess endpoint, not a parallel in-process path.
  ...
  - [ ] **P3b.2** — the **data-governance witness** (the POC gate; cross-crate, needs a home ABOVE both `mvm-runtime` [WasmBackend] and `mvm-hostd` [SubstitutionService] — likely mvm-cli / conformance / a workspace integration test): spawn a real `WasmBackend` + real endpoint, a WASI module egresses a `${NAME}`-bearing `WireRequest` to a bound (allowed) + unbound (denied) dest → assert deny-by-default, allow-by-policy, a chain-verifying audit entry, and the module never holds the secret value — the same witness the microVM backends pass. Then RELAX the `NetworkingNotSupported` gate to permit governed egress. (Full TLS-terminating substitution for HTTPS dests → P3c.)
