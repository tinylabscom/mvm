...
- [ ] **In review (PR #868 `feat/vz-up-down-sub-second`, CI-green, awaiting merge)** — vz dev/builder VM **orphan-reaper correctness** + a **cache-prune warm-store guard**, surfaced while validating that PR's `up` orphan-sweep rewrite. The startup sweep (`sweep_orphaned_vm_helpers_on_startup`, run on every `up`/`dev up`/`ls`) classified *any* launchd-parented helper as an orphan and SIGTERM'd it — but the persistent dev builder (`mvm-persistent-builder-vz-dev`) and named workloads are detached under launchd **by design** to outlive the CLI, so the sweep killed the running VM on every invocation (the SIGTERM hit a headless guest with no host-side SIGKILL escalation, so the guest never stopped and the next call just re-signalled it). Fix (`9c1d83058`): gate reaping on the **supervisor's liveness**, not its parent — two phases per dir, where an alive supervisor on a *managed* dir (persistent builder / named workload) is the running VM (spared) while an *ephemeral* per-job builder's launchd-parented supervisor is a crashed build (reaped); a dependent helper's fate then follows its supervisor (a live VM keeps all its PIDs, a stopped VM's leaked gvproxy is still reaped). Related guard (`d536c3c9`): the same managed/ephemeral split stops `cache prune` deleting the persistent builder's warm Nix store when its supervisor is stopped (which would force a full nixpkgs re-fetch on the next `dev up`) — managed dirs are never auto-removed, only ephemeral scratch dirs. 12 reaper tests (each teeth-checked by reverting the guard); `cargo clippy -p mvm-cli --all-targets -- -D warnings` + full `mvm-cli` lib (943) + nightly fmt + `check-no-spec-refs-in-comments` clean. `crates/mvm-cli/src/commands/env/dev_vz.rs`-only.
- [ ] **In review (PR #868, CI-green)** — Firecracker `stop_vm` brought to the same **sub-second down** behavior as the Vz path: the unconditional `sleep(2s)`-then-kill is replaced by a 250 ms graceful window polled at 25 ms (ACPI `SendCtrlAltDel`, then `firecracker::is_vm_running` until exit), escalating to the hard `kill $(cat pid)` only when the guest ignores the graceful request — a headless workload guest has no power-button handler, so the old path paid 2 s on every `down` for nothing. `crates/mvm-backend/src/microvm.rs`. The escalation decision is extracted into a pure `await_graceful_exit(max_wait, poll_interval, is_running, now, sleep)` helper — probe/clock/sleep injected — so it's unit-tested without a live guest or wall-clock waits: 3 tests (exits-in-window, timeout-with-bounded-polls, probe-error-propagates), all green. `cargo clippy -p mvm-backend --all-targets -- -D warnings` clean. **Live-validated on the x86_64 Linux/KVM box** (Firecracker v1.14.1): `sleeper` workload `up`→`down` cycled three times under `--hypervisor firecracker`; warm `down` measured **0.78 s / 0.84 s / 0.86 s** (vs the old deterministic 2 s sleep floor), VM `Running` between cycles. The `up` wall-clock there is dominated by the builder-VM/nix **build leg** (~30 s warm), not the boot/teardown this change targets.
- [ ] **In review (PR #868, CI-green)** — Firecracker **flake `up` of a kernel-less mkGuest workload** now works: `resolve_vz_workload_kernel` → **`resolve_workload_kernel`**, and its cached-builder-VM-kernel fallback is broadened from `vz`-only to also cover `firecracker`. mkGuest images ship no kernel (libkrun supplies its own; firecracker boots them on the cached builder kernel — `expectedBootMs = 300`); the firecracker *direct/manifest* path already did this fallback, but the *flake* path went through the vz-gated resolver and refused with `read kernel <build>/vmlinux: No such file`, so a `sleeper`-style flake couldn't boot under firecracker at all. Found + fixed while live-validating the firecracker down path on the box (the `sleeper` boot above depends on it). 1 new unit test (`firecracker_missing_kernel_falls_back_to_builder_vm_cache`) beside the existing vz ones; call sites in `up.rs` + `checkpoint.rs` renamed. `crates/mvm-cli/src/commands/vm/{up,checkpoint}.rs`. `cargo test -p mvm-cli --lib resolve_workload_kernel` (5) + nightly fmt + `cargo clippy -p mvm-cli` clean.
- [ ] Authored [`plans/198-host-side-flake-build-cache.md`](plans/198-host-side-flake-build-cache.md) — the measured **why warm `up` is not yet sub-second**. Box profiling of `up --flake examples/sleeper --hypervisor firecracker` (warm): the firecracker microVM boots in **~70 ms** (already sub-second), but `up` spends **~29 s** in the build leg — a single-shot builder VM boots and runs `nix build` on **every** invocation, printing "Cache hit … discarding staged build" *after* the full nix eval, because the cache key (the nix revision) is only knowable post-eval. A warm/persistent builder (Plan 89) saves only the ~1.5 s VM boot, not the ~25 s eval. Confirms Plan 139's earlier "99 % build" finding; the gap is untracked (Plan 76's warm cache is the `--artifact` path, Plan 93 fingerprints the *builder* image). Fix: a host-side **input fingerprint → cached revision** short-circuit (modeled on Plan 93's `builder_vm_source_fingerprint`, with the same stale-image footgun discipline) that skips the builder VM + nix entirely on an unchanged flake; secondary lever trims the ~1.3 s config/secrets drive creation via `mkfs.ext4 -d` populate-at-format.
- [x] **Implemented + box-validated** [`plans/198-host-side-flake-build-cache.md`](plans/198-host-side-flake-build-cache.md). `crates/mvm-build/src/pipeline/build_cache.rs`: `workload_build_fingerprint` hashes every nix input host-side (user-flake tree + the filtered workspace tree that covers `path:/work/nix` + the buildRustPackage `src` + profile + mode + mvmctl version), with a drift guard keeping `EXCLUDED_BASENAMES` ⊆ `workspace-filter.nix` (hashing a superset of nix's inputs is sound). `fingerprint → revision` records under `~/.mvm/dev/build-cache/` (atomic, traversal-guarded); `dev_build` short-circuits the builder VM when the record maps to a build dir whose `rootfs.ext4` exists (`MVM_NO_BUILD_CACHE` disables). Secondary: config/secrets drives populate-at-format via `mkfs.ext4 -d` (no loop mount/sudo). **Box result (x86_64 KVM, FC v1.14.1, `examples/sleeper`):** warm `up` **30 s → 1.01 s** with the builder VM skipped entirely (`Build cache hit — reusing … (skipped builder VM + nix eval)`), VM still `Running`; a flake edit correctly busts the cache (rebuild, 32 s). 13 unit tests; `cargo clippy` + nightly fmt clean. `crates/mvm-build` + `crates/mvm-backend`.
- [x] Implemented [`plans/199-host-runtime-packaging-and-crate-boundaries.md`](plans/199-host-runtime-packaging-and-crate-boundaries.md) Workstream A and advanced release-install policy tracking: added an optional source-built Nix host package for `mvmctl`, exposed it as `packages.<system>.mvmctl` / default plus an overlay without changing the Linux-only `mkGuest` image API, added tests that reject project-release binary download paths while keeping native libkrun linkage explicit/opt-in, and documented binary install as the primary user path with host Nix explicitly optional. Remaining Plan 199 work: native VMM Nix recipes, release artifact matrix/signature CI, and the crate-boundary audit.
- [ ] Authored and docs-synced [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md): a concrete plan for a first-class `mvmctl machine` UX layer over the existing runtime primitives. The plan records the adopted packaging/UX strategy from the reference review: binary-first install, optional source-built Nix, current image-backed one-shot docs before flake/manifests, future `machine run` DX, explicit `--net` / `--allow-host` networking, persistent named machines, friendly `exec`/`shell` wrappers, SDK parity, verified portable artifacts, optional SSH-agent forwarding, and measured hot-start latency targets, while preserving deny-by-default network posture, OCI provenance, admission/audit, worktree-isolated data dirs, and the builder-VM boundary. It tracks `mvm.toml` schema v2 for machine workflows: `image` means an OCI-backed machine, `flake` means the existing flake-backed build flow, both are mutually exclusive for now, unknown keys are rejected, dev init/auth/volumes/network posture are security-gated, effective policy appears in admission/audit/receipts, portable artifacts verify before launch, elastic memory is messaged as cap vs initial commitment, GPU stays future-only, and beginner UX converges on `machine run/create/start/exec/shell/stop/pack`. The plan captures the broader session synthesis: Nix remains a maintainer/source-build tool rather than a user prerequisite, the managed macOS virtualization backend stays the safer default, custom kernels are modeled as signed runtime/artifact payloads, portable artifacts are product surface, SDKs mirror the CLI, and dependency weight is a first-class DX/security goal measured by default binary closure rather than raw crate count. New dependency work tracks replacing or forking `oci-client`, keeping builder/dev/backend extras out of the normal machine-run path, replacing heavy test-only HTTP fixtures, auditing CLI UI deps, freezing native bindings, preserving security-critical deps, and adding closure/duplicate-major/heavy-dep/binary-size CI budgets. The de-duplication pass now makes Plans 199/200 the priority product path: Plan 199 owns install/host packaging; Plan 200 owns beginner machine UX; Plan 125/114 feed SDK primitives; Plan 126/156 own dependency and size mechanics; Plan 155/136 feed portable-artifact internals; Plan 159/189 stay VZ-specific; Plan 193/197 stay security substrate; Plan 198 is completed perf input.
