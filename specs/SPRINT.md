...
- [ ] Route the remaining bypass sites through `mvm-core::config`; add the anti-bypass CI lint. _(WS4.2 — includes the ~10 hand-rolled `mvm_home()+"vms"` joins → `vms_dir()` (review DRY catch), the 2 stale `mvm-protocol` comments, and the dead in-VM echo round-trip in `microvm.rs::resolve_vm_dir`.)_
