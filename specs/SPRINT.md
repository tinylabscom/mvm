...
- [ ] 1d `mvm-net`: fold `mvm-network` + host tunnel/gateway/dns + guest net; vsock/UDS transport + egress seam. _(crate rename `mvm-network`→`mvm-net`, the bounded/fuzzed DNS codec, strict answer guard, pins-first `MVM_DNS/1` host handler + async/Linux-blocking dispatch, and metadata-only chain-signed per-query audit landed; rate-limit/tunnel/guest-net absorption pending; 2,562 core+hostd tests green)_
