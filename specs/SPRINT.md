...
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) WS-C persistent spec substrate — **named machine specs are implemented for non-booting verbs**. Added `<MVM_DATA_DIR>/machines/<name>/machine.json` helpers in `mvm_core::config`, `MachineSpec` strict JSON, atomic writes, safe-name validation before path use, and `mvmctl machine create` / `ls` / `inspect` / `rm --yes`. Tests cover parser routing, state persistence under isolated `MVM_DATA_DIR`, overwrite refusal without `--force`, sorted listing, strict unknown-field rejection, and confirmed deletion. Lifecycle verbs (`start`/`exec`/`shell`/`stop`), full `mvm.toml` machine mapping, SDK parity, docs, portable artifacts, and CI budgets remain.
