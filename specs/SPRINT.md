...
- [x] 2026-07-08 source-checkout `machine start` / Stage 0 dev-DX hardening: `cargo run -- ...` no longer prints the build-script stub warning ahead of normal CLI output, source checkouts now build the workload kernel locally on a cold cache instead of fetching the versioned release artifact, and stubbed embedded host binaries now fall back to an on-demand `cargo zigbuild` from the source checkout for Stage 0 / builder-boot paths instead of requiring `MVM_EMBED_BINARIES=1`. Validation: `cargo run -- machine ls`; targeted `mvm-cli` tests for the source-checkout workload-kernel bootstrap decision and host-binary source-build argv; `cargo check --workspace`; `cargo clippy --workspace --all-targets -- -D warnings`. `cargo test --workspace` still ends on three pre-existing `mvm` failures outside this slice.
