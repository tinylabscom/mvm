...
- [~] **P3** — the governed egress seam. Design SIMPLIFIED (`bf3eac389`, doc 11): NO new transport — wasm egress is just a `WireRequest` client (`mvm_core::substitution_wire`) over the existing `Uds` to the SAME substitution endpoint the microVM backends use (faithful by construction). Reachability recon (`127b22e44`) settled it: governance lives in mvm-hostd (unreachable in-process from WasmBackend in mvm-runtime), so route through the subprocess endpoint, not a parallel in-process path.
  ...
  - [ ] **P3b.2** — the **data-governance witness** (the POC gate). **FULL PLAN WRITTEN: `specs/plans/13-ws11-wasm-egress-poc.md`** (5 TDD tasks, execution-ready). Findings baked into the plan: the witness lands as a `mvm-runtime` integration test (`tests/wasm_egress_witness.rs`) — that crate can *spawn* the endpoint bin via `spawn_substitution_endpoint` without depending on mvm-hostd (resolving the cross-crate concern); HTTP-only mock destination (TLS→P3c); Task 1 relaxes the `NetworkingNotSupported` gate to governed egress; Tasks 3–4 assert allow-by-policy + host-side substitution (dest sees the real secret, module never does) + deny-by-default + chain-verifying audit. (Full TLS-terminating substitution for HTTPS dests → P3c; browser → P4 — both flagged in the plan as their own subsequent plans.)
