...
- [ ] **Remove ssh-agent forwarding entirely — no SSH anywhere (core promise; ADR-001).** The tree carries a dev-tier ssh-agent-forwarding feature (host `$SSH_AUTH_SOCK` → vsock port 5301 → guest `/run/mvm/ssh-agent.sock`) that contradicts the "no SSH in microVMs, ever" promise and hands the guest the host's whole ssh-agent (bypassing bound-destination secret substitution). Delete the whole surface: the `ssh_agent` spec field, manifest `[auth] ssh_agent`, `AuthMode::SshAgentSocket` + `AuthPolicy` (drop the enum/struct entirely — `None` is all that's left), the proxy (`mvm-cli::commands::ssh_agent_proxy`, `SSH_AGENT_PORT`, `run_ssh_agent_proxy_*`), the guest `SSH_AUTH_SOCK`/`/run/mvm/ssh-agent.sock` injection, tests, and CLI docs. **Strengthen ADR-001 to absolute** — delete the "SSH-agent forwarding, when offered…" carve-out. Add a `check-no-ssh` CI symbol gate (sibling to `prod-agent-no-console`). The one dev interactive surface stays the builder-VM shell — nothing SSH.
