...
> rollout is complete and locally revalidated. Guest-executed runtime binaries now live in
...
> rerun on `88.99.197.234` from a fresh source snapshot. Older
...
- [x] 2026-07-08 Plan 213 §I builder-pack revocation consumer: the installed
      ...
      `~/.cache/mvm/pack-revocations/` with the same 24-hour refresh / 7-day
      offline tolerance / `404` bootstrap rules as the dev-image recall list,
      and unions fetched entries with the operator's local `pack-trust.json`
      revocations. Validation: focused `pack_revocation_cache_tests` under
      `mvm-cli` plus the PR's green CI matrix.
- [x] 2026-07-08 gvproxy-specific test retirement: the remaining gvproxy-only
      bridge witnesses are removed from
...
- [x] 2026-07-13 embedded-host-binary production cleanup: source builds no longer carry a live zero-byte-stub/embed-bypass mode, `mvmctl` now always embeds the real Stage 0 / host / guest runtime binaries up front, the bootstrap helper no longer injects `MVM_SKIP_EMBED_BINARIES` or clears `MVM_EMBED_BINARIES`, and the old source-checkout host-binary rebuild fallback has been removed from `mvm-cli`. The manifest parser in `build.rs` was tightened at the same time so `BOOTSTRAP_SUPPORT_BINARIES` entries like `mvm-egress-client` are no longer accidentally swept into the `-p mvm-build --bin ...` loop. Validation: `cargo fmt --all --check`; targeted `mvm-cli`/`mvm-build` tests for host-binary extraction, helper selection, bootstrap-helper env shape, and Zig autodetect; `cargo check -p mvm-cli -p mvm-build --all-targets`; `cargo clippy -p mvm-cli -p mvm-build --all-targets -- -D warnings`.
- [x] 2026-07-08 Linux source-checkout guest-helper build fix: `mvm-oci-init`
      now passes the loopback `SIOCGIFFLAGS` / `SIOCSIFFLAGS` request codes to
      `libc::ioctl` as `libc::Ioctl`, matching the current libc signature and
      unblocking Linux-musl guest-helper builds in source checkouts. Validation:
      `cargo fmt --all --check` in the worktree; on the Hetzner Linux/KVM host,
      `cargo test -p mvm-guest --bin mvm-oci-init`, `cargo clippy -p
      mvm-guest --bin mvm-oci-init -- -D warnings`, and `cargo build --bin
      mvmctl` all green, and the rebuilt `mvmctl machine run --image alpine
      --name issue1462n-base --agent-verb ping -d --hypervisor firecracker`
      path boots again.
...
- [x] 2026-07-09 HVF host-vsock-proxy availability fix for OCI `--allow-host`: macOS backend selection no longer keys HVF OCI egress support off the parent `mvmctl` process holding the hypervisor entitlement. Instead, the HVF backend now advertises `{ no_guest_nic, host_vsock_proxy, virtiofs_root }` only when the detached `mvm-hvf-supervisor` workload path is actually launchable on this host, and explicit `--hypervisor hvf` validation now fails closed early when that path is unavailable. That preserves the PR #1600 ordering guarantee: the refusal still happens before OCI pull, kernel acquisition, or VM boot work, but a launchable supervisor no longer gets rejected just because the caller itself cannot pass `hv_vm_create`. Docs now call out the macOS OCI `--allow-host` behavior and the fail-closed troubleshooting path, and `scripts/check-hvf-oci-allow-host-smoke.sh` packages the live proof (`machine run --image ... --allow-host ...` plus admit/deny relay evidence) behind `just hvf-oci-allow-host-smoke`. Focused validation green: `cargo test -p mvm-backend hvf_backend --lib`; `cargo test -p mvm-cli exec::tests::validate_backend_for_egress_refuses_unavailable_hvf_before_boot_work --lib`; `cargo test -p mvm-cli exec::tests::select_backend_name_for_egress_picks_hvf_when_proxy_support_is_available --lib`.
...
- [x] 2026-07-08 guest/host reconnect backoff hardening: the guest's host→guest
      attach path (`connect_to_port`) and guest→host vsock dial path
      (`connect_host_vsock`) now treat short restart races as transient and
      retry them with bounded exponential backoff (100 ms base, 500 ms cap, 4
      attempts total) instead of only retrying read timeouts. The per-tenant
      host-agent control client (`send_control`) now does the same while a
      wrapped worker is restarting, including a truncated reply / early EOF.
      The retry boundary stays at connect/setup only: established request
      streams are not replayed. Deterministic restart coverage now exercises
      both sides directly: guest `CONNECT` retries across a listener restart
      before the `OK` ack, and host-agent registration retries across a control
      socket restart mid-request. Validation from the branch worktree:
      `cargo test -p mvm-guest --lib
      vsock::tests::test_transient_connect_errors_include_restart_races -- --exact`;
      `cargo test -p mvm-guest --lib
      vsock::tests::test_connect_to_port_retries_across_listener_restart_before_connect_ack -- --exact`;
      `cargo test -p mvm-backend --lib
      host_agent_spawn::tests::transient_control_errors_cover_restart_races -- --exact`;
      `cargo test -p mvm-backend --lib
      host_agent_spawn::tests::register_vm_retries_across_control_socket_restart -- --exact`;
      `cargo clippy -p mvm-guest -p mvm-backend --tests -- -D warnings`.
...
- [x] Started
      ...
      work inside `nix/images/kernel/{base,builder,workload}.nix`: surface the
      resolved builder/workload configs and metrics cleanly, land the
      workload-first removals plus shared decompressor cuts, then finish with
      measured Linux-side validation, the workload size-mode comparison, and a
      ratcheted kernel budget gate.
...
- [x] Advanced [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md)
      ...
      now threads network policy, `mem_initial`, validated volumes, dev-init
      execution, and dev-tier `ssh_agent` socket forwarding through the
      admitted OCI-backed launch path. Manifest-relative volume host paths are
      resolved from the manifest directory, read-only remains the default unless
      `:rw` is explicit, and create rejects `dev.init` outside `--profile dev`.
      Docs updated: quickstart, CLI reference, and manifests guide. Focused
      verification: `cargo test -p mvm-cli commands::machine` (29 passed).
- [x] Advanced [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md)
      C2 SDK parity for Python and TypeScript. Both SDKs now expose
      `Machine.run/create/start/exec/shell/stop` wrappers that shell only to
      `mvmctl machine ...`, preserving CLI admission/audit ownership instead of
...
- [x] Advanced [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md)
      ...
      `mvmctl run` dry-run/preflight and receipt-summary path, proving
      default-deny and `--allow-host` network posture match the CLI without
...
- [x] Advanced Stage 0 bootstrap performance on the Plan 200 auth-proof branch.
      ...
      slow cold materialization path. Firecracker-host measurement at
      `156391a4` with isolated cache/data dirs: cold `build image` cache miss
      reached `Fetching Stage 0 bootstrap assets … 0.7s` and `Materializing
      Stage 0 root dir … 1.7s`; immediate warm rerun reached `Fetching … 0.1s`
      and `Materializing … 0.1s`. The current Nix seed has no `mkfs.ext4`, so
      the branch now prepopulates the dedicated libkrun
      `nix-store-stage0-<arch>.img` host-side when host `mkfs.ext4` is
      available, marks it with a `.stage0-seed` sidecar, and lets Stage 0 PID 1
      adopt that prepopulated store instead of copying to tmpfs. Bounded
      Firecracker-host libkrun proof logged host prepopulation
      (`blocks_4k=16777200`, sparse image + sidecar written) before a 180s
      timeout; full in-guest adoption timing remains gated.
...
- [x] Landed template-identifier path-traversal hardening from the 2026-06-16
      prior-art audit note: legacy `template_load` and `template_create` now
      validate template names at the path dereference/write chokepoints, while
      `manifest export-oci` validates its legacy-name fallback before dispatch.
      Regression coverage proves traversal names reject cleanly, valid legacy
      names still round-trip, and 64-char slot hashes still resolve through the
      slot path rather than the 1–63 char legacy-name validator.
...
- [x] Advanced [`plans/199-host-runtime-packaging-and-crate-boundaries.md`](plans/199-host-runtime-packaging-and-crate-boundaries.md) Workstream B: added pinned source-built Linux host recipes for `libkrunfw` v5.5.0 (including Linux 6.12.91 source) and `libkrun` v1.18.1 (including Cargo vendor hash), exposed `libkrunfw`, `libkrun`, and opt-in `mvmctl-native-libkrun` package/overlay attrs without changing `packages.default`, and documented the expert Nix path. Focused structural coverage proves the recipes are pinned source builds, default mvmctl remains non-native, `BLK=1`/`NET=1` stay enabled, and project release binaries remain banned. Builder-VM `nix flake check` / Linux package builds remain the verification follow-up.
- [ ] Added [`adrs/089-builder-vm-resident-control-plane.md`](adrs/089-builder-vm-resident-control-plane.md) and [`plans/204-builder-vm-resident-control-plane.md`](plans/204-builder-vm-resident-control-plane.md): long-term builder execution moves behind a resident `mvm-builderd` service inside the builder VM, reached by typed vsock requests from host `mvmctl`. This preserves the simple one-binary host UX, keeps host Nix optional for normal use, makes build/eval progress and provenance structured, and explicitly keeps host/builder tools out of workload guest images.
  - [x] Workstream A protocol pair: `mvm_build::builderd_protocol` defines the typed, allowlisted `BuilderRequest`/`BuilderResponse` wire (`Handshake`/`Probe`/`FlakeCheck`/`BuildGuestImage`/`BuildHostTool`/`PrefetchSource`/`QueryStorePath`/`CancelJob` → `Accepted`/`Progress`/`LogChunk`/`ArtifactReady`/`StorePathReady`/`Completed`/`Failed`/`Cancelled`) — no generic shell variant. Each operation carries an `OperationId`; `Failed` carries a stable `FailureCategory`; version negotiation (`PROTOCOL_VERSION` + `negotiate()` + `handshake_reply()`) is exact-match v1 and refuses unknown versions fail-closed. Externally-tagged snake_case + `deny_unknown_fields` everywhere, inheriting the 256 KiB `mvm_guest::vsock` framing, so unknown peer fields/kinds reject rather than silently drop. Lives beside the legacy controlled-shell-job `builder_protocol` channel (Plan 204 keeps that as the WS-D compatibility adapter). 26 unit tests (roundtrip, kind/category tag stability, unknown-field + unknown-kind rejection, version refusal); `cargo clippy -p mvm-build --all-targets -- -D warnings` + fmt clean.
  - [~] Workstream A daemon request-handling core: `mvm_build::builderd` adds stateless `dispatch()` (serves `Handshake`/`Probe`/`CancelJob`; recognized-but-unimplemented build ops fail closed with the new `FailureCategory::Unsupported`) and `serve_connection()` — the framed read-dispatch-write loop over the 256 KiB `mvm_guest::vsock` framing, returning `Ok` on clean EOF. Cross-platform and driven from `UnixStream` pairs in 9 tests, so the daemon core is exercised without booting the builder VM. The bin entrypoint + Linux AF_VSOCK listener are deferred to land with the builder-VM boot wiring (a listener with no boot path is untestable dead code). clippy `-D warnings` + fmt + `check-no-spec-refs-in-comments` clean.
  - [x] Workstream A doctor readiness: host-side `probe_builderd_readiness` (connect + typed `Handshake` → `BuilderdReadiness::{Ready,VersionMismatch,NotRunning,Unreachable}`), `readiness_summary`, and `builderd_control_socket_path` over the new `BUILDERD_CONTROL_PORT` 21473 in `mvm_build::builderd`; `mvmctl doctor` gained an informational `builder daemon` platform check scanning the persistent builder-VM `vms/` root and probing each present control socket with a 300 ms bounded handshake (always `ok` — absence is the normal "builder VM down" state, mirroring the Plan 202 `host-agent daemon` check). Tested end-to-end against a real `UnixListener` driving `serve_connection` (no VM boot); doubles as the first WS-B host-client leg. 43 builderd tests across mvm-build + mvm-cli; clippy `-D warnings` + fmt clean.
  - [x] Workstream B host client: `mvm_build::builderd_client::BuilderdClient` — `connect()` (over the shared `connect_with_timeout` + `perform_handshake` factored out of the readiness probe), one-operation-per-connection `run_operation()` (writes the typed request, correlates every response frame to the request `OperationId`, streams `OperationEvent::{Progress,Log}` to a caller sink, returns typed `OperationOutcome::{Artifact,StorePath,Completed,Failed,Cancelled}`), `request_cancel()`, and typed `BuilderdClientError::{NotReady,VersionMismatch,Transport,Timeout,Protocol}` (read-timeout → `Timeout`, missing/refused socket → `NotReady`). Transport-only — starts/stops no VM and does no git, so the WS-B data-dir-isolation + git-host-side boxes stay with the lifecycle owner; mid-flight async cancel from a second handle lands with the listener. 11 client tests (every streamed/terminal/error path via `UnixStream` pairs + a live integration against the real `serve_connection`); full mvm-build suite 614 green; clippy `-D warnings` + fmt clean.
  - [x] Workstream C FlakeCheck typed-op core + Workstream A §5 structural gate: `mvm_build::builderd` gains `flake_check_argv` (`nix flake check --no-build path:<flake>`), `flake_check_outcome` (clean exit → new `BuilderResponse::Completed` terminal; non-zero → `FailureCategory::NixEval`; spawn error → retryable `Internal`), an injectable `OpExecutor` (`CommandExecutor` for the daemon, fakes for tests), `dispatch_with_executor`, and `serve_connection_with_executor`; the protocol + client grew the `Completed` terminal/outcome (pre-release, no back-compat). Real in-VM `nix` exec is boot-gated; every path around it is unit-tested. `xtask check-guest-images-no-builder-tools` (comment-stripping source-grep over `nix/lib/mk-guest.nix`, wired into `ci.yml`) proves mkGuest bakes no `mvmctl`/`mvm-builderd`. 60 builderd tests + 2 xtask tests; clippy `-D warnings` + fmt clean. Remaining and **boot-gated** to one on-box slice: the `mvm-builderd` bin + AF_VSOCK listener + rootfs baking + boot launch + lifecycle owner (the `mvm-build` lib pulls `reqwest`/`tokio`, so the bin must `#[path]`-include + musl-cross-compile — unverifiable off-Linux); then build-image/host-tool ops (WS-C), compat shrink (WS-D), docs (WS-E). **Over-the-wire transport live-proven 2026-06-20 (macOS-26 Vz):** a new `crates/mvm-build/examples/builderd-flakecheck.rs` diagnostic (first `BuilderdClient` consumer) drove a real `FlakeCheck` over the live `vsock-21473.sock` → handshake v1 → typed `Failed{NixEval}` terminal (request → in-VM `nix` exec → classification → terminal round-trips). It caught a daemon bug the fake-executor tests could not — the builder image's `nix.conf` lacks the `nix-command`/`flakes` experimental features, failing every `nix` call. Fixed: the daemon now passes `--extra-experimental-features "nix-command flakes"` on all four Nix argvs via a shared `nix_argv` helper (argv tests updated + `nix_argv` test; 75 builderd tests green). The clean-`Completed` over-the-wire proof still needs the fix in a rebuilt image + a staged flake (WS-D).
  - [~] Workstream A boot wiring (follow-up PR `feat/plan-204-builderd-bin` off #1088): `crates/mvm-build/src/bin/mvm-builderd.rs` `#[path]`-includes the daemon modules (so it musl-cross-compiles like every builder bin, not via the heavy lib) and runs a Linux AF_VSOCK accept loop on `BUILDERD_CONTROL_PORT` (21473) → `serve_connection_with_executor(&CommandExecutor)`. Embedded into the builder/dev rootfs at `/sbin/mvm-builderd` via `HOST_BINARIES` (`host_binaries/manifest.rs` + `nix/lib/mvm-host-binaries.nix`, `check-mvm-host-binaries-sync` green); `mvm-host-vm-init` PID 1 `spawn_builderd()`s it at dispatch-loop entry (non-fatal); the persistent libkrun + Vz builder launchers forward vsock 21473 to `<vm_state_dir>/vsock-21473.sock` (the exact path `doctor` + `BuilderdClient` use). All CI-compiled incl. the musl zigbuild. Daemon boot + reachability live-validated 2026-06-20 on macOS-26 Vz: `mvmctl dev up --no-shell --json` → `{backend: vz, outcome: started}`, then `mvmctl doctor` → `builder daemon: OK (mvm-persistent-builder-vz-dev: ready (protocol v1))` (a real `Handshake` over vsock 21473 negotiated protocol v1). The remaining step is a typed `FlakeCheck` over the wire, which needs the WS-D host driver. Lifecycle owner (routing real builds through the client) = WS-D, open.
  - [~] Workstream C build-op handlers (`mvm_build::builderd`): `nix_build_argv` (`nix build <ref>#<attr> --no-link --print-out-paths`) + `dispatch_nix_build` drive both `BuildGuestImage` and `BuildHostTool` → `ArtifactReady{artifact_path,store_path}` from the printed out-path (non-zero/empty → `NixBuild`, spawn error → retryable `Internal`); `prefetch_source_argv` (`nix flake prefetch --json`) + `dispatch_prefetch_source` → `StorePathReady` parsing `storePath` (retryable `Fetch` on failure); `query_store_path_argv` (`nix path-info`) + `dispatch_query_store_path` → `StorePathReady` with `already_present = exit==0`. `OpExecResult` gained a `stdout` field so store paths read back; all wired into `dispatch_with_executor` + `serve_connection_with_executor` (over-the-wire build test). 764 mvm-build tests (full `--features builder-vm` suite); clippy `-D warnings` + fmt + no-spec-refs clean. Live in-VM `nix` exec is boot-gated. **Correction:** WS-D is not owned by Plan 205 (Plan 205 is complete and carries only builder-residency work); WS-D — routing real builds through `BuilderdClient`, the compat-adapter diagnostic, the raw-shell debug gate, and the structural lint — is unimplemented, since `BuilderdClient` has no consumers yet. The WS-C "normal mvmctl flows need no host Nix" item landed (2026-06-21) as `xtask check-no-host-nix` (CI-wired in ci.yml + ci-full.yml): it flags any literal host `Command::new("nix")`/`nix-build`/… in `crates/*/src/**/*.rs`, proving every nix invocation routes through the builder VM (`shell_exec*`, in-guest) or the in-guest `mvm-builderd` daemon; the lone `platform::has_host_nix` debug probe (zero normal-path callers) carries an explicit `// allow(host-nix):` marker and new unannotated host-nix Commands fail the gate (3 unit tests). WS-D's adapter decision-seam + compat diagnostic landed (2026-06-21) as `mvm_build::builder_route` (`resolve_route` typed-vs-legacy + `MVM_BUILDERD_TYPED` opt-in + `legacy_shell_diagnostic`), wired into `persistent_builder::submit` so every legacy shell-job dispatch emits a structured `tracing` diagnostic. The first `BuilderdClient`-running consumers then landed: typed `FlakeCheck` from `mvmctl build validate` (#1201), and typed `BuildGuestImage` from `dev_build_via_builder_vm_uncached` (opt-in `MVM_BUILDERD_TYPED`, default-off) — host-readable `output_dir` export to the `/out` share (#1213) + the `dev_build` wiring (#1215). Override-input needs no wire change: the typed route is persistent-only and the persistent legacy path is itself override-free (`stage_flake_dispatch_job` builds `/work#<attr>`, drops `mounts.staged_user_flake`), so the two are equal by construction. A live typed `BuildGuestImage` on macOS-26 Vz then caught a real daemon bug — `nix` was run with no `HOME`/XDG and died on `//.cache/nix` (read-only rootfs); fixed in `builderd::CommandExecutor` (writable `HOME`/XDG contract + cache-dir creation, `daemon_command_env` unit-tested) plus a `builderd-buildimage` diagnostic. The build-specific default-flip + raw-shell debug gate are now merged (#1250): `try_typed_build` reads `build_typed_opt_in` so a guest-image build takes the typed route whenever a daemon is reachable unless `MVM_BUILDERD_RAW_SHELL` forces the legacy in-VM shell build; `try_typed_flake_check` is untouched (flake check stays opt-in via `MVM_BUILDERD_TYPED`), so the flip is build-only. Three unit tests cover the default-on/gate semantics + build↔flake-check independence. The `check-builder-shell-job-sites` allowlist stays at 4 (no drop): `persistent_builder.rs` retains the `HostVmRequest::Run` construction as the daemon-unreachable + raw-shell fallback, so it still constructs a shell job and legitimately stays allowlisted. Routing behaviour live-confirmed on macOS-26 Vz (typed attempted by default with the gate unset; straight-to-legacy with `MVM_BUILDERD_RAW_SHELL=1`). The full WS-D typed-routing chain is thus merged (#1201/#1211/#1213/#1215/#1226/#1250); the one remaining item is a verification, not a routing blocker: the literal over-the-wire artifact-equality proof — a live run on a loaded box hit exactly the documented cascade (stale-resident-builder `/work`-not-a-flake, missing `mvm-vz-supervisor` bin, stale dispatch socket, cold-cache Stage 0 `BadActivate`), none of it WS-D code; the flip merged ahead of the proof, which stays open as the per-backend equality verification (libkrun + Vz on macOS, Firecracker on KVM). `pool_build` is mvmd-scoped.
- [x] Completed [`plans/199-host-runtime-packaging-and-crate-boundaries.md`](plans/199-host-runtime-packaging-and-crate-boundaries.md): the builder-VM follow-up is now verified. `nix flake check` evaluated the `nix/` flake package set, `.#mvmctl` built and checked to `/nix/store/68xqmybxxlpckymlfqfvc1ka0x2yqvhx-mvmctl-0.16.1`, and opt-in `.#mvmctl-native-libkrun` built and checked to `/nix/store/0sg78jmbiv0yll6csmv8201ap167sm6m-mvmctl-0.16.1`. The native package keeps `libkrunfw` explicit beside `libkrun`; signed binary install remains the primary user path, source-checkout Nix remains source-built, and microVM guest images do not install `mvmctl`.
- [ ] Authored and docs-synced [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md): the beginner product path is `mvmctl machine run/create/start/exec/shell/stop/pack` over existing runtime primitives, with binary-first install, no host-Nix prerequisite for normal use, image-backed one-shot docs before flakes/manifests, explicit network opt-in, persistent named machines, SDK parity, verified portable artifacts, measured hot-start claims, and a strict `mvm.toml` schema v1. Final closeout additions now explicitly track local image sources (registry refs, local OCI archives, stdin archive streams, unpacked rootfs directories) through hardened extraction/provenance/admission, scenario-led beginner docs, explicit limitations docs, SDK/CLI parity and non-bypass tests, and portable artifact pack/verify/inspect/run/transfer/cleanup workflow. The same pass de-duplicates ownership so Plan 200 owns beginner UX, Plan 199 owns install/host packaging, Plan 126/156 own dependency and binary-size mechanics, Plan 155 owns low-level artifact execution, Plan 159/189 stay VZ-specific, Plan 193/197 stay security substrate, and Plan 198 is completed perf input.
- [ ] Plan 200 auth/security proof follow-up: persistent-machine `ssh_agent` is explicit in signed/admitted policy metadata, receipts, dry-run, and audit, while remaining dev-tier/socket-only with no SSH key or known-host material copied or mounted. The 2026-06-19 Firecracker-host proof now reaches a running dev-profile `alpine:latest` persistent machine and captures honest auth metadata in dry-run, receipt, and audit output. Host-side raw SSH-agent proof succeeds through both the throwaway `SSH_AUTH_SOCK` and the spawned per-machine proxy UDS, but the in-guest raw probe against `/run/mvm/ssh-agent.sock` still fails with `Connection reset by peer`; the narrowed remaining live gate is Firecracker guest-to-host host-listen forwarding for dev `SSH_AGENT_PORT` 5301. Follow-up code now routes Firecracker through the same per-port runtime UDS transport used by the host-listen bridge and unit-tests that backend selection, but the live in-guest raw probe still needs to be rerun before the smoke is claimed. Non-standard-port SSH-banner denial now has a runtime ingress banner classifier (`ssh-banner-protocol-deny`) plus reverse-flow kill matching, and Firecracker's default bridge/TAP path drops inbound TCP packets containing the same `SSH-` banner prefix. Firecracker KVM proof at branch tip `4ce7d938` against `140.82.121.36:443` opened TCP but delivered no `SSH-2.0-...` bytes to the correctly scoped guest IP.
- [x] Implemented [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) Workstream A/B kickoff — the flagship `mvmctl machine run`. New `crates/mvm-cli/src/commands/machine/` group whose `run` verb (`--image <ref> [--cpus/--memory/--profile/--add-dir/--env/--timeout/--receipt/--json/--dry-run] -- <cmd>`) translates into the existing admitted/audited `vm::exec::run_secure` path, so the security posture is identical: signed `ExecutionPlan`, OCI provenance, **deny-all egress preserved** (no `--net` yet — that plumbing is the security-sensitive WS-B follow-up), SDK transport surfaces pinned off. Wired the new top-level command into the audit verb-name map (`cmd_audit.rs`) and the `audit_total_coverage` posture table (`machine run` = `InteractiveOrControl`, mirroring top-level `run`). README quickstart now leads with `machine run`; CLI reference gained a Machine section. 7 parser/translation unit tests + the 3 audit-coverage tests green; `cargo clippy -p mvm-cli -p mvmctl --all-targets -- -D warnings` + nightly fmt clean. Persistent verbs (`create/start/exec/shell/stop/ls/inspect/rm`), `--net`/`--allow-host`, local image sources, `mvm.toml` schema v2, SDK parity, and `pack` remain.
- [ ] 🟡 In flight (branch `feat/plan-200-machine-net`, NOT merged — gated on live validation + security review) — [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) Workstream B: `--net` / `--allow-host HOST[:PORT]` with **uniform egress enforcement across Firecracker, libkrun, and Vz**. One `NetworkPolicy` on `VmStartConfig` (default deny-all, additive); FC enforces via its firewall, libkrun/Vz via the gateway bridge. **Step 1** threads the policy `VmStartConfig` → `SupervisorConfig` (libkrun-sys + `mvm_build::vz`) → `BridgeConfig` → `run_bridge_inner`'s no-bundle conversion (`PlanFlowPolicy::from_network_policy`: deny-all drops at flow-open, allow-list opens + DNS-sinkholes, unrestricted opens — all under mandatory-deny). **Step 2** routes every transient `run`/`machine run` through the existing `admit_plan_for_boot` admission (sets `tenant_id`+`plan_json`) so the bridge spawns and the run is chain-audited — `run`/`run_captured`/`run_inner` gained an `admit` closure mirroring the `invoke`/session path. Effective posture recorded (non-sensitive) in dry-run/receipt/human-preflight; `NetworkPolicy::posture_label()` + `trusted_build_egress()` added. Builder/dev keep the legacy non-bridge path (their `trusted_build_egress` opt-in + `run_legacy` removal is the deferred step 3); MCP code-run stays un-admitted (follow-up). Workspace compiles; 977 mvm-cli lib + new policy/resolver/posture/flow-policy tests green; clippy + nightly fmt clean across mvm-core/mvm-hostd/libkrun-sys/mvm-build/mvm-backend/mvm-vm-host/mvm-cli. **Live-validated 2026-06-16 (macOS/Vz):** no regression (A/B vs `main` identical guest-net), admission+bridge+boot+run+teardown work, posture surface correct; and the bare-`NetworkPolicy` enforcement is **proven through the live gateway bridge with real Unix datagram sockets** — deny-all drops at the flow gate, an allow-listed host's DNS+TCP forward, an unlisted host is sink-holed (`bare_*_through_the_live_bridge` tests; lowering extracted to `bare_network_policy_egress` so the test drives the exact production path). The macOS *VM-level* egress smoke is blocked by a **pre-existing** transient-run networking gap (transient guest never brings up `eth0`; unprivileged exec can't DHCP — `main` identical), filed as a deferred follow-up that also unblocks macOS `machine run --net`; the VM smoke runs on Linux/KVM/Firecracker. Also surfaced two pre-existing OCI bugs (cache index `schema_version 0`; OCI rootfs missing `mvm-meta.json` sidecar) — filed. **Adversarial security review done (verdict `merge-after-fixes`); all three blockers fixed in-branch:** (1) warm-claim AllowAll bypass — egress policy now threaded through `StandbyClaim`/`SupervisorAttachConfig`/`from_base_and_attach`, and `run_bridge_inner`'s no-bundle/no-policy fallback fails CLOSED to deny-all (not `AllowAll`); (2) the `mvm_keys_dir` stale-base revert restored (MVM_DATA_DIR-aware, no raw `$HOME`); (3) accidentally-committed local cache + chain-signed audit log removed from history + gitignored. The fix pass also closed two further egress holes — the deny-all fail-closed fallback above, and the **primary Vz path** (`VzGvproxy`) now routes the resolved `flow_policy` instead of `cfg.policy` (proven by `bare_deny_all_policy_drops_egress_through_the_live_vz_bridge`). Honest per-backend `egress_enforcement` tier recorded in the signed receipt so it can't overstate FC's host:port vs libkrun/Vz's host-name-only fidelity. Full suite green (3309 nextest + doctests), clippy/fmt/spec-gates clean. **Remaining before merge:** open PR (+ optional Linux/KVM VM smoke).
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) WS-B deferred follow-up — **remove the vestigial `BridgeConfig.policy` field + the `AllowAll` type** (pure hygiene, no behavior change). `run_bridge_inner` no longer read `cfg.policy` — the flow gate is derived from `bundle` (when present) or `network_policy` (the bare path), failing closed to deny-all — so the field was a write-only footgun. Dropped the field + the `AllowAll` `FlowPolicy` impl; the four supervisor-bin construction sites (`mvm-libkrun-supervisor`, `vz_objc`, `mvm-vz-drainer`, `mvm-firecracker-bridge`) no longer set it; gateway-bridge tests that used `Arc::new(AllowAll)` as an allow-all gate now use `PlanFlowPolicy::from_network_policy(&NetworkPolicy::unrestricted())` (the production allow-mode gate); the `AllowAll`-only unit test was removed. cfg(linux) `mvm-firecracker-bridge` cross-compiled with cargo-zigbuild since macOS clippy skips it.
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) WS-B deferred follow-up — **uniform `host:port` L4 egress on the libkrun/Vz bare path** (stacked on the #1019 hygiene PR). The no-bundle path previously lowered an allow-list to a host-**name**-only `DnsSinkholeScan` (`egress_l4 = None`), so the port was ungated and a direct-IP dial bypassed the gate; Firecracker gates `host:port` via nftables. Closed: `run_bridge_inner` resolves the bare allow-list's hosts host-side in its sync prologue (`resolve_bare_dns_pins` — the admission-time DNS pin, mirroring nftables resolving `-d <host>` at insert) and `mvm_core::policy::projection::canonicalize_network_policy` lowers `(pinned IP, port)` into `CanonicalEgress::Rules` (one TCP rule per pinned IP + a **UDP/53-only** name-gated DNS carve-out; TCP/53 is deliberately not carved out — the qname gate only covers UDP/53). `L4PolicyScan` then drops a direct-IP dial to an unlisted address and a wrong-port dial — uniform with Firecracker. An unresolvable/expired/empty pin fails **CLOSED** to deny-all. The receipt tier collapsed to a uniform `<backend>:l4-host-port` (no more `dns-name-only`) — `egress_enforcement_label` + the receipt doc/tests updated. **Adversarial security review (verdict `merge-after-fixes`):** finding 1 (label could overstate on a scan-less Vz substrate) — verified the Vz data plane is exclusively `VzGvproxy` (in-process, full scan); `VzIngest`/drainer ingests NDJSON audit events only and never splices guest egress, so the label is honest; finding 2 (DNS carve-out covert channel) — tightened to UDP/53 only. Proven by `canonicalize_network_policy` unit tests (deny-all/unrestricted/allow-list/multi-IP/missing-expired-empty-pin) + `bare_allow_list_l4_{forwards_pinned_host_port,drops_direct_ip_to_unlisted,drops_wrong_port_on_pinned_host}_through_the_live_bridge` (libkrun + Vz bridge paths, real Unix datagram sockets). fmt/clippy `-D warnings`/spec-ref+spec-number gates clean; cfg(linux) `mvm-hostd`/`mvm-vm-host` bins cross-compiled with cargo-zigbuild. Bundle (signed-policy) path, claims 1–3, and ingress untouched (additive bare-only function).
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) WS-B deferred follow-up — **decide + pin the DHCP/ARP posture under deny-all** (stacked on the uniform-L4 PR). Decision: **loopback-only, no control-plane carve-out**. Under deny-all the flow gate drops every egress flow, and DHCP (UDP 67/68) is an egress flow → no lease is offered; the guest self-assigns the static gvproxy fallback address (the eth0 bring-up already runs `udhcpc -n`, so it exits on no-lease rather than hanging on a never-arriving OFFER, then applies the static fallback). `eth0` is up but has no admitted egress; only loopback + the egress-denied local link are usable. ARP / IPv6-ND are non-IP L2 frames the bridge forwards unchanged (it gates IP 5-tuples) — local-only, harmless, no special handling. A minimal DHCP/ARP carve-out was considered and **rejected** (the static fallback already keeps `eth0` up; a UDP 67/68 allowance would be a needless flow-gate special case and, if unscoped, a covert-channel surface). Documented in ADR-002 §"Deny-all control-plane posture (DHCP/ARP)"; pinned by the live-bridge test `bare_deny_all_drops_dhcp_discover_through_the_live_bridge`. fmt/clippy/spec-gates clean.
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) WS-B deferred follow-up — **transient-guest eth0 enabler ticked + live-validated** (stacked on the DHCP/ARP PR). The eth0 bring-up itself landed in **#1020** (unconditional/host-enforced — guest-side gating isn't a boundary; deny-all is enforced host-side + the static fallback keeps `eth0` up without hanging). This PR ticks the plan checkbox and records the **live result**: the `examples/egress-probe` workload booted on libkrun and reached BOTH external probe targets (`up --wait` verdict 0), proving the guest now gets a working `eth0` (link-up → DHCP → static gvproxy fallback). The Vz VM-level smoke is gated on `up --wait` being libkrun-only (Plan 152 WS-A); the Vz *bridge* enforcement is covered by the deterministic `*_live_vz_bridge` tests. **Surfaced a tracked follow-up** (added to the plan's deferred list): `up --network-allow` did not enforce egress on the libkrun `up` path (verdict 0 = no enforcement, not even the default deny-all). The later follow-up below corrected the initial blame on foreground start-config threading: the real issues were signed-plan threading being gated off by default and `local-default` bundle resolution shadowing the bare allow-list. Pre-existing, not introduced by the WS-B egress work.
...
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) C1 runtime-mapping follow-up — `machine create --manifest <path>` now reads image-backed `mvm.toml` / `Mvmfile.toml`, persists manifest-driven network/resource/dev fields into `MachineSpec`, resolves relative manifest volume paths against the manifest directory, and `machine start --name` now threads persisted `mem_initial` and volume shares into the admitted persistent OCI launch while running `dev.init` through the existing guest-agent path for dev-capable profiles. The slice fails closed for unsupported `ssh_agent = true` starts instead of ignoring them. Verified with expanded `mvm-cli` machine tests, `mvm-core` manifest tests, `audit_total_coverage`, and `cargo clippy -p mvm-cli --all-targets -- -D warnings`.
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) C1 auth follow-up — real dev-tier `ssh_agent` support for persistent machines is implemented. `machine start --name` now accepts `ssh_agent = true` only on dev-capable profiles, requires host `SSH_AUTH_SOCK` to be a Unix socket, spawns a detached per-machine proxy that connects only to that socket, registers the existing guest→host vsock host-listen path on libkrun/Vz, and asks the dev guest agent to bind `/run/mvm/ssh-agent.sock`. `machine exec`, `machine shell`, and `dev.init` inject only that guest socket path; stop/failure paths reap proxy PID/socket state. Dry-run/receipts/audit report `ssh-agent-socket` rather than implying key-file mounting. No private key files, `~/.ssh`, or known-hosts material are copied or mounted. Follow-up hardening now removes built-in `github.com:22` preset grants, rejects `--allow-host ...:22`, denies TCP/22 even under open egress, and makes `mkGuest` fail evaluation when Nix templates try to add SSH packages/config/material through `packages` or `extraFiles`. Verified with focused `mvm-guest`, `mvm-cli`, `mvm-backend`, and SSH-ban policy tests plus `cargo check -p mvm-guest -p mvm-cli -p mvm-backend --all-targets` and `cargo clippy -p mvm-guest -p mvm-cli -p mvm-backend --all-targets -- -D warnings`; live VM SSH-agent round-trip and non-standard-port SSH protocol-denial smoke remain follow-ups.
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) C1 policy-surfacing follow-up — `machine start --dry-run` / `--dry-run --json` now report the effective network posture, enforcement tier, auth mode, dev-init hash/count, and redacted volume policy without booting; real `machine start --receipt <path>` writes the same policy summary into a signed machine-start receipt plus the resolved digest and start timestamp; and successful starts emit a machine-specific `VmStart` policy audit line. The later auth follow-up above replaced the former fail-closed SSH-agent placeholder with dev-tier socket forwarding; the remaining auth-policy gap is a distinct signed-plan admission field for forwarded-agent policy. Verified with expanded `mvm-cli` machine tests, `audit_total_coverage`, and `cargo clippy -p mvm-cli --all-targets -- -D warnings`.
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) C1 auth/admission proof follow-up — persistent-machine `ssh_agent = true` is now explicit signed-plan metadata, not just launch/runtime state. `ExecutionPlan` schema v6 carries `auth.mode` (`none` / `ssh_agent_socket`), `machine start` passes `ssh_agent_socket` only after the existing dev-capable profile gate, and `auth_mode=ssh-agent-socket` is copied into `plan.admitted` / `plan.policy_resolved` audit labels so admission, dry-run, receipts, and machine-start audit agree. Focused proof: `cargo check -p mvm-core -p mvm-cli -p mvm-hostd -p mvm-vm-host --all-targets`; `cargo test -p mvm-cli --lib auth`; `cargo test -p mvm-core --test replay_protection`; `cargo test -p mvm-core --lib ssh`; `cargo test -p mvm-cli --lib allow_host_rejects_ssh_port`; `cargo test --test nix_flake_structure mk_guest_rejects_ssh_template_inputs_structurally`. Live VM SSH-agent protocol round-trip and non-standard-port SSH-banner denial remain builder-VM-gated follow-ups and are documented in Plan 200 without overclaiming.
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) C1 live auth-proof attempt recorded — the 2026-06-19 Firecracker-host run now reaches guest boot on branch-local `mvmctl`: a dev-profile `alpine:latest` persistent machine with `[auth].ssh_agent = true` starts, signed receipt/dry-run/audit surfaces report `ssh-agent-socket`, and direct raw `SSH_AGENTC_REQUEST_IDENTITIES` probes to the throwaway host agent and spawned per-machine proxy UDS both return an SSH-agent identities answer. The proof remains unclaimed because the in-guest raw probe copied to `/tmp/mvm-agent-probe-c` reaches `/run/mvm/ssh-agent.sock` but reads `Connection reset by peer`, narrowing the remaining blocker to Firecracker guest-to-host host-listen forwarding for port 5301. The PR now includes the Firecracker backend-selection fix that uses the per-port runtime UDS for that path rather than raw host AF_VSOCK; the live probe still needs to be rerun before the checkbox above can close. Non-standard-port SSH-banner denial now has runtime classifier coverage and still needs the live non-22 guest witness before it can be claimed.
- [x] Authored [ADR-088](adrs/088-dev-vm-promotion-boundary.md) and synced Plan 200/refactor status: mutable dev-machine state is now explicitly documented as non-authoritative for prod/sealed builds, promotion back to production inputs must be explicit, and future `ssh_agent` support is constrained to a dev-tier socket-forwarding feature only, never a prod/sealed capability.
- [x] [`plans/200-machine-ux-dx-layer.md`](plans/200-machine-ux-dx-layer.md) WS-C persistent machine substrate — **named machine specs and running-VM wrappers are implemented**. Added `<MVM_DATA_DIR>/machines/<name>/machine.json` helpers in `mvm_core::config`, `MachineSpec` strict JSON, atomic writes, safe-name validation before path use, and `mvmctl machine create` / `ls` / `inspect` / `rm --yes`. Added `machine exec` / `shell` / `stop` wrappers that require a persisted spec first and reuse the existing console/down paths; `machine exec` mirrors transient exec's argv shell-quoting for the current guest `Exec` string protocol. Tests cover parser routing, state persistence under isolated `MVM_DATA_DIR`, overwrite refusal without `--force`, sorted listing, strict unknown-field rejection, confirmed deletion, wrapper spec guards, and argv quoting. Full `mvm.toml` machine mapping, SDK parity, docs, portable artifacts, and CI budgets remain.
- [x] Implemented [`plans/202-host-services-daemon.md`](plans/202-host-services-daemon.md) Phase 2a/2b — signer-helper daemonization base and host-agent forwarding. Added the shared `SignerHelperRequest`/`SignerHelperResponse` wire (`register_vm`, `deregister_vm`, `append_entry`, `probe`) in `mvm-core`, a new `mvm-hostd::audit_signer::helper::SignerHelper` that owns a `vm_id → Chain` map, and a `mvm-signer-helper` bin. `mvm-host-agent` now starts the helper as a supervised child, passes only the tenant key path across the moat, waits for `signer-helper.sock` before serving control, and forwards accepted `host.audit.v1` entries to the helper by server-derived `vm_id`; the host-agent still does not read the key. Tests cover protocol roundtrips/unknown-field refusal, helper config defaults, per-VM chain routing, deregister close/drop behavior, cross-tenant refusal, unsafe `vm_id` refusal, workload mismatch refusal, and cross-VM chain isolation. Remaining Plan 202 Phase 2 work: 2c admission signing via helper, 2d restart/head rebuild.
- [x] Closed/rehomed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) after the Plan 202 daemon split clarified ownership. Plan 125 is now the completed SDK/CLI veneer plan: Sandbox/CodeSandbox/BrowserSandbox, TS parity, `--secret`, doctor capability matrix, named security profiles, guest broker transport, typed `host.audit.v1` / `host.time.v1` / `host.cost.v1` clients, Python/TypeScript cdylib shims, host-spine coverage, and libkrun live `host.audit.v1` proof. Remaining host-services daemon/process-model work now lives only in Plan 202.
...
- [x] Started Plan 185 Phase 6 (Task 13 doc verification). Ran `cargo doc --workspace --no-deps` with `broken_intra_doc_links` on: the Phase 3 renames introduced **zero** broken links (grepping the unresolved set for `Backend`/`EgressProxy`/`DeviceMapperBackend`/`BackendKind` is empty — #892/#894/#895 fixed their own links), but the run surfaced ~115 *pre-existing* broken intra-doc links across every crate. Cleared `mvm-core` (16 sites): public-API references became real qualified links (`[`SecretStore::list`]`, `[`crate::crypto::snapshot_hmac`]`, `[`std::str::FromStr`]`, the `SIG_ALG_*` consts, `[`PlanArtifact`]`, `[`Self::has_vz`]`), while private-item / test-fn / prose / non-resolving module-doc references were backticked. `cargo doc -p mvm-core --no-deps` is now clean under `-D rustdoc::broken_intra_doc_links`; `cargo clippy -p mvm-core --all-targets -- -D warnings`, nightly fmt, and the spec gates pass. Remaining crates' broken links are tracked in the plan and gate Phase 7's workspace doc build (no host/platform blocker — the debt is fixable).
- [x] Advanced Plan 185 Phase 6 Task 13 into `mvm-build` (#941) and refined the finding. Fixed the *unconditional* broken intra-doc links (45 → 32 under `--all-features`): `vz_builder.rs` dropped the `crate::` prefix on 10 `libkrun_builder::…` links the code itself qualifies; `vz.rs` `Boot`/`Restore` → `StartupMode::{Boot,Restore}`; `builder_vm.rs` → `crate::libkrun_builder::LibkrunBuilderVm`; private consts + external objc2 types backticked. **Refined finding:** the doc-link count is feature- and platform-sensitive — many targets are `#[cfg(feature)]` modules (resolve only with `--all-features`) and a cluster is `#[cfg(target_os = "linux")]` builder-VM bins that aren't in the doc graph on a macOS host (they resolve on a Linux doc build; backticking them would degrade the valid Linux docs). So the Phase 7 doc gate must run on **Linux with `--all-features`, per-crate**; only links broken *there* are real bugs.
- [x] Completed Plan 185 Phase 6 Task 12 (secret/debug exposure) (#943). Audit conclusion: the codebase already enforces secret hygiene — `check-no-display-on-secret-types` is clean, raw values live in zeroizing `secrecy::SecretBox`, secret structs carry hand-written redacting `Debug`, and a field-name sweep found no unprotected types (only an opaque `pid_token` CLI arg + the already-redacted `vmgenid::GenerationToken`). Closed the Step 3 gap by adding negative redaction tests for the types that had a redacting `Debug` but no test pinning it: `HostSigner` (mvm-hostd), `ResolvedBinding`/`ResolvedSecrets` + `EgressCa` (mvm-core) — so a future edit dropping the impl can't silently leak (the name-gate only checks an impl exists, not that it redacts). Tests-only; `cargo clippy -p mvm-core --features egress-ca -p mvm-hostd --all-targets -- -D warnings`, nightly fmt, and the secret/spec gates pass.
- [x] Completed Plan 185 Phase 6 Task 10 (error shapes / typed-error tests). Audit finding: the actionable surface is tiny — the `thiserror`-vs-`anyhow` boundary is already the intended one (typed enums at library surfaces callers react to: keyholder `ProxyError`/`TerminatorError`, `RotationError`, `OciUnpackError`; `anyhow` + `.context()`/`bail!` everywhere else), and where typed errors exist the tests already use `matches!` (the hostd security paths). Almost every `unwrap_err().to_string().contains(...)` in the tree is on an `anyhow` or `serde_json` error where the rendered string is the only handle and the correct thing to assert. Converted the one genuine candidate: `load_master_key` surfaces an anyhow-wrapped `RotationError::KeyFilePerms { mode }`, so its test now `downcast_ref::<RotationError>()`s and matches the structured `mode` field instead of substring-matching `"0644"` (robust to message rewording, asserts the real field). Verified `cargo test -p mvm-core load_master_key` + `cargo clippy -p mvm-core --all-targets -- -D warnings`.
- [x] Completed Plan 185 Phase 6 Task 11 (consolidate repeated fixtures) (#953). Six near-identical ~40-line minimal `ExecutionPlan` fixtures across the mvm-hostd audit/admission cluster (`audit/{emitter,bind,host_keypair,plan_persist}`, `supervisor/{lifecycle_hooks,audit_recorder}`) collapsed into a single `mvm_core::plan::test_support::PlanFixture` builder — a valid minimal local plan with `with`-style overrides for the fields a test pins (tenant/plan_id/workload/runtime_profile/nonce/secrets/validity). Gated `cfg(any(test, feature = "test-support"))` (both mvm-hostd + mvm-cli already enable `mvm-core/test-support`); no new deps so `check-core-runtime-free` stays green. Each `fixture_plan` is now a thin wrapper (call sites unchanged, pinned values preserved); removed the now-unused per-file plan-type imports. Net −156 lines. The realistic-shape fixtures (`signing::sample_plan`, supervisor aggregate/audit, mvm-cli policy_resolver) are a materially different plan and intentionally left alone. Verified 2 new `PlanFixture` unit tests + 83 migrated mvm-hostd tests pass, `cargo clippy -p mvm-core --features test-support -p mvm-hostd --all-targets -- -D warnings`, nightly fmt, spec/core-runtime gates. **Phase 6 Tasks 10/11/12 done + Task 13 mvm-core/mvm-build; remaining Task 13 doc-link crates deferred to a Linux + `--all-features` pass, then Phase 7 closeout.**
- [x] Plan 185 Phase 7 closeout — validated on the x86_64 Linux box (the required env: `mvm-backend` tests SIGKILL under macOS amfid, so they only run on Linux). `cargo test --workspace` green: 3720+ tests across the heavy crates alone (mvm-core 1244, mvm-hostd 962, mvm-backend 914, …) + the rest of the suite, **zero genuine failures**. The Linux run *found and fixed* a real Phase-1-class test-isolation bug: `mvm-build`'s `cfg(linux)` `mvm-host-vm-init` test set `TMPDIR=/inherited-from-parent` (a non-existent path) via unguarded `std::env::set_var`, leaking it to 15 parallel `tempfile::tempdir()` tests — invisible on macOS where the bin is `cfg`-excluded. Fixed with `TestEnv` + a valid sentinel path (#960; bin now 151/151). The only other non-green is `each_embedded_binary_starts_with_elf_magic`, which fails by design under `MVM_SKIP_EMBED_BINARIES=1` (stub payloads, not real ELF) — green in a real-embed build. `cargo check --workspace` green (the test build compiled all targets on Linux). clippy green in the required macOS CI env for all 185 changes; a Linux clippy run additionally surfaced 11 *pre-existing* doc-format/collapsible-if lints in the `cfg(linux)` `mvm-host-vm-init` bin (unrelated to 185, deferred). `cargo doc` is a documented blocker: ≈122 pre-existing broken intra-doc links (not a 185 regression — the renames were clean), to be re-measured + fixed on Linux + `--all-features`. **Deferred debts recorded in plan Task 14 closeout:** Task 8 `vz_objc.rs` (vz active), Task 13 remaining-crate doc links, and the pre-existing Linux `mvm-host-vm-init` clippy lints.
- [x] Plan 185 Task 13 — broken intra-doc links **fully cleared**. Measured on the x86_64 Linux box with `--all-features` (the real gate; the macOS default-feature run under-reports `cfg`-gated modules): 122 pre-existing broken intra-doc links across every crate + xtask. Fixed all 122 (doc-comment only, no logic): backticked `<placeholder>` paths and literal `[bracket]` prose (`[mvm]`, `[error]`, `argv[0]`), private/test/method links, cross-crate refs (broken under `--no-deps`), and module-doc `//!` submodule/type-overview lists; preserved valid `///` item-doc links and type-defining-file links. `cargo doc --workspace --all-features --no-deps` is now clean under `-D rustdoc::broken_intra_doc_links` (exit 0). Landed on `docs/plan-185-task13-doclinks` (batches 1–3). With this, the only remaining Plan 185 deferrals are Task 8 (`vz_objc.rs`, vz-active) and the pre-existing Linux-only `mvm-host-vm-init` clippy doc-format lints (orthogonal to 185).
- [x] Cleared the pre-existing Linux-only clippy lints in `mvm-build`'s `cfg(linux)` `mvm-host-vm-init` bin (surfaced while validating Plan 185's Phase 7 gate on the Linux box; the per-PR root `clippy --all-targets` never lints a dependency's bin, but the `ci-full` `clippy -p mvm-build -p mvm-backend --all-targets` Linux lane does). Fixed the 12 lints — collapsed a nested `if let Ok(pid) = … { if let Ok(pid) = pid {` → `if let Ok(Ok(pid))`; converted a dangling `///` overview block (followed by a `//` divider + consts, documenting nothing) to plain `//`; re-indented 9 `doc_lazy_continuation` list-continuation lines. Verified `cargo clippy -p mvm-build -p mvm-backend --all-targets -- -D warnings` green on the x86_64 Linux box (no cascade) + nightly fmt clean. **With Task 13 and this done, the sole remaining Plan 185 item is Task 8 (`vz_objc.rs`), parked until vz quiets.**
- [x] **Plan 185 COMPLETE.** Closed the last task — Task 8, the `unsafe`-boundary SAFETY audit of the macOS-only objc2 Vz bridge `crates/mvm-vm-host/src/vz_objc.rs` (#976). The handoff's "~120 raw `msg_send` blocks" estimate was stale: the file already carried 89 `SAFETY:` notes and uses typed objc2 (only 3 raw `msg_send`). Audited the unsafe surface and filled the ~16 gaps — the `define_class!` protocol conformances, the two `msg_send![super(this), init]` delegate initializers, `validateSaveRestoreSupportWithError`, and the bound-dgram-socket test's libc cluster — each citing the file's established serial-dispatch-queue / single-guest-per-process ownership invariant. Every `unsafe` block now carries a SAFETY note (89 → 105). Comment-only; verified on **macOS arm64** (the only host that compiles this objc2/Apple-Virtualization code, so not the Linux box): `cargo clippy -p mvm-vm-host --bin mvm-vz-supervisor -- -D warnings`, the lib test, nightly fmt, spec-ref gate. Done despite live vz work because the additive notes sit in the delegate/init/validate/test regions, disjoint from the `feat/plan-152-vz-supervisor-hardening` struct/host-listen reshape. **All of Phases 1–7 are done; no remaining deferrals.**
...
- [x] Completed [`plans/195-builder-vm-fingerprint-narrowing.md`](plans/195-builder-vm-fingerprint-narrowing.md): the builder-VM source fingerprint no longer folds in the redundant whole-workspace `Cargo.lock` (the builder-VM flake forbids `buildRustPackage`, so embedded host-bin byte hashes are the authoritative baked-Rust input), and `mvm-cli/build.rs` watches the real binary inputs (`Cargo.lock` + `crates/mvm-build/src`) so those bytes rebuild when their dependency/lib source changes. Closeout also fixed stale code comments that still described the removed Cargo.lock layer. Verified 2026-06-19 with focused fingerprint tests (6/6), real non-stub embedded-binary build, `mvm-build/src` touch → `cargo zigbuild` rerun proof, `cargo nextest run -p mvm-cli --no-fail-fast` (1088 passed, 1 skipped; after building `mvmctl` for integration tests), `cargo clippy -p mvm-cli --all-targets -- -D warnings`, and nightly `cargo fmt --all`. Live `mvmctl dev up` manual was not re-run in this session because the working agreement requires runtime commands inside the project builder VM and this checkout has no wrapper for that; deterministic cache-key tests cover the same hit/mismatch transitions without booting a VM. Build-perf only, no security-claim impact.
...
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E4 — **named security profiles**. `resolve_security_profile(name)` in `mvm-core::policy::security_profile` maps a name → `SecurityProfile { seccomp: SeccompTier, egress: NetworkPreset, snapshot_allowed, deployable }`. **Binary production-vs-development model** (not a strictness gradient): **`production` is the default** — the highest-security, production-ready posture (seccomp `standard` floor + deny-all egress + no snapshot) and **the only deployable profile**; **`dev`** is a development-only convenience (unrestricted seccomp + open egress + snapshots) carrying `deployable = false`, so it **can never reach production** — which is exactly why it's allowed to be loose. The invariant `every deployable profile is_bounded()` is asserted, so a one-word profile can never silently un-sandbox a *deployable* workload. Aliases `prod`/`production`, `dev`/`development`; unknown name fails closed. Surface: `--security-profile <name>` on `up` (`--profile` was already the flake profile) — default `production`, supplies the defaults for `--seccomp` + the egress preset, explicit `--seccomp`/`--network-preset` win, production is byte-identical to today's seams; the prod build path (`--prod`) **refuses a non-deployable profile** via `enforce_profile_deployable` (extracted + unit-tested). RED-first: 6 resolver tests (mvm-core) + 4 precedence/deploy-guard tests + 1 CLI-parse test (mvm-cli). `cargo test -p mvm-core/-p mvm-cli` green (954 cli lib), clippy + nightly fmt + spec-ref + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. (Snapshot-allowance enforcement is a follow-up; `up` has no snapshot flag yet — the resolver carries the disposition.) Rust-side. Remaining Phase E: E5 host-services SDK; Phase A is closed as satisfied by the following scope amendment.
- [x] Closed out [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) **Phase A** (52 flat verbs → ≤15 nested groups) as **SATISFIED** — docs/scope amendment, no code churn. The sprawling-flat-CLI premise is stale: current `main` already exposes grouped command families plus deliberate one-token convenience verbs, and forcing a literal count would remove the kept conveniences or merge distinct concepts. The remaining flat internal entries are hidden subprocess entrypoints. A1/A2 and matching acceptance boxes are ticked under the amended target: grouped surface + deliberate conveniences.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.1 — Layer 1 broker transport** (the foundational piece all host services ride on; E5 is the sole remaining Plan 125 task, security-critical — claims 8/12/13). **Resolved two security questions before writing any transport.** (1) *Guest-facing port*: the supervisor's guest-facing broker listener is unbuilt (deferred), so the path is the `host_listen_ports` allowlist mechanism `SUBSTITUTION_PORT` uses — added `BROKER_PORT = 5300` (the broker's documented port; reuses the freed pre-broker secrets-channel number) to `mvm-guest::vsock`, disjoint from 5251/5252/5253 and the 10k/20k forward/console ranges. (2) *Session binding (claim 12)*: the broker path carries a **bare `ServiceCall`, not an authenticated frame** — the host derives workload identity from the connection (the supervisor builds `ServiceCallCtx`; the registry's bound-set comes from the admitted `ExecutionPlan.services`), so the guest presents nothing and holds no key/secret. **The client is advisory-only**: every gate (binding, `host.audit.v1` category-forcing, size/rate caps, correlation-id assignment) is host-side. **Homed in `mvm-guest`** (`broker_client.rs`), sibling to `substitution_client.rs` — not `mvm-sdk` as the scoping note read, because mvm-sdk has neither `mvm-guest` nor `mvm-core` as a runtime dep (dev-dep only), while mvm-guest is the in-guest crate baked into the image and already carries the vsock/framing primitives + the `mvm-core` broker types. API mirrors the substitution relay: `call(stream, &ServiceCall) -> Result<Value, BrokerError>` (testable core; one call per connection — the host assigns `correlation_id`, so no multiplexing) + `broker_call(req, timeout)` (`connect_host_vsock(BROKER_PORT)` + `call`); `ServiceResponse::Err` surfaces as typed `BrokerError::Service { code, message }`, transport/framing as `BrokerError::Transport`. RED-first, 5 mock-I/O unit tests over `UnixStream::pair()` (mirroring the host `broker_proxy` mocks): Ok-payload roundtrip + exact-envelope assertion, `Err`→typed error, oversize-frame rejection (>256 KiB read cap), truncated-body rejection, malformed-JSON-body rejection. `cargo test -p mvm-guest` green (274 lib + suites, 0 failures), `clippy -p mvm-guest --all-targets -D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` all clean. Remaining: E5.2 typed `host.audit.v1` (host forces `workload_audit`, caps), E5.3 codegen/pure-language veneer + live-VM E2E (box), E5.4 `host.time.v1`/`host.cost.v1`.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.2 — typed `host.audit.v1` methods** (Layer 2, on the E5.1 transport). **Surprise on grounding: the host side is already built** — `mvm-hostd`'s `HostAuditV1Handler` forces `category: workload_audit`, enforces the 4 KiB per-record cap (→ `BadRequest`) and the 20/s/workload token bucket (→ `RateLimitExceeded`), implements `emit` + `emit_batch`, and chain-signs via `mvm-audit-signer`; the wire payloads (`EmitRequest`/`EmitResponse`/`EmitBatchRequest`/`EmitBatchResponse`) already live in `mvm-core::protocol::host_audit`. So E5.2 is purely the **guest half**: `mvm-guest::host_audit` `emit`/`emit_batch` (+ `_on` stream variants for testing) build the `host.audit.v1` `ServiceCall` over `broker_client` and map the host's typed `ServiceErrorCode` onto a typed `AuditError` (`RateLimited` / `BadRequest` / `Unavailable` / `Service{code}` / `Transport`) the veneer can turn into distinct exceptions. **Claim 8 is structural, not advisory**: `EmitRequest` has no `category` field, so a workload literally cannot express a host category, and the host stamps `workload_audit` regardless — the guest can't spoof it even by bypassing the SDK. The 4 KiB cap + 20/s limit stay **host-enforced** (single source of truth); the guest only surfaces the host's refusal as the typed error (no client-side cap to drift). `correlation_id` is a placeholder (atomic counter, no new dep) the supervisor reassigns at ingress. RED-first, 7 mock-broker unit tests over `UnixStream::pair()`: envelope shape (`host.audit.v1`/`emit`, **no category key**, parses as `EmitRequest`) + chain_head return, `BadRequest`/`RateLimitExceeded`/`Unavailable`/other-code → typed `AuditError`, `emit_batch` envelope + per-entry statuses, distinct minted ids. `cargo test -p mvm-guest` green (0 failures), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Remaining: E5.3a `BROKER_PORT` host-listen reservation, E5.3b broker-services lifecycle + veneer + live E2E, E5.4 `host.time.v1`/`host.cost.v1`.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3a — reserve `BROKER_PORT` in `host_listen_ports`** (the host-side first brick toward the live broker round-trip). **Grounding found E5.3 is much larger than "veneer + E2E"**: the broker-services subprocess lifecycle is unbuilt — `BrokerProxy::new`, `AuditSignerProxy::new`, and the broker `serve` are all **test-only**, and the per-VM supervisors spawn nothing but `codesign`, so nothing spawns `mvm-broker` / `mvm-audit-signer` per VM or binds the broker UDS. E5.3a is the correctly-sized first step: register `BROKER_PORT` (5300) in `host_listen_ports` on **both workload backends** (libkrun `add_host_listen_port`, vz `vsock.host_listen_ports`), the same **fail-closed staging** `SUBSTITUTION_PORT` uses — registered unconditionally, but nothing binds the per-VM UDS until E5.3b spawns the broker, so a stray guest dial gets `ECONNREFUSED` (no eager listener, no hang). RED-first: dedicated disjoint-union assertion tests per backend (`BROKER_PORT must be in host_listen_ports` ∧ `must not appear in vsock_ports`/`vsock.ports`), mirroring the substitution-port tests; watched both go red then green. `cargo test -p mvm-backend` green (320 passed, 0 failed — no codesign SIGKILL this run), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. **The large remainder — E5.3b broker-services subprocess lifecycle (spawn+supervise `mvm-audit-signer` + `mvm-broker` per VM, `BROKER_PORT`→UDS bind, `ServiceCallCtx` enrichment incl. correlation rewrite, spawn process-moat hardening, codegen/pure-language veneer, live-VM E2E) — is grounded in `specs/notes/plan-125-e5-3b-broker-services-lifecycle-scoping.md`; it's process-moat work (Plan 128-adjacent), recommended as its own workstream.** Remaining E5: E5.3b (above), E5.4 `host.time.v1`/`host.cost.v1`.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.4 — typed `host.time.v1` / `host.cost.v1` methods** (#933 — done in a parallel session, merged into the E5.2 branch, carried through this rebase). `mvm-guest::host_time` (`now` + `now_on`) + `mvm-guest::host_cost` (`workload`/`tenant` + `_on` variants) ride the E5.1 `broker_client` exactly like E5.2's `host.audit.v1`. The host time/cost handlers aren't built yet (only `HostAuditV1Handler` exists), so this establishes the shared wire contract — `mvm-core::protocol::host_time` `TimeNowResponse{wall_ms}` + `host_cost` `CostReport{spent_micros_usd}` (int-only clock/money, `deny_unknown_fields`, reused unchanged when the host handler lands) — plus the guest half. Typed `TimeError`/`CostError` (`NotBound`/`Unavailable`/`Service`/`Transport`, + `NotImplemented` for the mvmd-delegated `cost.v1::tenant`); scope = the verb, so the request body is empty (a workload can't ask for another scope's spend). 18 RED-first tests (4 core serde + 14 guest mock-I/O). codegen/pure-language veneer for all three services rides E5.3b.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-0 — workload audit-chain verifier** (resolves E5.3b open-question 4; makes the workload chain verifiable before the spawn lands it). **Grounding flipped the scoping note's approach.** "Unify the audit-signer onto the existing `SignedEnvelope` chain" turned out to be a *refactor of the shipped claim-8 type*, not a prerequisite: `SignedEnvelope.entry` is a plan-bound `AuditEntry` with `#[serde(deny_unknown_fields)]` + **string-only `labels`** — no `category`, `session_id`/`correlation_id`, or JSON `fields` — so it can't hold a workload record without mutating the type `verify_audit_chain` deserializes. The new `mvm-audit-signer` already has the right model (`OnDiskEntry`/`CanonicalEntry`, JCS/RFC-8785, `sig_alg`). **Option A**: workload audit is a **separate per-VM chain** `~/.mvm/audit/<tenant>.<vm>.workload.jsonl` in that format, signed under the **same host key** (one trust root), verified *additively* — the claim-8 lifecycle chain (`<tenant>.jsonl`) is untouched. **Per-VM, not per-tenant** (caught + maintainer-confirmed): the signer's `Chain` is single-writer (in-memory head, `O_APPEND`, no flock), so two VMs of one tenant can't co-write one file — each per-VM signer owns its own. Slice: `verify_workload_chain(path, &VerifyingKey) -> Result<usize, WorkloadVerifyError>` in `mvm-hostd::audit_signer::verify` (re-derive `entry_hash`, check the `prev_hash` link, **re-canonicalize** to confirm JCS, verify the Ed25519 sig over the canonical bytes, gate the category allow-list, reject non-Ed25519 `sig_alg`); extracted a shared `compute_entry_hash` so writer + verifier can't drift; put the per-VM path convention + matcher in `mvm-core::config` (`workload_audit_path` / `workload_audit_vm_name` / `WORKLOAD_AUDIT_SUFFIX`) so the writer (backend spawn, E5.3b-1) and this reader share one source. `mvmctl audit verify` now verifies the lifecycle chain **and** globs+verifies every `<tenant>.<vm>.workload.jsonl` against the host pubkey (nonzero on any failure). RED-first, 12 tests (10 verifier: clean-count, empty, wrong-key, tampered-sig, tampered-entry_hash, broken-prev-link, disallowed-category, unsupported-sig_alg, malformed-line, blank-line-skip; 2 path-helper round-trip/negative); existing `Chain` tests stay green through the refactor. `cargo test -p mvm-core`/`-p mvm-hostd` (908)/`-p mvm-cli` green, clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Remaining E5.3b: -1 per-VM `mvm-audit-signer` spawn (`software_chain_key_path`=host key + `workload_audit_path(tenant, vm)`), -2 broker spawn + ctx, -3 veneer, -4 live E2E.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-1 — per-VM `mvm-audit-signer` spawn helper**. `mvm-backend::broker_services_spawn::spawn_audit_signer` (sibling to `substitution_spawn`): emit the bin's `deny_unknown_fields` `SubprocessConfig` as raw JSON on stdin (mvm-backend has no mvm-hostd dep — the same `json!` approach substitution uses for `EndpointConfig`), `setsid`-detach, and — since the audit-signer emits **no stdout handshake**, it just binds — detect readiness by **polling for the UDS to appear** (write the PID file then; SIGKILL + fail closed on timeout); `reap_audit_signer` for the stop path. **Aligned to E5.3b-0's convention** (and caught a per-tenant path bug on the rebase): `audit_jsonl_path = workload_audit_path(tenant, vm)` — the per-VM `<tenant>.<vm>.workload.jsonl` the verifier checks, NOT the per-tenant lifecycle chain (the signer is single-writer, so two VMs of one tenant must not co-write); `software_chain_key_path` = the host-signer key (verifies against the same host pubkey as the lifecycle chain); secondary head per-VM under `state_dir`. RED-first, 3 stub-bin tests (config shape == `workload_audit_path`, fail-closed-on-no-bind, missing-bin). The gated `start()`/stop() wiring moves to **b2** (wired alongside the broker, so no idle audit-signer spawns before a consumer exists — zero behavior change on a real `up` until b2). `cargo test -p mvm-backend` green (0 failures), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Remaining E5.3b: -2 `mvm-broker` spawn + `BROKER_PORT`→UDS bind + `start()`/stop() wiring (both subprocesses, gated) + ctx enrichment (the round-trip unblocker), -3 veneer, -4 live E2E.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-2a — per-VM `mvm-broker` spawn helper**. `spawn_broker` in `mvm-backend::broker_services_spawn`, mirroring `spawn_audit_signer`: binds `vm_vsock_port_socket(name, BROKER_PORT)` (the per-VM socket the VMM forwards the guest's `connect_host_vsock(BROKER_PORT)` dial straight to), emits the broker bin's `deny_unknown_fields` `SubprocessConfig` as raw JSON on stdin carrying the **`audit_signer_uds_path`** (from b1's `AuditSignerHandle` — gates whether `host.audit.v1` registers) + the host-signer **pubkey**; `setsid` detach, UDS-poll readiness (no stdout handshake), PID file, `reap_broker`. Extracted a shared `spawn_detached_with_config` (and generalized `resolve_subprocess_bin` + `wait_for_uds`) so the audit-signer + broker spawns can't drift — the audit-signer path refactored onto it, its 3 tests stay green. RED-first broker test (config shape: `uds_path == vm_vsock_port_socket(name, BROKER_PORT)`, `audit_signer_uds_path` present, host-signer pubkey). Still unwired — the gated `start()`/stop() integration is **E5.3b-2b** (the actual round-trip unblocker), `ServiceCallCtx` enrichment is **-2c**. `cargo test -p mvm-backend` green (0 failures), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Remaining E5.3b: -2b start/stop wiring (libkrun+vz, gated) + -2c ctx enrichment, -3 veneer, -4 live E2E.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-2b-core — gated spawn of both broker-services subprocesses + RAII reaper**. `spawn_broker_services_if_admitted` in `mvm-backend::broker_services_spawn`: gates on `tenant_id` (unadmitted VM → defused no-op, so a stray `BROKER_PORT` dial gets `ECONNREFUSED`; admitted → spawn the **audit-signer then the broker**, ordered so the audit UDS exists before the broker's `host.audit.v1` is pointed at it), arming a `BrokerServicesGuard` **before** the broker spawn so a broker-spawn failure drops the guard and reaps the already-running audit-signer (fail closed). `BrokerServicesGuard` holds the `state_dir` and reaps both on drop until `defuse()` (mirrors `substitution_spawn::EndpointGuard`); `reap_broker_services` for the stop path. RED-first, 2 stub-bin gating tests (admitted → both PID files present via two stubs; unadmitted → nothing spawns, even with no bin overrides set). The libkrun/vz `start()`/stop() **call-site wiring is split out as E5.3b-2b-wire** — it needs grounding (is the `mvm-broker`/`mvm-audit-signer` bin available at runtime in a release mvmctl; should a broker-spawn failure fail-close the whole `up`?), so it gets its own slice rather than a rushed addition. Still zero behavior change on a real `up` (nothing calls the gate yet). `cargo test -p mvm-backend` green (0 failures), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Remaining E5.3b: -2b-wire start/stop call sites + -2c ctx enrichment, -3 veneer, -4 live E2E.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-2b-wire (libkrun) — broker services spawn on an admitted `up`**. Wired `spawn_broker_services_if_admitted` into `LibkrunBackend::start()` (right after the substitution-endpoint guard) + `stop()` (`reap_broker_services`), mirroring the substitution-endpoint guard pattern. **Grounded the two flagged questions:** the `mvm-broker`/`mvm-audit-signer` bins are workspace `[[bin]]`s (in `target/` for any source checkout — where b4's live E2E runs) with no confirmed release-shipping, same status as the substitution endpoint. **Resolved to best-effort, not fail-closed** (user-confirmed): an absent broker only disables `host.audit.v1` (the guest's emit fails) while the workload still runs and the **system** audit chain — written host-side, not via the broker — stays intact, so a spawn failure is logged (`warn`) and the guard defused, never a launch rollback. (Contrast the substitution endpoint's fail-closed posture, right there because a missing endpoint *with secrets present* is a leak.) On success the `BrokerServicesGuard` still reaps both subprocesses if a *later* start step fails; it's defused once the VM is up and `stop()` owns teardown. **This is the round-trip unblocker on libkrun** — an admitted libkrun `up` now spawns the broker + audit-signer, binds `BROKER_PORT`, and a guest `mvm.audit.emit` reaches the host → per-VM workload chain (verified by E5.3b-0's `verify_workload_chain`). No unit test for the call site itself (start() needs a real VM, like the substitution wiring) — compile + clippy-checked, exercised by b4. `cargo test -p mvm-backend` green (0 failures, no new side effects — no unit test drives `start()` with a tenant), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Remaining E5.3b: -2b-wire **vz** (same wiring), -2c ctx enrichment, -3 veneer, -4 live E2E (the round-trip is now live-testable on libkrun).
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-2b-wire (vz) — completes the start/stop wiring on both workload backends**. Mirrored the libkrun change in `VzBackend::start()` (best-effort `spawn_broker_services_if_admitted` right after the substitution-endpoint guard; `Err` → warn + defused guard, no rollback; `defuse()` once up) and `stop()` (`reap_broker_services`). Identical pattern + rationale (an absent broker only disables `host.audit.v1`; the system audit chain is independent). So an admitted **vz** `up` now spawns the broker + audit-signer and binds `BROKER_PORT` too — the round-trip is live on both libkrun and vz (the two workload backends; FC/qemu carry no broker by design, like the gateway-audit substrate). `cargo test -p mvm-backend` green (0 failures), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. **E5.3b-2b is complete.** Remaining E5.3b: -2c `ServiceCallCtx` enrichment (mvm-hostd: correlation rewrite / session / profile — the broker server uses a stub ctx today), -3 codegen/pure-language veneer, -4 live-VM E2E on the box (now validates b1→b2b end-to-end).
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-2c (correlation rewrite) — the broker stops trusting the guest's correlation id**. `mvm-broker`'s `handle_connection` (`mvm-hostd::broker::server`) now mints a **server-authoritative** `correlation_id` at frame ingress (`mint_correlation_id`: process-id prefix + monotonic counter, no new dep, opaque/string so a future ULID swap is serde-compatible) and uses it for the `ServiceCallCtx` (hence the chain-signed audit entry — `host_audit_v1`'s `build_append_entry` reads `ctx.correlation_id`) **and** the response. The guest-supplied `ServiceCall.correlation_id` is never trusted/echoed — a workload could otherwise choose an id that collides with or impersonates another entry in the audit chain. The guest doesn't match on it (one call/connection), so reassignment is transparent. TDD: flipped the existing `round_trips_…` server test from asserting the echoed guest id to asserting a server-minted one (`!= guest`, `starts_with "brk-"`) — watched it go red on the old echo code, green after. **session_id + profile enrichment deferred** (a `SubprocessConfig` + serve-signature + backend-spawn cascade for fields that don't gate the only registered handler, `host.audit.v1`; they ride when the profile-gating time/cost handlers land). `cargo test -p mvm-hostd` green (0 failures), clippy `-D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` + `check-core-runtime-free` + `cargo check --workspace --all-targets` clean. Remaining E5.3b: -2c session/profile (deferred), -3 codegen/pure-language veneer, -4 live-VM E2E (validates b1→b2b on a real VM; needs b3 for the headline `mvm.audit.emit` test + a libkrun/vz venue).
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-3a — in-guest host-services SDK type codegen**. The veneer goes the codegen route, not PyO3/napi: feature-gated JsonSchema derives on the broker wire types, a new `mvm-core` `emit_broker_schema` bin, `gen-stubs` output for `schema/broker-services-v0.json`, generated Python/TypeScript broker service types, `xtask check-stubs` no-drift, default closure schemars-free, and `cargo check -p mvm-core` with/without `schema` clean.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-3b (Python) — in-guest host-services veneer**. Added pure-Python `mvm._broker.transport` over lazy `AF_VSOCK(HOST_CID:BROKER_PORT)` length-framed `ServiceCall`, plus `mvm.audit.emit`/`emit_batch` and `mvm.host.time()`/`cost()` over generated types with typed service exceptions and injectable test transport. Full Python suite 167 passed / 7 skipped, ruff clean. TypeScript veneer is deferred until the transport decision because Node has no native `AF_VSOCK`.

- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **E5.3b host-spine integration test — the round-trip proven end-to-end with real subprocesses**. `crates/mvm-hostd/tests/broker_audit_round_trip.rs` spawns the real `mvm-broker` + `mvm-audit-signer` bins (resolved via `env!("CARGO_BIN_EXE_*")`, so `cargo test -p mvm-hostd` builds them — no target-guessing, no skip-logic), readiness via UDS-poll (neither bin emits a stdout handshake), connects a client to the broker UDS, sends a `host.audit.v1::emit` `ServiceCall`, and then **`verify_workload_chain`s the result against the host-signer pubkey** (==1 entry). Proves the whole E5.3b host spine — spawn → broker dispatch → audit-signer chain-sign → `workload_audit`-categorised, host-key-signed, verifiable per-VM chain (b1→b2c) — end to end, real processes, **no VM and no veneer**, fully deterministic / CI-runnable. (Chain key + verify pubkey are the one `host_keypair::load_or_init_at` keypair; all paths explicit in a tempdir, no HOME dependency.) `cargo test -p mvm-hostd --test broker_audit_round_trip` green; clippy `-p mvm-hostd --all-targets -D warnings` + nightly fmt `--all` + `check-no-spec-refs-in-comments` clean. Test-only addition. **Per owner direction: E5.3b-3 veneer is an in-process LIBRARY call (typed request/IR → linked fn), NOT a subprocess (shell exec = injection + cross-platform hazard).** Remaining E5.3b: -2c session/profile (deferred), -3 library veneer, -4 live-VM E2E (vz-on-Mac or libkrun-on-box; needs b3 for the Python `mvm.audit.emit`).
- [x] Landed [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-1 (proven) + WS-1.5 (parity-gate scaffold + CI lane)**. WS-1: a live `mvmctl dev up` through **rvproxy** on macOS/libkrun built the builder-VM rootfs cold (~540k egress connections relayed; DHCP + DNS + sustained download) and reached "Dev environment ready" — the libkrun-`unixgram` transport is confirmed. Took three rvproxy fixes (sibling repo): DNS reply sourced from the gateway IP (rvproxy #38), guest-bound TCP segmented to the MTU (#42 — a full-size response was EMSGSIZE-tearing the vfkit unixgram transport), and read/write timeouts cut to per-poll budgets (#53 — a 30s read froze the single-threaded pump ~30s per idle poll, crawling the boot). WS-1.5: `scripts/rvproxy-gateway-parity.sh` ran the claim-10 / flow-audit / Plan-129-substitution witness families plus the one binary-discriminating conformance witness (`gvproxy_dhcp_offer_roundtrips_through_bridge`) against **both** gvproxy (control) and rvproxy (candidate via `MVM_GATEWAY_BIN`), refusing the default flip unless rvproxy genuinely ran and passed — validated head-to-head on macOS (both PASS) and negatively (a non-gateway binary → REFUSED). Historical CI lane `.github/workflows/rvproxy-parity.yml` ran on macos-latest (ADR-038: ~10× ubuntu cost, so paths-filtered off unrelated PRs), built the candidate from a pinned rvproxy rev (`RVPROXY_DEFAULT_REF`, `workflow_dispatch`-overridable) via the `RVPROXY_CHECKOUT_TOKEN` repo secret, and failed closed without it. Secret provisioned + validated green end-to-end in CI 2026-06-15 (gvproxy PASS + rvproxy PASS + enforcement witnesses green), then promoted to `workflow_dispatch` + a paths-filtered `pull_request` trigger on the gateway-contract files. Retired 2026-06-30; see the retirement entry below for current status.
...
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-2.2d cleanup before splice deletion**. Removed the dead `BridgeConfig.policy` member and production `AllowAll` `FlowPolicy`: `run_bridge_inner` already derives the live gate from the resolved bundle or threaded bare `NetworkPolicy`, with no-bundle/no-policy failing closed to deny-all, so the field was write-only and risky to leave around before deleting the splice. The four supervisor-bin construction sites no longer pass an ignored policy, and gateway-bridge tests use `PlanFlowPolicy::from_network_policy(NetworkPolicy::unrestricted())` when they need an intentional open-mode gate. This is behavior-preserving cleanup; remaining Plan-141 hook removal is now gated only by transparent terminator support because the parity gate is required.
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-2.2d native-default selection**. macOS/libkrun now selects `NetworkingPreference::Native` by default when `MVM_GATEWAY_BIN` names the rvproxy candidate, so the existing supervisor native path renders `rvproxy.toml`, launches `rvproxy run --config`, and tails native flow-audit without requiring `MVM_NETWORKING=native`. Safety fallbacks are pinned by tests: hosts without `MVM_GATEWAY_BIN` keep gvproxy, Linux remains passt by default, and `MVM_NETWORKING=gvproxy` still overrides the native default. This does not delete the splice; Plan-141 hook removal now waits on transparent terminator support because the parity gate is required.
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-2.2d admitted-bridge default status**. Reconciled the remaining-work list with the already-landed Plan 200 handoff: admitted libkrun/Vz workload starts already thread the signed plan by default, so there is no separate future `MVM_GATEWAY_BRIDGE` default flip for those backends. Added a regression check that Firecracker's separate bridge sidecar remains explicit because default Firecracker egress is enforced by nftables. Superseded by the transparent-terminator requirement guard below: remaining Plan 193 work is rvproxy transparent-terminator schema/support, mvm wiring once that exists, then splice/Plan-141 hook deletion.
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-2.2d transparent-terminator requirement**. `EgressSubstitutionTransport` now separates proxy-aware substitution from transparent `:80`/`:443` terminator support, and a tested `require_transparent_egress_terminator(&dyn WorkloadBackend)` gate accepts Firecracker's nft terminator while refusing libkrun/Vz/mock. This is the mvm-side deletion guard: macOS rvproxy interception is still the implementation prerequisite before the splice and Plan-141 hooks can be removed. Remaining Plan 193 work is rvproxy schema/support first, mvm wiring once that exists, then splice/Plan-141 hook deletion.
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-1.5 required-check shim** (historical). `.github/workflows/rvproxy-parity.yml` ran on every PR and merge-group SHA, and emitted a stable `rvproxy gateway parity` job suitable for branch protection. A cheap Ubuntu changes job skipped unrelated PRs/merge groups without spending macOS minutes; gateway-contract PRs/merge groups had to pass the macOS `rvproxy vs gvproxy parity` candidate/control run. Retired 2026-06-30; see the next entry for current status.
- [x] Retired [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-1.5 required-check shim** (2026-06-30). The old cross-repo `rvproxy`/`gvproxy` parity lane no longer matches the active gateway strategy, so `.github/workflows/rvproxy-parity.yml` and `scripts/rvproxy-gateway-parity.sh` are removed and `rvproxy gateway parity` is removed from `main` branch protection. Future gateway proof stays in the active backend/security CI gates instead of an always-present shim.
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-2.2d transparent-terminator blocker classification**. The local rvproxy schema has no typed transparent `:80`/`:443` interception section or host-terminator destination yet (`RvproxyConfig` exposes network/backend/transport/api/dns/policy/audit/transforms; `GatewayConfig` exposes policy/flow fields only). mvm therefore keeps libkrun/Vz at `VsockUdsChannel` and cannot honestly satisfy `supports_transparent_terminator()` for them until rvproxy lands the contract. Remaining Plan 193 sequence is rvproxy schema/implementation first, then mvm config/wiring + tests, then splice/Plan-141 hook deletion.
- [x] Advanced [`plans/193-rvproxy-network-substrate.md`](plans/193-rvproxy-network-substrate.md) **WS-2.2d libkrun transparent-terminator wiring**. rvproxy's transparent interception contract is now available, and mvm wires it on the libkrun native path: `render_rvproxy_config` emits `[transparent]`, the libkrun supervisor carries a deterministic per-VM loopback terminator port, `mvm-substitution-endpoint` accepts rvproxy's original-destination preamble, and `require_transparent_egress_terminator` now accepts libkrun. Vz remains guarded because its launch path does not yet emit/use native rvproxy config. Remaining Plan 193 sequence is Vz wiring, the live allow/deny matrix, then splice/Plan-141 hook deletion.
...
**2026-06-20: default-path Vz warm-claim fixed (#1112 / #1101).** The 2026-06-13 "Claimed a warm standby" record above only ever exercised the explicit `--warm-pool-size` path. On the default dev/transient path (`up --dev`, transient `run`) the claim silently fell through to a cold boot: `resolve_warm_shape` baked the standby's `kernel_sha256` from the **prod** image, but those launches resolve their kernel from the **dev** image, so `StandbyHandle::is_compatible` was always false despite matching image/vcpu/mem. The standby's kernel is now paired to its rootfs variant (dev rootfs ↔ dev `vmlinux`, prod ↔ prod — the same sibling the claiming launch boots), with a default-kernel fallback for a non-default `--rootfs`. libkrun is unaffected (image-agnostic kernel). New unit test `paired_kernel_prefers_sibling_vmlinux_else_default`; live on macOS 26 a dev-rootfs standby now records the dev kernel sha and `up --dev --hypervisor vz --warm-pool-size 1` logs the claim instead of cold-booting.
...
> **Process-model revision 2026-06-16 — ADR-084 / [Plan 202](plans/202-host-services-daemon.md) (merged #977).** The broker/audit-signer that actually shipped (via Plan 125 E5.3b) became **per-VM detached subprocesses** forked on each admitted `up` — `2N` host processes for `N` VMs, plus a per-boot fork/bind/poll cycle, with `host.audit.v1` availability coupled to `MVM_GATEWAY_BRIDGE`. Grounding the first live in-guest round-trip (E5.3b-4, proven on libkrun, #973) showed this fails mvmd density and silently disables audit on a normal launch. ADR-084 re-architects host services to **two long-lived per-tenant daemons** that VMs register/deregister with: `O(active tenants)` processes not `O(VMs)`; the moat (keyless broker / key-holding signer, separate address spaces) and claims 12/13 preserved; the guest-facing wire unchanged; registration driven by the admitted plan (decoupled from the egress bridge); **mvmd consumes the same daemon per tenant**, so local mvm is a single-tenant slice of production. This supersedes ADR-059's in-process model and the per-VM spawn — Plan 202 phases the migration and retires `spawn_broker_services_if_admitted`. The W1 "three-subprocess" framing below is read through that lens: same roles + moat, daemon lifetime instead of per-VM fork.
...
>
> **Plan 202 vz live verification (2026-06-18):** default-on host services are now proven on vz as well as libkrun. A direct-boot `audit-probe` VM (`tenant=p202vz`, `vm=p202vzprobe`) ran with no `MVM_GATEWAY_BRIDGE` and no `MVM_HOST_AGENT_DAEMON` override, emitted 22 workload entries through `host.audit.v1`, verified clean with `mvmctl trust audit verify --tenant p202vz` (4 lifecycle entries + 22 workload entries), and deregistration left the per-tenant registration journal empty. The flake builder path was blocked by a builder-VM `BadActivate` before workload boot, so this proof isolates the vz runtime path by using a previously built `audit-probe` rootfs with `MVM_DIRECT_BOOT=1`.
>
> **Plan 202 Phase 2a/2b (2026-06-17):** signer-helper daemonization base and host-agent forwarding landed. `mvm-signer-helper` is a resident per-tenant child of `mvm-host-agent`, owns `vm_id → Chain` heads, opens/closes per-VM workload chains on register/deregister, routes appends by server-derived `vm_id`, refuses tenant/workload identity mismatches before signing, and keeps tenant key material out of the host-agent address space. Remaining Phase 2 slices move admission signing and restart/head rebuild onto the helper.
...
> **Plan 202 closeout (2026-06-19):** cross-repo Phase 5 is now complete through mvmd PR #162. mvmd owns one per-tenant `mvm-host-agent` + `mvm-signer-helper` pair, replays registrations from `host-agent.tenant` markers, binds Firecracker guest→host broker traffic on `runtime/v.sock_5300`, validates tenant/pool/instance IDs before any marker-derived recovery, and carries the five low-risk delegated tenant-scoped reads (`host.cost.v1::tenant`, `host.catalog.v1::tenant`, `host.peers.v1::tenant`, `host.config.v1::tenant`, `host.rate_budget.v1::tenant`) over the native typed ALPN protocol with centralized tenant/workload authz, replay refusal, and strict malformed-response rejection. mvmd-side density accounting now proves the resident host-services target is `active_tenants * 2`, not `O(VMs)`, and the tenant-key boundary is pinned by refusing forged cross-tenant marker recovery before re-registration. ADR-084 is accepted and Plan 202 is complete.
...
- **D — Snapshot park/resume**  ✅ #1099 — `StandbyState::Parked`; reaper demotes idle
  ...
  stale snapshot markers instead of accidentally resuming. The Vz dev-builder now records
...
- [x] Task 3: `--agent-verb VERB` (repeatable) added to `MachineRunArgs` (the live `machine run` surface; the flag also covers transient runs via `RunArgs.agent_verb` threaded into the admit closure). `AdmitPlanForBootParams` gains `agent_verb_override: Vec<String>` + `is_sealed_prod: bool`; `SynthesisInput.agent_verbs` wired at `admit_plan_for_boot` as `parse_agent_verb_override(&p.agent_verb_override)?.or_else(|| default_agent_verbs(p.is_sealed_prod, !p.shares.is_empty()))`. `MachineSpec.agent_verb` persists the CLI override for persistent machines; `PersistentImageStartParams.agent_verb` threads it to the admit site; `start_persistent_oci_machine` derives `is_sealed_prod = profile != "dev"`. Test `up_populates_agent_verbs_default_and_override` in `up.rs`; `agent_verb_flag_persisted_in_spec_and_survives_roundtrip` + `agent_verb_forwarded_to_run_args_on_transient_path` + `agent_verb_empty_on_transient_path_when_not_specified` in `machine/mod.rs`. **Note:** runtime enforcement only activates once the grant-delivery path (PR #1385) lands — populating the field is a prerequisite, not sufficient alone.

### Plan 224 Phase 1 — `machine reconfigure` verb — COMPLETE (2026-07-05)

- [x] **CLI verb** `mvmctl machine reconfigure <name>` on branch `worktree-machine-reconfigure`. Flags: `--net`/`--no-net`, `--allow-host <host[:port]>` (repeatable), `--clear-allow-host`, `--cpus <n>`, `--memory <size>`, `--mem-initial <size>`. Patch semantics: only the flags the user passes are changed; image, volumes, profile, and all other fields are inherited. Running machine → auto stop + start (re-synthesizes a fresh signed `ExecutionPlan` via the existing `admit_plan_for_boot` path); stopped machine → persist only, staged for the next `machine start`.
- [x] **`MvmClient` facade** (`crates/mvm-client`): `reconfigure_machine(&MachineId, ReconfigureRequest) -> Result<MachineState>` trait method; `ReconfigureRequest` DTO in `dto.rs` (fields `net: Option<bool>`, `allow_host: Option<Vec<String>>`, `cpus: Option<u32>`, `memory_mib: Option<u32>` — the common four; `mem_initial` is CLI-only and not in the DTO). `#[serde(skip_serializing_if = "Option::is_none")]` on every field (patch semantics over the wire).
- [x] **`GatewayBackend`** (`gateway.rs`): `POST /api/v1/sandboxes/{id}/reconfigure` with the `ReconfigureRequest` body; response parsed as the standard single-item sandbox envelope `{ "data": { <SandboxDto> } }`. Coordination note for the mvmd server handler: `specs/notes/2026-07-05-machine-reconfigure-mvmd-endpoint.md`.
