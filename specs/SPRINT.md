...
- [x] Implemented [`plans/202-host-services-daemon.md`](plans/202-host-services-daemon.md) Phase 2a — signer-helper daemonization base. Added the shared `SignerHelperRequest`/`SignerHelperResponse` wire (`register_vm`, `deregister_vm`, `append_entry`, `probe`) in `mvm-core`, a new `mvm-hostd::audit_signer::helper::SignerHelper` that owns a `vm_id → Chain` map, and a `mvm-signer-helper` bin. `mvm-host-agent` now starts the helper as a supervised child, passes only the tenant key path across the moat, and waits for `signer-helper.sock` before serving control; the host-agent still does not read the key. Tests cover protocol roundtrips/unknown-field refusal, helper config defaults, per-VM chain routing, deregister close/drop behavior, cross-tenant refusal, and unsafe `vm_id` refusal. Remaining Plan 202 Phase 2 work: 2b host-agent→helper forwarding, 2c admission signing via helper, 2d restart/head rebuild.
...
>
> **Plan 202 Phase 2a (2026-06-17):** signer-helper daemonization base landed. `mvm-signer-helper` is a resident per-tenant child of `mvm-host-agent`, owns `vm_id → Chain` heads, opens/closes per-VM workload chains on register/deregister, and keeps tenant key material out of the host-agent address space. Remaining Phase 2 slices wire host-agent audit forwarding, admission signing, and restart/head rebuild.
