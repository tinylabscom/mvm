...
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-3a — in-guest host-services SDK type codegen** (the auto-generation foundation for the veneer). **The veneer goes the codegen route, not PyO3/napi** — owner preference is "auto-generate the SDK as much as possible," and the repo already has Plan 124 D's schema-codegen (`xtask gen-stubs`/`check-stubs`: Rust types --schemars→ JSON Schema --datamodel-codegen / json-schema-to-typescript→ `sdks/*/`). So this slice extends that pipeline rather than adding a Rust↔language binding (zero new `pyo3`/`napi` deps — each language gets a generated client type set + a thin pure-language vsock veneer in -3b). Added feature-gated `#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]` to the broker wire types (`ServiceCall`/`ServiceResponse`/`ServiceErrorCode`/`ServiceId`/`CorrelationId` in `broker.rs`; `EmitRequest`/`EmitResponse`/`EmitBatchRequest`/`EmitBatchResponse`/`EmitBatchEntryStatus`/`EmitErrorCode` in `host_audit.rs`; `TimeNowResponse`; `CostReport`), a new `mvm-core` `emit_broker_schema` bin (`required-features = ["schema"]`, mirroring `mvm-guest`'s `emit_protocol_schema` — keeps `schemars` out of the default closure), and a `gen-stubs` `StubArtifact` → `schema/broker-services-v0.json` + generated `sdks/python/mvm/_broker/services.py` (+ a hand-written `_broker/__init__.py` re-export, mirroring `_protocol`) + `sdks/typescript/src/broker/services.ts`. **Option B** (a focused broker-services schema, not widening `protocol-v0.json`) so in-guest workloads don't drag in the unrelated `GuestRequest`/`GuestResponse` agent verbs. Gates: `cargo check -p mvm-core` (default — derives gated off) + `--features schema` both build; `xtask check-stubs` **no drift**; `check-core-runtime-free` clean (default closure stays schemars-free); `cargo test -p mvm-core` (118 protocol tests green with the derives); clippy `-D warnings` + nightly fmt `--all` + `cargo check --workspace --all-targets` clean; Python `import mvm._broker` OK (TS typecheck is `npm`-gated/CI — the generated `services.ts` is pure json-schema-to-typescript output, same generator as the compiling `protocol.ts`). Remaining E5.3b: -2c ctx enrichment, **-3b** the thin pure-Python/TS `AF_VSOCK(BROKER_PORT)` veneer (`mvm.audit.emit`/`emit_batch`, `mvm.host.time()`, `mvm.host.cost()`) over these generated types, -4 live E2E.
- [x] Landed [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) Task E5 **slice E5.3b-3b (Python) — the in-guest host-services veneer** over E5.3b-3a's generated types. `mvm/_broker/transport.py` (the one hand-written piece — the codegen emits *types*, never a client) opens an `AF_VSOCK` stream to `HOST_CID(2):BROKER_PORT(5300)`, writes a 4-byte-BE-length-prefixed JSON `ServiceCall`, reads the framed `ServiceResponse` (256 KiB read cap mirroring the guest), and maps `kind:"err"` → `BrokerServiceError(code,message)` / socket+framing faults → `BrokerTransportError`; **`AF_VSOCK` is resolved lazily** (`getattr(socket,"AF_VSOCK")` inside connect — macOS/host imports never touch it; it exists only inside a Linux guest) and the `connect` factory is **injectable** so tests drive a mock broker over an `AF_UNIX` socket pair (mirrors the Rust `_on` stream variants). `mvm/audit.py` (`emit`/`emit_batch` → `EmitResponse`/`EmitBatchResponse`, `ServiceErrorCode` → typed `AuditBadRequest`/`AuditRateLimited`/`AuditUnavailable`; the record carries **no category** so a workload can't write a host-category entry) + `mvm/host.py` (`time()` → `wall_ms`, `cost(scope)` → `spent_micros_usd`, verb-scoped) surfaced on the `mvm` package (`import mvm; mvm.audit.emit(...)`). 10 new tests (emit envelope+chain_head, bad_request/rate_limit typed mapping, emit_batch + bare-fields wrapping, time, cost workload+tenant, unknown-scope ValueError, service-error map, malformed→transport, top-level reachability); **full Python suite 167 passed / 7 skipped, ruff clean**. **E5.3b-3c (TypeScript) deferred for cause**: Node has no native `AF_VSOCK` (and `mvm-addon-vsock-bridge` is TCP↔vsock *per configured binding*, not a generic path) — the TS veneer needs a transport decision first (native vsock addon, weighed under ADR-002, vs a `BROKER_PORT` TCP↔vsock bridge binding baked into the image so both languages dial a localhost TCP port); shipping a TS surface whose real transport throws would be worse than deferring. Stacked on E5.3b-3a (no Rust change in this slice). Remaining E5.3b: -2c ctx enrichment, -3c TS veneer (transport-gated), -4 live E2E (venue = local libkrun/vz, not the FC box).
