...
- [ ] 1d `mvm-net`: fold `mvm-network` + host tunnel/gateway/dns + guest net; vsock/UDS transport + egress seam. _(crate rename `mvm-network`→`mvm-net` and the bounded no_std A/AAAA DNS codec landed; host DNS handler/tunnel/guest-net absorption pending)_
