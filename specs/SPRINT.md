...
- [ ] 1d `mvm-net`: fold `mvm-network` + host tunnel/gateway/dns + guest net; vsock/UDS transport + egress seam. _(crate rename `mvm-network`→`mvm-net`, the bounded no_std A/AAAA DNS codec, and its 100k-run fuzz target landed; host DNS handler/tunnel/guest-net absorption pending)_
