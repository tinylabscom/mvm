...
- [~] **P3** — the governed egress seam. Design SIMPLIFIED (`bf3eac389`, doc 11): NO new transport — wasm egress is just a `WireRequest` client (`mvm_core::substitution_wire`) over the existing `Uds` to the SAME substitution endpoint the microVM backends use (faithful by construction). Reachability recon (`127b22e44`) settled it: governance lives in mvm-hostd (unreachable in-process from WasmBackend in mvm-runtime), so route through the subprocess endpoint, not a parallel in-process path.
  ...
  - [ ] **P3b**: WasmBackend `start()` spawns the REAL substitution endpoint (`spawn_substitution_endpoint`, Uds) + sets the host-import's endpoint path → the **data-governance witness** (must be built — referenced specs 04/06, not yet a concrete test): a WASI module's egress default-denied-then-policy-allowed, chain-signed audit entry, module never holds the real secret (only `${NAME}`) — the same witness the microVM backends pass. (Full TLS-terminating substitution deferred to P3c.)
