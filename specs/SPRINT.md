...
- [x] Closed out [`plans/125-cli-sdk-dx-surface.md`](plans/125-cli-sdk-dx-surface.md) **Phase A** (52 flat verbs → ≤15 nested groups) as **SATISFIED** — docs/scope amendment, no code churn. The "52 flat verbs" premise is stale: the sprawling-flat-CLI was already grouped incrementally by other plans. `main` exposes ~12 real nested groups (`env`/`ops`/`build`/`vm`/`trust`/`image`/`storage`/`manifest`/`deps`/`pool`/`bundle`/`catalog`) + the deliberately-kept top-level convenience verbs (`up`/`down`/`run`/`invoke`/`logs`/`ls`/`console`/`dev`/`doctor`/`init`). Audited every candidate fold and found none clean+safe: hitting a literal ≤15 would fold those conveniences (contradicts the phase's own rule) or merge distinct concepts (`catalog` browse vs `image` OCI runner); the only other flat entries (`shell-init`/`reconcile`/`persistent-builder`/`__qemu-vsock-bridge`) are `hide=true` internal subprocess entrypoints spawned by argv (and referenced in hint text / shell-rc `eval`) that must stay top-level. A breaking re-fold for a marginal count cut is net-negative under the no-shims constraint, so the literal ≤15 is amended to "grouped surface + deliberate conveniences" and A1/A2 + the matching Acceptance items are ticked. **Plan 125 now has one piece left: E5 host-services SDK.**
