...
2. **Wall clock is enforced only where a supervisor process holds the plan.
   ...
   `fn:a_wall_clock_bound_needs_a_clock_that_can_stop_the_workload`, and
   `fn:a_bounded_admitted_plan_arms_the_timer` — the last covers the arming
   step against the exact carrier the launch path sends (the signed envelope),
   which the other two do not: they exercise the timer and the capability
   answer from an in-memory plan, so a supervisor that could not decode its own
   admitted plan off the wire passed both.
