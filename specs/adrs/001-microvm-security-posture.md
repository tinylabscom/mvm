...
The guest agent starts as uid 901 through `mvm-setpriv --no-new-privs` and
narrows its Linux capability bounding set through `PR_CAPBSET_DROP`; the
host-side vsock proxy socket is mode 0700; the proxy's port allowlist admits
only the agent port and the declared forward range; console and daemon logs
are mode 0600; and `~/.mvm` and `~/.cache/mvm` are mode 0700.
...
|---|---|---|---|
| 1 | No host-fs access from a guest beyond explicit shares | L2/L5 | Per-service uid; seccomp tier `standard` default; `mvm-setpriv --no-new-privs` plus agent-side `PR_CAPBSET_DROP`; user-volume allow-list with a read-only default; admission-enforced share matching |
| 2 | No guest binary can elevate to uid 0 | L2/L4 | `mvm-setpriv --no-new-privs`; `/etc/{passwd,group,nsswitch.conf}` are read-only bind mounts, so a compromised service cannot mint a uid-0 entry |
...
| 10 | No untrusted workload reaches the network unless explicitly admitted by policy | data containment | `NetworkPolicy` defaults to deny-all; no workload guest has a NIC, so every claim-bearing backend's egress leaves over vsock to one per-VM network endpoint, where `EgressGate` decides every connect, datagram, and DNS answer against the admitted policy's canonical projection, refusing the mandatory-deny ranges and TCP/22 under every policy, and private ranges under any policy that does not name them |
...
- The production guest closure carries the dedicated static `mvm-setpriv`
  helper for the per-service privilege drop.
...
|----|-------|-----------|-----------|--------|
| 1  | No host-fs access from a guest beyond explicit shares | fn:seccomp_allows_listed_denies_unlisted, ci:seccomp-functional, fn:validated_conversion_enforces_mount_allow_list, fn:bare_mnt_is_refused_because_it_shadows_the_config_drive, fn:dir_share_two_part_defaults_ro, fn:libkrun_refuses_any_virtio_fs_share_before_mapping, fn:enforce_admitted_shares_refuses_unadmitted_or_mismatched | seccomp + `mvm-setpriv --no-new-privs` + agent-side `PR_CAPBSET_DROP` + user-volume allow-list, refusing both descendants and ancestors of a runtime path / ro-default / admission-enforced shares (mvm-core + mvm-cli + mvm-backend) | Shipped |
| 2  | No guest binary can elevate to uid 0 | fn:set_no_new_privs, fn:virtiofs_mount_flags_keep_workspace_read_only, ci:check-abi-layout | `mvm-setpriv --no-new-privs` + RO config binds | Shipped |
| 3  | A tampered rootfs ext4 fails to boot | ci:verified-boot-artifacts, fn:verify_and_resume_rejects_tampered_mem, ci:check-abi-layout | dm-verity + roothash on **block+ext4** backends — Firecracker + Option B; the restore path also verifies the sealed snapshot envelope (HMAC + epoch) before resuming. A virtiofs root serves a trusted-host directory rather than a block device and therefore does **not** witness this claim; see "Claim 3 backend scoping" below | Shipped |
| 4  | A production-safe run cannot invoke DevOnly guest-agent verbs | fn:prod_safe_grant_refuses_all_dev_only_requests, ci:guest-agent-runtime-boundary | runtime profile + signed VerbGrant intersection | Shipped |
| 5  | Vsock framing + supervisor-config JSON + FlowMux decode/state are fuzzed | ci:fuzz_guest_request, ci:fuzz_sealed_frame, ci:fuzz_supervisor_config, ci:fuzz_network_flow_decode, ci:fuzz_network_flow_state, ci:fuzz_input_frame | cargo-fuzz | Shipped |
| 6  | The pre-built dev image is hash-verified | ci:hash-verify-tests, fn:download_runtime_overlay_rejects_checksum_mismatch | SHA-256 manifest | Shipped |
| 7  | Cargo deps are audited on every PR | ci:cargo-deny, ci:cargo-audit, ci:reproducibility | RUSTSEC + deny.toml + reproducibility double-build | Shipped |
| 8  | Every workload runs from a signed, audited ExecutionPlan | fn:synthesize_plan, fn:admit_for_run, fn:verify_audit_chain, fn:naively_dropping_old_entries_fails_verification_at_line_zero, fn:a_prune_record_that_over_claims_is_refused, fn:the_same_deletion_without_a_record_is_still_refused, fn:pruning_a_broken_chain_is_refused_before_anything_is_deleted, fn:a_spliced_segment_is_refused, fn:a_missing_segment_is_named_not_silently_skipped, fn:an_interrupted_rotation_continues_history_instead_of_restarting_it, fn:a_correctly_signed_seal_that_lies_is_refused_field_by_field, fn:truncating_through_the_seal_leaves_the_session_unsealed | Ed25519 + chain-signed audit log (ADR-014). The chain may be rotated into sequenced segments (Plan 319): `verify_audit_chain` now attests an unbroken chain from genesis **or from a signed handoff naming its predecessor segment and that predecessor's final chain hash**, and `verify_segment_set` attests the ordering and completeness of the segment set. A retired prefix may also be deliberately pruned (Plan 326): the chain then verifies **with a corroborated gap** rather than whole, and the prune record may only claim what the surviving handoff independently attests, so it cannot relabel an edit as a removal. Only the upper boundary of a pruned range is cross-checked. Each session that reaches its end is closed by a chain-signed `session.sealed` record (entry count, first/last entry, chain head, RFC 6962 root over the session's entries, link to the previous seal) that `trust audit verify <session>` holds to the chain field by field; truncating through a seal turns the session `UNSEALED`. Tail truncation of whole sessions, and of a session that never sealed, stays undetectable without an off-host witnessed root | Shipped |
...
| 10 | No untrusted workload reaches the network unless policy-admitted | fn:policy_default_is_deny_all, fn:run_net_default_is_deny_all, ci:single-network-path, ci:fuzz_dns_codec, fn:private_link_local_loopback_ula_metadata_are_forbidden, fn:emits_resolved_query_with_ip_list, fn:admitted_projection_is_one_object_graph_for_every_network_surface, fn:assert_vsock_only_device_model, fn:verify_and_resume_refuses_nic_on_restore, fn:fork_restore_refuses_nic, fn:private_range_is_denied_by_default_and_readmitted_only_by_a_grant_naming_it, fn:metadata_and_the_absolute_ranges_are_never_readmitted, fn:a_name_that_resolves_inward_is_refused_as_the_address_it_reached, fn:an_embedded_ipv4_form_is_classified_by_the_address_it_reaches, fn:the_forward_leg_gets_the_gate_s_answer_not_a_fresh_lookup | one authenticated FlowMux endpoint + one admitted policy/budget/identity/audit projection + permanent single-path/socket-owner gate + bounded DNS codec fuzzing + DNS-answer SSRF/rebinding filtering + chain-signed per-query DNS audit; warm-restore and fork-restore refuse any restored device model carrying a NIC before resuming vCPUs + private-range default deny (RFC1918, ULA, multicast, reserved; re-admitted only by a grant naming the address) and absolute deny of metadata, loopback, link-local, CGNAT, the unspecified block and embedded IPv4 forms, each refusal audited with its class + the forward leg connects only to gate-admitted addresses | Shipped |
...
| 20 | Every published release artifact is authenticated under the release workflow's identity, directly or through a signed checksum manifest, and the build, fetch, install, and self-update paths refuse an artifact whose required signature is missing or invalid | ci:verify-release, ci:release-provenance, fn:accepted_identities_are_the_versioned_release_workflow, fn:a_missing_bundle_refuses_and_names_the_asset, fn:fetch_expected_hashes_refuses_an_unsigned_manifest_before_parsing, fn:skip_hash_verify_does_not_waive_the_manifest_signature, fn:an_archive_without_a_bundle_is_refused, fn:a_real_release_bundle_verifies_under_its_tag, fn:fresh_install_trust_precedes_verifier_execution, fn:fresh_install_requires_tag_pinned_bundle, fn:fresh_install_has_no_unsigned_fallback | keyless cosign over release archives and checksum manifests with Fulcio + Rekor bundles; raw kernels, root filesystems, and metadata are authenticated by digest entries in those signed manifests. The `verify-release` job re-verifies the signatures after publication and checks each covered blob; the build gate refuses a missing or malformed bundle and the fetch gate refuses an unsigned manifest before parsing. `mvmctl env update` verifies in-process. `install.sh` uses an installed verifier or cosign when available; on a fresh host it authenticates the baked archive against its carried hash before probing its `mvmctl`. A legacy or unpinned archive uses a separately hash-pinned temporary cosign, and an unpinned archive is never executed before its mandatory exact-tag bundle verifies. Every path refuses a missing or invalid signature | Shipped |
| 21 | OCI base images are OS-package- and kernel-inventoried, CVE-scanned against OSV, and SBOM-enumerated at pull time, and production admission refuses a missing scan, a sidecar bound to another digest, or any high/critical finding | fn:inventory_rootfs, fn:scan_inventory, fn:apply_base_image_gate, fn:a_prod_run_refuses_a_base_image_with_no_scan, fn:a_prod_run_refuses_a_base_image_with_a_high_severity_finding | the pull pipeline inventories the unpacked rootfs (dpkg status, apk installed, os-release, in-image kernel version; rpm is a named unsupported gap) and queries OSV directly over the repo HTTP client, writing `claims/<digest>.{cve.json,sbom.cdx.json}` beside the provenance record; `resolve_or_pull_run_image` applies the gate on every `--prod` run. Limits: the sidecars live in the host-controlled cache, so the gate verifies the scan's digest binding and verdict, not its honest production — a host insider can forge a clean sidecar (the image bytes themselves remain digest-pinned and dm-verity sealed); severity is best-effort (Debian/Alpine OSV records generally carry none, and `unknown` severities are reported but never gate-refusing); the gate covers registry-pulled images — the local ingest paths are dev-only | Shipped |
...
### Claim 3 backend scoping

Claim 3's witness, dm-verity, is
block-device-specific: it ratifies the claim on the **block+ext4** backends
— Firecracker and the in-process Option B materialize path. A
**virtiofs root** (Plan 221 Option A) serves a host directory, not a block
device, so it cannot be dm-verity-sealed. Virtiofs-root is a dev/local-tier
boot mechanism carrying an explicitly weaker contract
...
**Claim 20 boundaries — four refusing paths and the installer trust root.** The
three consuming paths inside `mvmctl` verify the Sigstore bundle in-process
against the embedded trust root and refuse a missing, unparseable, or
foreign-signed one. The installer reaches the same verdict through a bounded
bootstrap path.
...
- **Install path — refuses.** An installed `mvmctl` remains the preferred
  verifier and host `cosign` remains the fallback. On a host with neither,
  `install.sh` compares the archive with the target-specific SHA-256 carried by
  the published installer before extracting or executing only its `mvmctl` as
  a temporary verifier. A legacy archive that predates `env verify-release`
  falls back to a temporary cosign whose version and target hash are pinned in
  the installer. A non-default version may supply an independent
  `MVM_TRUSTED_ARCHIVE_SHA256`; without one, no archive byte executes before
  the pinned cosign verifies its exact-tag bundle. A mismatched supplied hash,
  missing bundle, invalid verifier hash, or invalid signature refuses before
  installation.
...
One implementation boundary remains outside the claim, and the installer has a
separate trust-root qualification:
...
- **The installer script is the fresh-host trust root.** Its baked target hashes
  are populated from the just-published release checksum manifest only after
  its bundle verifies under the exact release-workflow tag identity, before the
  stable installer is deployed. This authenticates the selected archive before
  its verifier capability is probed. Legacy releases use the separately
  code-reviewed cosign version and hashes carried in the same installer; its
  bytes are checked before execution. The subsequent Sigstore check
  independently authenticates the release archive. Pinning a different version
  cannot inherit the default release's hash, so it uses the independently
  pinned cosign unless the operator supplies an archive hash or a trusted
  verifier is already installed.
...
attestation store and verifiable with `gh attestation verify`. It witnesses this
claim because the attestation is signed under the same workflow identity and
binds the artifact to it — one more way the same authenticity holds.

It does **not** upgrade the claim to say anything about build *integrity*. This
is SLSA Build L2: the attestation is produced by the same workflow that produces
the artifact, so a compromised release job forges both. What provenance adds over
the signature is the commit and the build inputs in a machine-readable predicate,
not evidence the build was uncorrupted. `ci:reproducibility` under claim 7 is the
control that speaks to that, because an independent rebuild landing on the same
bytes requires no trust in this builder at all.

Provenance covers the tarballs — the same set the pipeline signs directly. The
kernels, root filesystems and metadata remain anchored by digest inside the
signed manifests, exactly as the statement says.
