...
- **Install path — refuses.** An installed `mvmctl` remains the preferred
  ...
  the installer. A non-default version must receive
  `MVM_TRUSTED_ARCHIVE_SHA256` from an independent trusted source. The selected
  authenticated verifier then requires the archive's bundle under the exact
  tag. A missing trust anchor, mismatched hash, missing bundle, or invalid
  signature refuses before installation.
...
- **The installer script is the fresh-host trust root.** Its baked target hashes
  ...
  cannot inherit the default release's hash, so it requires an independently
  obtained hash unless a trusted verifier is already installed.
