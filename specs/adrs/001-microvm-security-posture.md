...
- **Install path — refuses.** An installed `mvmctl` remains the preferred
  ...
  a temporary verifier. A non-default version must instead receive
  `MVM_TRUSTED_ARCHIVE_SHA256` from an independent trusted source. The
  authenticated verifier then requires the archive's bundle under the exact
  selected tag. A missing trust anchor, mismatched hash, missing bundle, or
  invalid signature refuses before installation.
...
- **The installer script is the fresh-host trust root.** Its baked target hashes
  ...
  stable installer is deployed. This authenticates the temporary verifier; the
  subsequent Sigstore check independently authenticates the release archive.
  Pinning a different version cannot inherit the default release's hash, so it
  requires an independently obtained hash unless a trusted verifier is already
  installed.
