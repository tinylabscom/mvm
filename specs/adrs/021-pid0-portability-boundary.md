...
**Status**: Proposed
**Date**: 2026-05-28
**Cross-refs**: ADR-001 (security posture), ADR-019 (guest protocol versioning & readiness), ADR-003 (passt virtio-net), ADR-020 (host services broker), ADR-020 (broker hardening), ADR-020 (broker rescope — drop secrets, add host.audit.v1), Plan 109 (guest control-layer dep-reduction + encryption design), Plan 25 (microVM hardening), Plan 64 (signed execution plans), Plan 104 (host services broker)
...
- The host services broker (ADR-020 / ADR-020 / ADR-020 / Plan 104) — the broker's 3 subprocesses (`mvm-broker`, `mvm-host-signer`, `mvm-audit-signer`) live on the **host**, not the guest. The guest is a *client* of the broker via vsock port `:5300`, but the broker is not part of the pid0 contract. (Pre-ADR-020 designs referenced a separate secrets channel on `:5301`; that port and `mvm-secrets-dispatcher` are gone as of ADR-020.)
...
The guest agent MUST implement the protocol-hello sequence from ADR-019:
...
The agent MUST emit `ReadinessStatus` events tracking the components named in ADR-019 §3:
...
- **Kernel cmdline contract** is a stable surface (ADR-001 §W3, Plan 27 / claim 3):
...
Any language change at the boundary MUST preserve these *byte-identically*. See ADR-001 (§"Consolidated from ADR-063 — Boundary Language Policy") for when non-Rust implementations are permitted at all.
...
- The host-side broker (ADR-020 / ADR-020 / ADR-020 / Plan 104) and its port `:5300` (pre-ADR-020 `:5301` is removed). The broker is host-side; the guest's *client* code that calls into the broker is in `mvm-sdk`, not the agent, and is not part of the pid0 contract.
...
- ADR-019 (guest protocol hello + readiness)
- ADR-003 (passt virtio-net / gvproxy)
- ADR-014 (no virtio-net bypass, claim 10)
- ADR-020 (host services broker over vsock — adjacent, not part of pid0)
