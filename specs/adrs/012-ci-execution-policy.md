...
A change that only breaks on macOS, under live KVM, or on the hosted no-KVM
...
platform has been exercised." The no-KVM lifecycle job was moved out of the
merge queue because a cold TCG build can take hours and serialized merges. It
uses stable hosted x86_64 with KVM deliberately denied because hosted arm64
runners repeatedly terminated it after 6–28 minutes; native aarch64 tests and
the local Apple Silicon lifecycle retain the architecture-specific evidence.
