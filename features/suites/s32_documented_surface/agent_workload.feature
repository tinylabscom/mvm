Feature: The agent workload example boots with a placeholder, not a key
  ...
  and sends a non-secret marker through the baked entrypoint's bounded per-call
  input. The smoke path does not contact a model API; it proves the guest
  received the host-minted placeholder and the pinned agent binary starts.
