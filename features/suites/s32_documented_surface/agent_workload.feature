Feature: The agent workload example boots with a placeholder, not a key
  ...
  Scenario: the documented agent workload runs without putting a key in the guest
    ...
    When I run the agent workload example with "machine run --flake examples/agent-workload --entrypoint --from-workload-ir examples/agent-workload/workload-smoke.json --allow-host api.anthropic.com:443 --stdin - --timeout 120"
