...
#[must_use]
#[cfg(any(target_os = "linux", test))]
pub(crate) fn cgroup2_is_mounted(mounts: &str) -> bool {
...
#[must_use]
#[cfg(any(target_os = "linux", test))]
pub(crate) fn cgroup2_unsupported(error: &MountError) -> bool {
...
";
        ...
        // A cgroup2 mounted anywhere else does not count.
        assert!(!cgroup2_is_mounted("cgroup2 /mnt/other cgroup2 rw 0 0\n"));
        // A non-cgroup2 fs at the mount point does not count.
    ...
    /// End-to-end where the host permits it (a Linux host with cgroup2 and
    /// mount privileges, e.g. the builder VM). Any refusal — no cgroup2, a
    /// container without CAP_SYS_ADMIN — skips rather than fails, exactly the
    /// posture the boot path itself takes.
    #[cfg(target_os = "linux")]
    #[test]
