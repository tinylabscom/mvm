...
//! Both reader and consumer handoffs use a bounded channel, a retention ring, and
...
//!
//! Order survives each stage: one FIFO, drained oldest-first, so what reaches
//! the wire preserves each pipe's order. Interleaving between stdout and
//! stderr reflects drain order, not cross-stream causality. The two are
//! independent pipes; completion tails need not arrive in cross-pipe order.
    ...
    std::thread::scope(|scope| {
        ...
        match pumping.join() {
            Ok((outcome, tail)) => {
                ...
                terminal_event(outcome, timeout)
            }
