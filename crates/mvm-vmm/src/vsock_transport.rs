...
impl FirecrackerTransport {
    ...
    pub fn for_vm(vm_name: &str) -> Result<Self> {
        let instance_dir = mvm_core::config::running_vm_dir(vm_name);
        Ok(Self::new(
            instance_dir,
...
impl DevConsoleTransport {
    ...
    ///
    /// The resolved host-side Unix socket path for `port`, if any.
    pub fn socket_path(&self, port: u32) -> PathBuf {
