...
//!
//! These tokens remain shared by the runner-backed libkrun, HVF, and QEMU
//! command-line builders. They live in `mvm-vmm` so concrete backends can
//! build cmdline tokens without depending on `mvm-runtime`.
//!
...
pub fn verb_grant_cmdline_token(vm_name: &str) -> Option<String> {
...
pub fn host_signer_pub_cmdline_token(vm_name: &str) -> Option<String> {
    let _ = vm_name;
