...
/// independent of the platform auto-detect. QEMU and in-house have no
/// persistent host VM, so they fail closed with guidance.
...
mod tests {
    ...
    #[test]
    fn persistent_backend_rejects_inhouse_with_guidance() {
        let err = persistent_backend(Some(BuilderBackendChoice::InHouse)).unwrap_err();
        let msg = format!("{err}");
        assert!(msg.contains("in-house"), "{msg}");
