...
//!   console.
//! - **macOS 26+ Apple Silicon (or explicit `--builder inhouse`)** →
...
enum DevBackend {
    ...
    Libkrun,
    /// macOS 26+ Apple Silicon — the in-house HVF backend
    /// (`mvm_backend::hvf_backend::HvfBackend`, Hypervisor.framework).
...
///
/// Selection order:
///   1. Builder override prefers in-house **and** in-house is available →
///      `InHouse`. Catches both `--builder inhouse` (folded into the env
///      at startup by `commands::run`) and a bare
///      `MVM_BUILDER_BACKEND=inhouse`.
...
///      libkrun even on a macOS 26+ box where in-house would otherwise
///      win rule 3.
///   3. macOS 26+ Apple Silicon (the in-house default tier) → `InHouse`.
///   4. macOS with libkrun → Libkrun (legacy fallback).
///   5. Linux with KVM → LinuxKvm.
///   6. Otherwise → Unsupported.
///
/// On the *auto-detect* path (no explicit override) in-house still wins
/// over Libkrun (rule 3 vs 4) per the CLAUDE.md "Dev mode" rationale:
/// the in-house HVF backend is the documented Stage 0 boundary AND the
/// build boundary the builder-VM selection (`mvm_build::
/// builder_backend_select`) already defaults to on this tier. Splitting
/// the dev VM (Libkrun) from that build boundary would leave each under
/// a runtime the other never starts. An explicit `prefers_libkrun` is
/// the one signal that overrides this — because it also moves the
/// build boundary onto libkrun, keeping both halves on the same VMM.
///
/// `is_vz_default_tier` names the underlying `Platform` predicate
/// (`Platform::is_vz_default_tier`, the macOS-26-Apple-Silicon tier
/// check); it is reused here as the in-house-default-tier signal since
/// both backends share that tier boundary. Renaming the platform method
/// itself is tracked separately and out of scope for this change.
fn select_dev_backend(
    plat: Platform,
    prefers_inhouse: bool,
    ...
    // 1. Builder override picked in-house AND in-house is actually usable.
    if prefers_inhouse && has_inhouse {
        return DevBackend::InHouse;
    }
    // 2. Explicit libkrun override on macOS — run the dev VM on libkrun
    //    even where in-house would win auto-detect, so the dev VM rides
    //    the same VMM the build path uses.
...
fn builder_prefers_inhouse() -> bool {
    ...
    matches!(resolve_choice(), BuilderBackendChoice::InHouse)
}
...
fn current_backend() -> DevBackend {
    ...
    select_dev_backend(
        plat,
        builder_prefers_inhouse(),
        builder_prefers_libkrun(),
        // No dedicated `Platform::has_hvf()` probe exists yet; the
        // in-house HVF backend is only supported/detected on the same
        // macOS-26-Apple-Silicon tier `is_vz_default_tier()` already
        // identifies, so reuse it here too.
...
///
/// The libkrun dev VM and an in-house HVF dev VM can both bind the
/// runtime name `dev_vz::DEV_VM_NAME` ("mvm-dev"). LibkrunBackend's
/// status probe only sees its *own* state, so without an explicit
/// cross-check a `dev up` on libkrun would silently leave a stale
/// in-house "mvm-dev" running (e.g. after an operator switches macOS
/// tiers or overrides between runs). The in-house dev path has no
/// separate persistent-builder supervisor of its own to reap it, so
/// this dance stays libkrun-only, mirroring the pre-Plan-226 Vz
/// version of this check.
///
/// Returns `Some("inhouse")` when an in-house "mvm-dev" is currently
/// running, otherwise `None`. Errors propagated — a status probe
/// failure shouldn't be silently swallowed (it may mean the backend is
/// wedged, which is its own kind of "running").
fn inhouse_dev_vm_running() -> Result<Option<&'static str>> {
...
/// Pure: the info line to print when libkrun takes over a stale
/// in-house dev VM (None = stay quiet). Lifted out for hermetic unit
/// tests; the caller passes the probe result so the test doesn't need
/// to spoof the live host.
...
/// When a stale in-house `mvm-dev` is up, stop it and take over instead
/// of refusing. The dev VM is an ephemeral builder; both backends share
...
fn take_over_stale_inhouse_dev_vm() -> Result<()> {
    let probe = inhouse_dev_vm_running()?;
    if let Some(line) = switch_notice(probe) {
        ui::info(&line);
        let id = VmId(dev_vz::DEV_VM_NAME.to_string());
        if let Err(e) = HvfBackend.stop(&id) {
...
fn cmd_dev_libkrun(
    ...
    // Take over a stale in-house dev VM rather than refusing.
    take_over_stale_inhouse_dev_vm()?;
...
fn cmd_dev_libkrun_down(json: bool) -> Result<bool> {
    ...
    // Best-effort stop of the in-house side too (no-op if not running;
    // status check + stop is cheap).
    if let Ok(VmStatus::Running | VmStatus::Starting | VmStatus::Paused) = HvfBackend.status(&id)
        && let Err(e) = HvfBackend.stop(&id)
    {
        tracing::warn!(error = %e, "best-effort in-house dev VM stop failed during libkrun dev down");
    }
...
fn cmd_dev_libkrun_status(json: bool) -> Result<()> {
    ...
    if let Ok(Some(other)) = inhouse_dev_vm_running() {
        ui::info(&format!(
...
/// Boot the dev VM via the in-house HVF backend
/// (`mvm_backend::hvf_backend::HvfBackend`). Mirrors [`cmd_dev_libkrun`]:
/// a plain `VmBackend::start` with the shared dev image, no persistent
/// snapshot/park support (the in-house backend doesn't have one yet —
/// see `HvfBackend`'s doc comment).
fn cmd_dev_inhouse(
    cpus: u32,
    memory_gib: u32,
    open_shell: bool,
    ...
    if matches!(backend.status(&id)?, VmStatus::Running) {
        ui::success("in-house dev VM already running.");
        if open_shell {
            console::console_interactive(dev_vz::DEV_VM_NAME)?;
        }
        return Ok("already-running");
    ...
    ui::progress("Starting dev environment via in-house HVF...");
    let (kernel, rootfs) = dev_vz::ensure_dev_image()?;
    let memory_mib = memory_gib.saturating_mul(1024);
    let config = VmStartConfig {
        name: dev_vz::DEV_VM_NAME.to_string(),
    ...
    backend.start(&config)?;
    ui::success("Dev environment ready (in-house HVF).");
    if open_shell {
        console::console_interactive(dev_vz::DEV_VM_NAME)?;
    }
    Ok("started")
...
/// Stop the in-house dev VM. Mirrors [`cmd_dev_libkrun_down`]; no
/// cross-backend best-effort stop here because the libkrun stop path
/// already best-effort-stops a stale in-house VM (see
/// `inhouse_dev_vm_running`/`take_over_stale_inhouse_dev_vm`).
fn cmd_dev_inhouse_down(json: bool) -> Result<bool> {
    let id = VmId(dev_vz::DEV_VM_NAME.to_string());
    let was_running = matches!(
        HvfBackend.status(&id),
        Ok(VmStatus::Running | VmStatus::Starting | VmStatus::Paused)
...
fn cmd_dev_inhouse_status(json: bool) -> Result<()> {
    let id = VmId(dev_vz::DEV_VM_NAME.to_string());
    ...
    if json {
        // The in-house backend has no in-guest kernel probe wired here yet;
        // the dev-image + builder caches are backend-agnostic, so report
        // them too.
        return crate::json_out::emit_json(&dev_vz::build_dev_status_json("inhouse", state, None));
    }
    ui::info("Backend:  in-house HVF (Hypervisor.framework)");
    ui::info(&format!("VM:       {}", dev_vz::DEV_VM_NAME));
    ui::info(&format!("Status:   {state}"));
    Ok(())
}
...
pub(in crate::commands) fn run(_cli: &Cli, args: Args, cfg: &MvmConfig) -> Result<()> {
    ...
    // `current_backend()` honours `MVM_BUILDER_BACKEND=inhouse` /
    // `--builder inhouse` (folded into env at startup by `commands::run`)
    // directly, routing the dev VM through `HvfBackend`.
    ...
    match action {
        DevAction::Up {
            ...
            if dev_base.is_some() {
                anyhow::bail!(
                    "`mvmctl dev up --base` is not supported by any dev backend right now \
                     (it was Vz-only; the Vz dev backend has been removed)"
                );
            }
            ...
            let outcome = match backend {
                ...
                DevBackend::InHouse => {
                    warn_dev_volumes_unsupported(&dev_volumes, "in-house HVF");
                    cmd_dev_inhouse(effective_cpus, effective_mem, open_shell, &dev_volumes)
                }
                DevBackend::LinuxKvm => {
        ...
        DevAction::Down { reset, json } => {
            let was_running = match backend {
                ...
                DevBackend::InHouse => cmd_dev_inhouse_down(json),
        ...
        DevAction::Park { json: _ } => {
            ...
            match backend {
                ...
                DevBackend::InHouse => anyhow::bail!(
                    "`mvmctl dev park` requires a persistent-snapshot-capable dev backend; \
                     the in-house HVF dev VM does not support one yet (the Vz dev backend \
                     did, and has been removed)"
                ),
                DevBackend::LinuxKvm => {
                    anyhow::bail!(
                        "`mvmctl dev park` requires a persistent-snapshot-capable dev backend"
        ...
        DevAction::Status { json } => match backend {
            ...
            DevBackend::InHouse => cmd_dev_inhouse_status(json),
            ...
            DevBackend::Unsupported => {
                ...
                ui::info(
                    "Dev environment: not configured on this host (in-house HVF dev VM \
        ...
        DevAction::Rebuild {
            ...
            let _ = match backend {
                ...
                DevBackend::InHouse => cmd_dev_inhouse_down(false),
            ...
            match backend {
                ...
                DevBackend::InHouse => {
                    warn_dev_volumes_unsupported(&dev_volumes, "in-house HVF");
                    cmd_dev_inhouse(effective_cpus, effective_mem, shell, &dev_volumes).map(|_| ())
                }
                DevBackend::LinuxKvm => {
                    warn_dev_volumes_unsupported(&dev_volumes, "native Linux/KVM");
...
mod tests {
    ...
    #[test]
    fn switch_notice_libkrun_takes_over_inhouse() {
        let line = switch_notice(Some("inhouse"))
            .expect("a running in-house VM must yield a switch notice");
        assert!(line.contains("inhouse → libkrun"), "got: {line}");
        assert!(
    ...
    #[test]
    fn switch_notice_silent_when_no_stale_inhouse() {
        // No leftover in-house VM → nothing to announce, boot proceeds.
    ...
    #[test]
    fn builder_inhouse_with_inhouse_available_picks_inhouse_dev_backend() {
    ...
    #[test]
    fn builder_inhouse_on_default_tier_still_picks_inhouse() {
        // Explicit in-house override (rule 1) and the macOS-26
        // auto-detect tier (rule 3) both land on InHouse; the override
        // path is exercised here independent of the default tier.
        assert_eq!(
    ...
    #[test]
    fn builder_inhouse_without_inhouse_available_falls_through() {
        // Selection layer should refuse in-house on Linux already, but
        // the dev dispatch defends in depth: if somehow in-house was
        // chosen but it isn't actually usable, fall through to the
        // standard tree instead of returning a backend that can't run.
        assert_eq!(
            select_dev_backend(
    ...
    #[test]
    fn auto_detect_picks_inhouse_on_default_tier() {
        // Auto-detect tree (no explicit override): the macOS-26
        // in-house default tier wins over libkrun even though both are
        // present (CLAUDE.md "Dev mode" rationale). Regression guard
        // that auto-detect libkrun does *not* divert off in-house —
        // only an explicit ask does (next test).
        assert_eq!(
            select_dev_backend(
                Platform::MacOS,
                /* prefers_inhouse */ false,
                /* prefers_libkrun */ false,
                true,
                /* is_vz_default_tier */ true,
                true,
    ...
    #[test]
    fn explicit_libkrun_overrides_inhouse_default_tier() {
        // The symmetric counterpart to the in-house override: an
        // operator who sets MVM_BUILDER_BACKEND=libkrun on a macOS 26+
        // box (in-house default tier) gets the dev VM on libkrun, not
        // in-house.
        assert_eq!(
            select_dev_backend(
                Platform::MacOS,
    ...
    fn explicit_libkrun_without_libkrun_falls_through() {
        // Defend in depth (mirrors the in-house fall-through): if
        // libkrun was asked for but isn't actually installed, don't
        // return a Libkrun backend that can't run — fall through to the
        // auto-detect tree (the in-house default tier).
        assert_eq!(
            select_dev_backend(
                Platform::MacOS,
                /* prefers_inhouse */ false,
    ...
    fn builder_libkrun_falls_back_to_libkrun_on_older_macos() {
        // macOS 13-25 (not the in-house default tier) + libkrun installed.
        assert_eq!(
            select_dev_backend(
                Platform::MacOS,
                /* prefers_inhouse */ false,
    ...
    fn unsupported_host_returns_unsupported() {
        // macOS without in-house and without libkrun (Intel macOS,
        // macOS pre-13), Linux without KVM, etc.
        assert_eq!(
            select_dev_backend(
                Platform::MacOS,
                /* prefers_inhouse */ false,
