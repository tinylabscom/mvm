//! Dev VM shared infrastructure + bundled image fetching.
//!
...
//! `cmd_dev_vz_down`) kept only so cleanup/cache-clear guards and the
//! console picker can still detect and reap a stale persistent Vz dev
//! VM left over from before this change on an upgraded host.
...
// ============================================================================
// Dev environment (shared dev-VM infra; legacy Vz-specific probes retained
// below for stale-VM detection only — see module doc comment)
// ============================================================================
...
///
/// Retained only for the legacy stale-Vz-VM probes (`is_vz_dev_running`,
/// `dev_vsock_proxy_path`, `cmd_dev_vz_down`) — no code path creates a
/// new persistent Vz dev VM anymore.
#[cfg(feature = "builder-vm")]
...
/// Check if a *persistent Vz* dev VM is running *and* reachable
/// cross-process.
///
...
/// guest-agent socket to connect keeps this probe honest with what
...
/// Stop a stale persistent Vz dev VM by reaping its detached supervisor
/// via the PID file under the stable state dir. Returns whether a live
