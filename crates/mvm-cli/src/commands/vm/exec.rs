...
impl From<RunProfile> for mvm_client::launch::profile::RunProfile {
    fn from(profile: RunProfile) -> Self {
        match profile {
            RunProfile::Restrictive => Self::Restrictive,
            RunProfile::Standard => Self::Standard,
            RunProfile::Dev => Self::Dev,
            RunProfile::Permissive => Self::Permissive,
        }
    }
}

impl From<mvm_client::launch::profile::RunProfile> for RunProfile {
    fn from(profile: mvm_client::launch::profile::RunProfile) -> Self {
        match profile {
            mvm_client::launch::profile::RunProfile::Restrictive => Self::Restrictive,
            mvm_client::launch::profile::RunProfile::Standard => Self::Standard,
            mvm_client::launch::profile::RunProfile::Dev => Self::Dev,
            mvm_client::launch::profile::RunProfile::Permissive => Self::Permissive,
        }
    }
}

pub(crate) use mvm_client::launch::profile::ProfileGrants;

/// The flag's view of the profile policy, which lives in `mvm-client` so the
/// host library applies the same one.
impl RunProfile {
    /// The name the CLI, the receipt, and the docs all use.
    pub(crate) fn as_str(self) -> &'static str {
        mvm_client::launch::profile::RunProfile::from(self).as_str()
    }

    /// Parse the name a persisted machine spec stored. `None` for an unknown
    /// name, never a default.
    pub(crate) fn from_name(name: &str) -> Option<Self> {
        mvm_client::launch::profile::RunProfile::from_name(name).map(Self::from)
    }

    /// What this profile permits.
    pub(crate) fn grants(self) -> ProfileGrants {
        mvm_client::launch::profile::RunProfile::from(self).grants()
    }

    /// One line describing what this profile permits, for `doctor` and help.
    pub(crate) fn summary(self) -> String {
        mvm_client::launch::profile::RunProfile::from(self).summary()
    }
}
...
mod tests {
    ...
    fn the_receipt_records_the_profile_that_ran() {
        for profile in mvm_client::launch::profile::RunProfile::ALL.map(RunProfile::from) {
