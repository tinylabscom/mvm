...
pub(crate) fn refuse_unsupported_dir_shares(
    ...
    let shares = dir_shares
        .iter()
        .map(|s| format!("{}:{}", s.host_dir, s.guest_mount))
        .collect::<Vec<_>>()
        .join(", ");
    anyhow::bail!(
...
mod tests {
    ...
    /// `--mount` on a backend without virtio-fs shares is refused from the
    /// arguments, and the message names the backend.
    ///
    /// Only HVF serves directory shares, so on every other backend this flag
    /// used to be accepted, paid for — runtime detection, image resolution and
    /// preparation, VM directory setup — and only then rejected inside the
    /// runner. The refusal naming the *backend* is the part a caller can act
    /// on; the runner's own message names the volume, which they already knew.
    #[test]
    fn a_dir_share_is_refused_up_front_on_a_backend_without_virtiofs() {
        ...
        // The same request on a backend that serves shares is not refused.
        refuse_unsupported_dir_shares("hvf", true, &shares)
            .expect("a backend with virtio-fs shares must accept --mount");
    }

    /// No mounts, no refusal — the guard must not reject an ordinary launch,
    /// including on a backend that cannot serve shares at all.
    #[test]
