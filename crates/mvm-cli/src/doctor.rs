...
struct DoctorReport {
    ...
    /// Surfaces the honest capability matrix — Firecracker live-memory,
    /// libkrun disk-only — so a user can predict which backend
...
fn runtime_backend_check(plat: platform::Platform) -> Check {
    ...
    let info = match plat {
        ...
        Platform::Wsl2 => "WSL2 — a workload runtime needs nested /dev/kvm; without it \
            ...
            .to_string(),
        Platform::MacOS => "macOS — `hvf` (macOS 26+) or `libkrun` (macOS 13-25) \
...
///
/// Reads the persistent dev builder VM's host-side `console.log`. The
/// libkrun persistent path keys off an opaque per-run job id, so there is no
/// analogous fixed helper to probe there.
#[cfg(feature = "builder-vm")]
fn builder_egress_check() -> Check {
    let log_path = mvm_build::legacy_builder_vm::dev_builder_legacy_macos_console_log();
...
/// Stub when the `builder-vm` feature is off — the builder console-log helper
...
fn builderd_daemon_summary(vms_root: &std::path::Path) -> String {
    ...
    for entry in entries.flatten() {
        ...
        // A builder VM may expose `builderd` directly at `<dir>/vsock-<port>.sock`
        // or under `<dir>/vsock/vsock-<port>.sock`; probe whichever socket the
...
/// resumes from RAM (Firecracker live-memory) vs. reboots from a disk snapshot
/// (libkrun) before relying on a warm start.
...
fn nested_kvm_check(plat: Platform) -> Check {
    if !matches!(plat, Platform::LinuxNative) {
        return Check {
            ...
            category: "platform",
            ok: true,
            info: "n/a (Linux-only — macOS hosts use hvf/libkrun; Plan 100 W6 affects Linux only)"
                .to_string(),
        };
    }
...
/// prereq is reported by the platform-level `libkrun_check`; this
/// check is about the *selection*, not the availability.
#[cfg(feature = "builder-vm")]
...
fn builder_residency_check() -> Check {
    ...
    let session = builder_residency_session_summary(
        ...
        builder_legacy_macos_snapshot_present(&vms_root),
    );
    Check {
...
#[cfg(feature = "builder-vm")]
fn builder_legacy_macos_snapshot_present(vms_root: &std::path::Path) -> bool {
    let Ok(entries) = std::fs::read_dir(vms_root) else {
        return false;
    };
    entries.flatten().any(|entry| {
        let dir = entry.path();
        dir.is_dir() && mvm_build::legacy_builder_vm::builder_snapshot_path(&dir).is_file()
    })
}
...
fn builder_residency_session_summary(
    ...
    legacy_macos_snapshot_present: bool,
) -> &'static str {
    match kind {
        mvm_core::residency::ResidencyKind::Parked if legacy_macos_snapshot_present => {
        ...
        _ if legacy_macos_snapshot_present => "parked snapshot present",
...
mod tests {
    ...
    fn builder_egress_check_classifies_a_fixture_console_log() {
        ...
        let log = mvm_build::legacy_builder_vm::dev_builder_legacy_macos_console_log();
    ...
    fn collect_capability_table_reports_per_backend_dispositions() {
        ...
        // The Tier 3 `mock` test double is excluded; the real
        ...
        assert!(
            !fc.tap_networking,
            "firecracker workload egress is vsock-only (no host TAP device)"
        );
        assert!(fc.vsock);
        assert!(fc.balloon);
        assert!(
        ...
        assert!(
            !krun.tap_networking,
            "libkrun workload egress is vsock-only (no host TAP)"
        );
        assert!(krun.vsock);
        assert!(
            krun.standby_pool,
        ...
        assert!(
            by("legacy-macos").is_none(),
            "legacy LegacyMacos must not appear in the current workload-facing capability table"
        );
    }
    ...
    // hosts get covered by the existing `legacy_macos_check_macos_reports_*`
    ...
    fn builder_backend_check_linux_honors_env_override() {
        ...
        assert!(
            c.info.starts_with("hvf — "),
            "expected hvf-resolved line under env override; got: {}",
            c.info
        );
        assert!(
        ...
        assert!(
            c.info.contains("HVF builder"),
            "expected hvf availability segment on Linux override; got: {}",
            c.info
        );
    }
    ...
    #[test]
    fn builderd_daemon_summary_finds_a_legacy_macos_shaped_socket() {
        use std::os::unix::net::UnixListener;
        // Regression for the live LegacyMacos boot: the LegacyMacos supervisor nests the
        ...
        let sock = mvm_build::builderd::builderd_legacy_supervisor_control_socket_path(&vm_dir);
    ...
    #[cfg(feature = "builder-vm")]
    #[test]
    fn builder_legacy_macos_snapshot_present_detects_state_vzsave_under_vms_root() {
        let root = tempfile::tempdir().unwrap();
        assert!(!builder_legacy_macos_snapshot_present(root.path()));
        let vm_dir = root.path().join("mvm-persistent-builder-legacy-macos-dev");
        std::fs::create_dir_all(&vm_dir).unwrap();
        assert!(!builder_legacy_macos_snapshot_present(root.path()));
        std::fs::write(
            mvm_build::legacy_builder_vm::builder_snapshot_path(&vm_dir),
            b"snapshot",
        )
        .unwrap();
        assert!(builder_legacy_macos_snapshot_present(root.path()));
