...
fn flake_content_for_preset(preset: &str) -> Result<&'static str> {
    match preset {
        ...
        "python-pandas" => Ok(include_str!(
            "../resources/template_scaffold/flake-python-pandas.nix"
        )),
        other => anyhow::bail!(
            "Unknown preset {:?}. Valid presets: minimal, http, postgres, worker, python, python-pandas",
            other
        ),
    }
}
...
fn preset_manifest_defaults(preset: &str) -> (u8, u32) {
    match preset {
        "minimal" => (1, 256),
        "postgres" => (2, 1024),
        "http" | "worker" | "python" | "python-pandas" => (2, 512),
        _ => (2, 512),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_python_pandas_preset_renders_flake_with_pandas() {
        let flake = flake_content_for_preset("python-pandas").expect("python-pandas preset exists");
        assert!(
            flake.contains("ps.pandas"),
            "flake must install pandas: {flake}"
        );
        assert!(
            flake.contains("ps.numpy"),
            "flake must install numpy: {flake}"
        );
        assert!(
            flake.contains("ps.requests"),
            "flake must install requests: {flake}"
        );
    }

    #[test]
    fn test_python_pandas_manifest_defaults_match_catalog() {
        let toml = render_manifest_toml("demo", "python-pandas");
        assert!(
            toml.contains("vcpus = 2"),
            "python-pandas preset gets 2 vcpus: {toml}"
        );
        assert!(
            toml.contains("mem = \"512M\""),
            "python-pandas preset gets 512M: {toml}"
        );
    }
