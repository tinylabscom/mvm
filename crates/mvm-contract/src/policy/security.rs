...
pub struct SessionHello {
    ...
    /// Host's ephemeral X25519 public key for this session.
    pub host_ephemeral_pubkey: Vec<u8>,
}
...
pub struct SessionHelloAck {
    ...
    /// Guest's ephemeral X25519 public key for this session.
    pub guest_ephemeral_pubkey: Vec<u8>,
    /// Challenge generated by the guest and signed by the host in the confirm.
    pub guest_challenge: Vec<u8>,
}

/// Host → Guest: confirm that the host authenticated the guest's ephemeral key.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SessionHelloConfirm {
    /// Protocol version the host supports.
    pub version: u8,
    /// Echo back the session identifier.
    pub session_id: String,
    /// Host signature over the complete handshake transcript.
...
mod tests {
    ...
    fn test_session_hello_serde_roundtrip() {
        let hello = SessionHello {
            ...
            host_ephemeral_pubkey: vec![1u8; 32],
        };
    ...
    fn test_session_hello_ack_serde_roundtrip() {
        let ack = SessionHelloAck {
            ...
            guest_ephemeral_pubkey: vec![1u8; 32],
            guest_challenge: vec![2u8; 32],
        };
    ...
    #[test]
    fn test_session_hello_confirm_serde_roundtrip() {
        let confirm = SessionHelloConfirm {
            version: PROTOCOL_VERSION_AUTHENTICATED,
            session_id: "sess-002".to_string(),
            transcript_signature: vec![3u8; 64],
        };

        let json = serde_json::to_string(&confirm).unwrap();
        let parsed: SessionHelloConfirm = serde_json::from_str(&json).unwrap();

        assert_eq!(parsed.version, PROTOCOL_VERSION_AUTHENTICATED);
        assert_eq!(parsed.session_id, "sess-002");
        assert_eq!(parsed.transcript_signature.len(), 64);
    }
