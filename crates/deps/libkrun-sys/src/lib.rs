...
pub struct SupervisorAttachConfig {
    ...
    /// open egress on a pool hit. `None` only for pre-policy callers; the merge
...
impl SupervisorConfig {
    ...
    pub fn from_base_and_attach(
        ...
        Ok(SupervisorConfig {
            ...
            // cold-boots deny-all would silently run open egress on a pool hit
...
mod base_attach_tests {
    ...
    fn merge_threads_network_policy_onto_the_merged_config() {
        ...
        // of run_bridge_inner lowers it), not silently dropped to open egress.
        let deny =
