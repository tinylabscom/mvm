...
///
/// Public so host-side clients can read the same sidecar through the same
/// parser instead of re-rolling its wire format. Callers must still verify the
/// signed grant against the host trust anchor before using it as authority.
pub fn read_verb_grant_envelope(vm_name: &str) -> Result<Option<VerbGrantEnvelope>> {
