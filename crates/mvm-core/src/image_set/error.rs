...
pub enum ImageSetError {
    ...
    #[error(
        "the local image set declares no compatibility.builder_boot_abi; its manifest \
         predates the builder boot ABI and must be regenerated by rebuilding the set"
    )]
    LocalSetPredatesBuilderBootAbi,
...
impl ImageSetError {
    ...
    pub fn stage(&self) -> ImageSetStage {
        match self {
            ...
            | Self::LocalSetPredatesBuilderBootAbi => ImageSetStage::Structure,
