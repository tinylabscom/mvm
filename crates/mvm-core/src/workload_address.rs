...
//!
//! That taxonomy separates *what* is identified. Storage representations add
//! an orthogonal axis: an OCI layer is `tar+gzip`, so its verified layer digest
//! identifies stored bytes while `diff_id` identifies the unpacked content;
//! similarly, a sealed transcript records the digest of its ciphertext.
