//! Host-signer subprocess wire protocol.
//!
...
//! audit-signer subprocesses). The host-signer holds the key alone —
//! today as a software in-memory key; eventually as a hardware-enclave
//! handle (Apple Secure Enclave / Linux TPM 2.0).
//!
...
pub enum SignRequest {
    /// Sign an `ExecutionPlan`. The supervisor's admission ceremony
    /// calls this once per workload.
    SignPlan {
...
pub enum HostSignerErrorCode {
    /// Subprocess is still loading the key (e.g. waiting on TPM init).
    /// Caller should retry; supervisor should not pass this through
    ...
    /// (software-fallback path) the in-memory key was never
    /// initialised. Caller treats as fatal for the current request;
    /// retry is doomed.
    KeyUnavailable,
    /// HW enclave returned an error (only emitted on the enclave
    /// path; the software path never sets this). Carries the
...
mod tests {
    ...
    fn sign_response_ok_accepts_ecdsa_p256_alg() {
        // The byte is reserved today; the host-signer rejects it on
        // the verify side until the Secure Enclave path is wired. But
        // the envelope itself round-trips so the wire surface is stable
        // across the software → enclave transition.
        let resp = SignResponse::Ok {
