...
impl AnyBackend {
    ...
    /// `BackendKind::Libkrun` etc. instead of string-matching `name()`. The
    /// runner-role variant reports the same in-house VMM kind as the raw
    /// `Hvf` variant.
...
mod tests {
    ...
    fn vz_selector_no_longer_resolves_to_vz() {
        ...
        assert_ne!(
            b.name(),
            "vz",
            "vz selector must fall back to default after removal"
        );
    }
    ...
    fn test_any_backend_from_hypervisor_vz_falls_back_to_default() {
        ...
        for alias in ["vz", "virtualization"] {
            ...
            assert_eq!(
                backend.name(),
                "firecracker",
                "alias {alias} must fall back to the default backend"
            );
        }
    }
    ...
    fn for_started_vm_no_longer_resolves_vz_by_marker() {
        ...
        assert!(
            result.is_none(),
            "vz.pid marker is no longer a recognized backend"
        );
    }
