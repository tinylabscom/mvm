...
mod tests {
    ...
    {
        let _legacy_guard = HOME_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::tempdir().expect("tempdir");
        let mut env = mvm_core::util::test_env::TestEnv::new();
        env.set("HOME", tmp.path());
        // `meta_path` resolves via `vm_state_dir`, which prefers MVM_DATA_DIR`
        // over $HOME — clear it so these $HOME-rooted assertions stay local to
        // this tempdir even if another test previously overrode the data dir.
